<template>
  <div>
    <el-drawer
    title=""
    :visible.sync="viewSensitiveDialogVisible"
        ...
        <div class="headBar">
            ...
            <div class="titleIcon">
              <p>
                ...
                <img
                  ...
                  :style="{display:isAdminOrUser?'inline-block':'none'}"
                />
              </p>
          ...
          <div class="rightSlide">
            <div>
              ...
              <el-button type="primary" icon="el-icon-plus" @click="addSensitive" v-if="!isAddSensitive">添加到应用</el-button>
              <el-button type="primary" icon="el-icon-remove-outline" @click="removeSensitive" v-else>从应用中移除</el-button>
            </div>
        ...
        <div class="flex">
          ...
          <span class="flex" style="cursor: pointer" >
            ...
            <!-- <span style="color: #1747E5; font-size: 16px; vertical-align: middle" @click="batchSettings()">
                ...
                {{$t("setType")}}</span> -->
            <span style="color: #1747E5; font-size: 16px; vertical-align: middle" @click="deleteData('', 2)" v-show="isAdminOrUser">
          ...
          <div style="margin-top: 16px">
            ...
            >
              ...
              <el-table-column :label="$t('action')" width="120">
                <template slot-scope="scope">
                  <el-button type="text" size="small" @click="editMenu(scope.row)" :disabled="!isAdminOrUser">{{
                    $t("edit")
                  }}</el-button>
                  <el-button type="text" size="small" @click="deleteData(scope.row, 1)" :disabled="!isAdminOrUser">{{
                    $t("delete")
                  }}</el-button>
                </template>
              </el-table-column>
...
export default {
  ...
  computed:{
    isAddSensitive(){
      if(this.sensitiveArr.length==0) return false
      return this.sensitiveArr.some(item=>item==this.sensitiveRow.id)
    },
    isAdminOrUser(){
      let obj=JSON.parse(sessionStorage.getItem('user'))
      console.log(obj.accountName,this.sensitiveRow.createUserAccount);
      return obj.powerType==0 || obj.accountName==this.sensitiveRow.createUserAccount
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    addSensitive(){
      const arr=[...this.sensitiveArr]
      arr.push(this.sensitiveRow.id)
      this.$emit("update:sensitiveArr",arr)
      this.$emit("update:viewSensitiveDialogVisible",false)
      this.$emit("changeSensitive")
      this.$message({
        type:"success",
        message:"操作成功"
      })
    ...
    removeSensitive(){
      const arr=[...this.sensitiveArr]
      const index = arr.findIndex(item=> item== this.sensitiveRow.id);
      arr.splice(index,1)
      this.$emit("update:sensitiveArr",arr)
      this.$emit("update:viewSensitiveDialogVisible",false)
      this.$emit("changeSensitive")
