<template>
  ...
  >
    <div class="radioOuter flex-center just">
      ...
      <div v-if="showUrlBtn" style="color: ##BCC1CC;font-size:14px;margin-top: 6px">您现在可通过互联网访问该应用</div>
      <p class="tips" v-if="params.publishStatus == '2'">
        {{ $t("APIKeyAccess") }}
      </p>
    </div>
    <span slot="footer" class="dialog-footer">
      <el-button @click="cancelConfig" style="width: 124px">{{ $t("close") }}</el-button>
      <el-button
        type="primary"
        style="background: #1c50fd;width: 124px"
        v-if="params.publishStatus == '1' || showUrlBtn"
        @click="openWindows"
