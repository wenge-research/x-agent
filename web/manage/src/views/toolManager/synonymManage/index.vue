      ...
      <div class="inner">
        <div style="display: flex; justify-content: space-between; margin-bottom: 20px">
          ...
          <div style="float: right; height: 40px">
            ...
            @click="addRoleDialog"><i class="el-icon-circle-plus" style="margin-right: 8px;"></i>{{ $t("createSynonyms") }}</el-button>
        ...
        <div v-loading="loading">
          <div style="margin-top: 16px">
            <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" class="table"
              ...
              <el-table-column prop="wordCount" :label="$t('numberOfSynonyms')"></el-table-column>
...
import addDialog from "@/views/toolManager/synonymManage/components/addDialog";
import editSensitiveLibrary from "@/views/toolManager/synonymManage/components/editSensitiveLibrary";
import associatedApplications from "@/views/toolManager/synonymManage/components/associatedApplications";
import dataImportDialog from "@/views/toolManager/synonymManage/components/dataImportDialog";
export default {
  components: {
    addDialog,
    editSensitiveLibrary,
  ...
  methods: {
    ...
    async deleteData(row) {
      this.$confirm(`${this.$t("confirmDelete")}?`, `${this.$t("tips")}`, {
        ...
        try {
          let res = await deleteSynonymWord([row.id]);
          if (res.code == "000000") {
    ...
    addRoleDialog() {
      ...
      keyWord: "",
      type: "",
      synonymWordList: [{ content: "" }] // 显式初始化synonymWordList
    };
      this.dialogVisible = true;
    },
    closeDialog() {
    ...
    async getSynonymWordList() {
      ...
      const res = await getSynonymWordList(params);
      if (res.code == "000000") {
    ...
    getSynonymWordKeyList() {
      getSynonymWordKeyList().then((res) => {
        if (res.code == "000000") {
