<template>
  <div class="dynamic-params">
    ...
    >
      ...
      <el-table-column prop="type" :label="$t('paramType')">
        <template slot-scope="scope">
    ...
    <span class="add-btn" @click="addParameter(null)"><em>+</em>{{$t('addParam')}}</span>
  </div>
</template>
...
export default {
  ...
  methods: {
    addParameter(parent) {
      ...
      const newRow = {parent:parent, id: newId, name: '', value: '', type: '', children: []};

        this.$set(parent.children, parent.children.length, newRow);
      } else {
        const newRow = {id: newId, name: '', value: '', type: '', children: []};
    ...
    deleteParameter(index, row) {
      ...
      if (row.parent) {
        ...
        const i = this.parameters.findIndex(child => child.id === row.id);
