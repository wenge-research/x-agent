...
<script>
  import configOutput from '@/views/toolManager/pluginManage/components/configOutput.vue';
  ...
  export default {
    ...
    watch: {
      dataList: {
        handler (newVal, oldVal) {
          this.$emit("updateDataList", JSON.stringify(this.dataList));
        },
        deep: true,
      },
    },
    mounted() {
      ...
      this.dataList = this.nodeStart.output.map(item =>{
        item.enabled = true
        return item
      })
