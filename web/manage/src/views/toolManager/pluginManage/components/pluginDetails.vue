    ...
    <div>
      <div class="headBar">
        ...
        <div class="rightSlide">
         ...
         <!-- <div>
          ...
          <el-button class="btns btnsXian" v-permission="'issue'" @click="submitApp" :disabled="!isEditEnabled" v-if="!isStore">
      ...
      <div style="padding: 0;">
        ...
        <div class="elTabs" style="margin-top: 20px">
            ...
            <el-tab-pane :label="$t('applicationInvocation')" name="first" class="no-scrollbar" style="height: calc(100vh - 330px);overflow: auto;">
                ...
                >
                    ...
                    ></el-table-column>
                    <el-table-column prop="required" :label="$t('isRequired')">
                    <template slot-scope="scope">
                        <span v-if="scope.row.required"> 是 </span>
                        <span v-else> 否 </span>
                    </template>
                    </el-table-column>
                    <el-table-column prop="desc" :label="$t('describe')">
                    </el-table-column>
                </el-table>
                </div>
                <div class="plugin-bd-tit">
...
export default {
  ...
  computed:{
    isEditEnabled(){
      return JSON.parse(sessionStorage.getItem('user')).powerType==0 || this.appConfigForm.ownerType&&this.appConfigForm.ownerType!='official'
    },
    isStore(){
      return this.$route.fullPath.includes("storemanage")
    }
  },
  watch: {
    // 监听 panels 数组的变化
  ...
  mounted() {
    ...
    console.log("this.appConfigForm",this.$route)
  ...
  methods: {
    ...
    queryWorkflowDetail() {
      queryWorkflowDetail({
        ...
        .then((res) => {
          if (res.code == "000000") {
            this.tableData = res.data.nodes.find(
                (el) => el.nodeName === "开始"
            ).output
            this.tableDataResult = res.data.nodes.find(
                (el) => el.nodeName === "结束"
            ).input
          }
        })
