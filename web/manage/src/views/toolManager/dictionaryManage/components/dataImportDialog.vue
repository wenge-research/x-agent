...
<script>
import { downloadInterceptWordDataTemp, importInterceptWordData } from "@/api/toolManager";
export default {
  props: {
    dialogDataVisible: {
      type: Boolean,
  ...
  methods: {
    async submitAddUser() {
      ...
      this.uploadForm.append("interceptWordHouseId", this.sensitiveRow.id);
      let res = await importInterceptWordData(this.uploadForm)
      if (res.code == "000000") {
    ...
    async handleDownloadKnowledgeDataTemp() {
      let res = await downloadInterceptWordDataTemp();
      let data = res;
      console.log(data);
