<template>
  <div class="qa-view-wrapper" style="position:relative">
    <div class="qa-view-left">
      <div class="left-search">
        <el-input v-model="filterText" size="small" :placeholder="$t('enterKnowledgetreeName')" clearable>
      ...
      <el-tree ref="tree" :data="categoryData" :props="defaultProps" v-loading="themeLoading" default-expand-all
        @node-click="handleNodeClick" :filter-node-method="filterNode" node-key="id">
      ...
      <div class="tree-btn" @click="themeVisible = true" v-if="isAdminOrUser">
    ...
    <div class="qa-view-right">
      <div class="right-header">
        <div class="form-box">
          <el-form :inline="true" :model="seachform">
            <el-form-item :label="$t('isPreciseAnswer')">
              <el-select clearable v-model="seachform.accurate" :placeholder="$t('pleaseSelectIsPreciseAnswer')"
                @change="getKnowledgeDataListData" style="width: 132px">
                <el-option v-for="item in ACCURATE_OPTIONS" :key="item.id" :label="item.type" :value="item.label">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="">
              <el-input v-model="seachform.title" class="my-input" :placeholder="$t('pleaseEnterQuestionOrAnswer')"
        ...
        <div class="btns-box">
          <el-button plain @click="openDrawer" class="btn-1"
            style="color: #1c50fd; border-color: #1c50fd; height: 40px" :disabled="!isAdminOrUser"><iconpark-icon name="file-copy-line" size="16"
              color="#1c50fd" style="margin-right: 8px"></iconpark-icon>相似问题合并</el-button>
          <el-button class="icon-export" plain @click="exportQa" style="height: 40px">
            <svg width="16" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="margin-right: 8px;">
              <path fill-rule="nonzero"
                d="M13 10h5l-6 6-6-6h5V3h2v7Zm-9 9h16v-7h2v8a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1v-8h2v7Z"
                data-follow-fill="#848587" />
            </svg>{{ $t("exportDate") }}</el-button>
          <el-button type="primary" plain @click="addQaDialog" :disabled="!isAdminOrUser"><i class="el-icon-plus" style="margin-right: 8px;"></i>{{
            $t("add")
          }}</el-button>
          <el-button type="primary" @click="importVisible = true" :disabled="!isAdminOrUser">
            <i class="el-icon-document-add" style="margin-right: 8px;"></i>{{ $t("dataImport")
            }}</el-button>
        </div>
      </div>
      ...
      <div class="table-btns">
        <div class="selected-num">
          {{ $t("selected") }}<span class="num">{{ multipleSelection.length }}</span>
        </div>
        <el-checkbox v-model="isSelectedAll" @change="handleSelectedAll">{{
          $t("all")
        }}</el-checkbox>
        <el-button type="text" :disabled="multipleSelection.length === 0" @click="handleClearSelection">{{
          $t("clearSelected") }}</el-button>
        <el-popconfirm :title="$t('confirmDelete')" @confirm="deleteInBatches" v-if="isAdminOrUser"
          popper-class="deleteIn-batches-popconfirm">
          <el-button type="text"  :disabled="multipleSelection.length === 0" slot="reference" icon="el-icon-delete">{{
      ...
      <div class="table-box">
        <el-table ref="table" v-loading="tableLoading" class="my-table" :data="tableData" height="100%"
          ...
          <el-table-column prop="status" :label="$t('enabledStatus')" width="120">
            <template slot-scope="scope">
              <el-switch v-model="scope.row.status" :active-value="$t('yes')" :inactive-value="$t('no')"
                active-color="#1747E5" inactive-color="#EAECF0" :width="24" :disabled="!isAdminOrUser"
                :class="[scope.row.status == $t('yes') ? 'switch-on' : 'switch-off']"
                @change="handleStatusChange(scope.row)">
              </el-switch>
              <span class="switch-status-text">{{
                scope.row.status == $t("yes") ? $t("active") : $t("inactive")
              }}</span>
            </template>
          </el-table-column>
          <el-table-column fixed="right" :label="$t('action')" width="90">
            <template slot-scope="scope">
              <div class="btns">
                <el-button type="text" @click="setQaDialogue(scope.row)" :disabled="!isAdminOrUser">{{
                  $t("edit")
                }}</el-button>
                <el-button type="text" @click="remQaDialogue(scope.row.id)" :disabled="!isAdminOrUser">{{
                  $t("delete")
                }}</el-button>
              </div>
            </template>
      ...
      <el-drawer class="my-drawer" :title="setForm.id ? $t('editQa') : $t('addQa')" :visible.sync="addQaVisible"
        :modal-append-to-body="false" :close-on-click-modal="false" :close-on-press-escape="false"
        :wrapperClosable="false" size="29.2%" append-to-body>
        <div slot class="qa-box">
          <div class="content">
            <el-form :model="setForm" ref="setForm">
              ...
              <el-form-item :label="$t('topic')">
                <el-cascader v-if="addQaVisible" clearable :placeholder="$t('pleaseSelectTopic')"
                  :props="{ value: 'id', label: 'type', checkStrictly: true }" v-model="setForm.category"
                  :options="categoryData"></el-cascader>
              </el-form-item>
              <el-form-item label="">
                <div class="switch-item">
                  ...
                  <el-switch v-model="setForm.suggest" :active-value="$t('recommend')"
                    :inactive-value="$t('notRecommend')" active-color="#1747E5" inactive-color="#EAECF0" :width="24"
                    :class="[
                      setForm.suggest == $t('recommend') ? 'switch-on' : 'switch-off',
                    ]">
                  </el-switch>
                </div>
              ...
              <el-form-item label="">
                <div class="switch-item">
                  ...
                  <el-switch v-model="setForm.accurate" :active-value="$t('yes')" :inactive-value="$t('no')"
                    active-color="#1747E5" inactive-color="#EAECF0" :width="24"
              ...
              <el-form-item label="">
                <div class="switch-item">
                  ...
                  <el-switch v-model="setForm.polishFlag" :active-value="$t('yes')" :inactive-value="$t('no')"
                    active-color="#1747E5" inactive-color="#EAECF0" :width="24" :class="[
                      setForm.polishFlag == $t('yes') ? 'switch-on' : 'switch-off',
                    ]">
                  </el-switch>
                </div>
          ...
          <div class="footer">
            <el-button :disabled="submitLoading" @click.stop="addQaVisible = false">{{
              $t("cancel")
            }}</el-button>
            <el-button :loading="submitLoading" type="primary" @click.stop="handleAddQaDialog">{{ $t("confirm")
              }}</el-button>
          </div>
        </div>
      </el-drawer>
      ...
      <el-dialog :title="$t('tips')" :visible.sync="remVisible" width="400px" :modal-append-to-body="false"
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button :loading="remLoading" type="primary" @click="handleRemQaDialog">{{
            $t("confirm")
          }}</el-button>
        </span>
      </el-dialog>

      <el-dialog class="my-dialog" :title="$t('importExcelFile')" :visible.sync="importVisible" width="720px"
        ...
        <el-upload action="#" drag accept=".xlsx" :class="isDrag ? 'el-upload-dragger' : 'drag'"
      ...
      <el-dialog class="my-dialog catalog-manage-dialog" :title="$t('operateTopic')" :visible.sync="themeVisible"
        width="560px" :modal-append-to-body="false" :close-on-click-modal="false" :close-on-press-escape="false"
        append-to-body>
        <el-tree :data="categoryData" :props="defaultProps" v-loading="themeLoading" default-expand-all draggable
          :expand-on-click-node="false">
          <div class="theme-item" slot-scope="{ node, data }" @mouseenter="mouseenter(data)"
            @mouseleave="mouseleave(data)">
            <div v-if="renthemeVisible && typeId == node.data.id" class="info"
              :class="{ 'theme-name-input': renthemeVisible && typeId == node.data.id }">
              <el-input ref="themeName" v-model="themeName" :placeholder="$t('enterKnowledgetreeName')" clearable
                @blur="handleRenTheme(name)">
              </el-input>
            </div>
            <div v-else-if="data.isAddingChild" class="add-child-input">
              <el-input ref="newChildInput" v-model="newChildName" @blur="handleAddTheme(newChildName)"
                :placeholder="$t('pleaseEnter')"></el-input>
            </div>
            <template v-else>
              <span class="info">
                <!-- <i :class="node.expanded ? 'el-icon-folder-opened' : 'el-icon-folder'"></i> -->
              ...
              <div class="options">
                ...
                <img v-if="node.label !== '全部'" class="icon" src="@/assets/images/edit-line2.svg" alt=""
                  @click="rentheme(node)" />
        ...
        <el-input v-if="addFirstLevelVisible" ref="themeName" v-model="themeName" :placeholder="$t('pleaseEnter')"
          clearable @blur="handleAddTheme(themeName)">
        </el-input>
        <!-- <div class="tree-btn">
          <el-button type="text" icon="el-icon-plus" @click="handleAddFirstLevel">{{
            $t("addFirstLevel")
          }}</el-button>
        </div> -->
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog :title="$t('addTheme')" :visible.sync="addthemeVisible" width="400px" :modal-append-to-body="false"
        ...
        <span slot="footer" class="dialog-footer">
          <el-button :loading="addthemeLoading" @click="addthemeVisible = false">{{
            $t("cancel")
          }}</el-button>
          <el-button :loading="addthemeLoading" type="primary" @click="handleAddTheme">{{
            $t("confirm")
          }}</el-button>
        </span>
      </el-dialog>
      ...
      <el-dialog :title="$t('deleteTopic')" :visible.sync="remthemeVisible" width="400px" :modal-append-to-body="false"
        ...
        <span>{{ $t("confirmDelete") }}</span>
        <span slot="footer" class="dialog-footer">
          <el-button :loading="remthemeLoading" @click="remthemeVisible = false">{{
            $t("cancel")
          }}</el-button>
          <el-button :loading="remthemeLoading" type="primary" @click="handleRemTheme">{{
            $t("confirm")
          }}</el-button>
        </span>
      </el-dialog>
      <!-- 相似问题合并 -->
      <MergeSimilarIssues ref="MergeSimilarIssues" v-model="drawerVisible" :knowledgeId="knowledgeId"
        @closeDrawer="closeDrawer" @openHistory="openHistory" />
      <!-- 历史记录 -->
      <HistoricalRecords ref="HistoricalRecords" v-model="historyVisible"
        @closeHistoricalRecordsDrawer="closeHistoryDrawer" :knowledgeId="knowledgeId" />
    </div>
    <el-dialog :modal="false" :visible.sync="progressVisible" width="800" :show-close=false>
...
import {
  ...
  ACCURATE_OPTIONS,
  IMPORT_OPTIONS,
} from "@/views/Kbm/content/index";
import {
  getKnowledgeDataList,
  importKnowledgeData,
  getKnowledgeDataTypeList,
  addKnowledgeData,
  deleteKnowledgeData,
  addKnowledgeDataType,
  updateKnowledgeDataType,
  deleteKnowledgeDataType,
} from "@/api/index.js";
import MergeSimilarIssues from "./mergeSimilarIssues";
...
export default {
  components: { MergeSimilarIssues, HistoricalRecords },
  props: {
    knowledgeId: {
      type: String,
      default: "",
  ...
  mounted() {
    ...
    this.ws.onerror = (error) => {
      console.log(error, '发生错误了')

    }
    this.ws.onmessage = (event) => {
      console.log('接收到消息了', JSON.parse(event.data))
      this.progress = JSON.parse(event.data).nums?.toFixed(1)
      if (this.isHandle == true) {
        this.ws.close()
      }
      if (JSON.parse(event.data).business == '知识库数据正在保存' && this.progress >= 100) {
        // this.isHandle=true
        // ws.close()
  ...
  data() {
    return {
      ...
      rules: {
        title: [
          { required: true, message: this.$t("pleaseEnterQuestion"), trigger: "blur" },
        ],
        content: [
          { required: true, message: this.$t("pleaseEnterAnswer"), trigger: "blur" },
        ],
        ...
        suggest: [
          {
            required: true,
            message: this.$t("pleaseSelectIsRecommended"),
            trigger: "change",
          },
        ],
        accurate: [
          {
            required: true,
            message: this.$t("pleaseSelectIsPreciseAnswer"),
            trigger: "change",
          },
        ],
        polishFlag: [
          {
            required: true,
            message: this.$t("pleaseSelectIsRefinementNeededn"),
            trigger: "change",
          },
        ],
      },
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      ...
      this.isSelectedAll =
        val.length === this.pageSize || val.length === this.tableData.length;
    },
    addQaDialog() {
      this.setForm.title = "";
      ...
      this.setForm.id
        ...
        : (this.setForm.category = this.categoryData[0]?.id || []);
    ...
    restartWebSocket() {
      console.log('重连~~~~')
      this.ws = new WebSocket(`wss://localhost/smart-agent-api-uat/model-agent-ws/${userId}`)
      this.ws.onopen = () => {
        this.ws.send('连接成功')
    ...
    closeProgress() {
      console.log('啊啊啊啊啊参数啊啊啊啊啊啊阿')
      this.isHandle = true
      console.log(this.isHandle, 'handle现在是？')
      this.progress = 0
      this.progressVisible = false
      this.tableLoading = true
      this.getKnowledgeDataListData()
    ...
    async getKnowledgeDataListData() {
      ...
      const { data } = await getKnowledgeDataList({
        ...
        knowledgeId: this.knowledgeId,
        ...this.seachform,
        category: Array.isArray(this.seachform.category)
          ? this.seachform.category.join("/")
          : this.seachform.category,
      });
    ...
    async handleAddQaDialog() {
      if (this.setForm.category.length == 0) {
        this.$message({
          type: 'warning',
          message: '请选择一个主题！'
        })
    ...
    async handleDownloadKnowledgeDataTemp() {
      window.location.href =
        process.env.VUE_APP_API_NEW + "/knowledgeData/downloadKnowledgeDataTemp"; // 生产
    },
