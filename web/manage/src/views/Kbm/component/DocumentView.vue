<template>
  <div class="document-view">
    <div class="left view-left-wrapper" v-loading="foldersLoading">
      ...
      <div class="tree-btn" @click="dialogVisible = true" v-if="isAdminOrUser">
    ...
    <div class="view-right-wrapper" :style="{ width: rightWidth + 'px' }">
      <div class="right-header">
        ...
        <div class="btns-box">
          ...
          > -->
          <el-button class="icon-import" type="primary" @click="importfolder" :disabled="!isAdminOrUser">
            <svg style="margin-right: 8px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill-rule="nonzero" d="m12 12.586 4.243 4.242-1.415 1.415L13 16.415V22h-2v-5.587l-1.828 1.83-1.415-1.415L12 12.586ZM12 2a7.001 7.001 0 0 1 6.954 6.194A5.5 5.5 0 0 1 18 18.978v-2.014a3.5 3.5 0 1 0-1.111-6.91 5 5 0 1 0-9.777 0 3.5 3.5 0 0 0-1.292 6.88l.18.03v2.014a5.5 5.5 0 0 1-.954-10.784A7 7 0 0 1 12 2Z" data-follow-fill="#848587"/></svg>{{$t('importFileData')}}</el-button
          >
        </div>
      </div>
            ...
            <template slot-scope="scope">
              <div class="btns">
                ...
                <!-- <el-button
                  ...
                  >{{$t('preview')}}</el-button
                > -->
                <el-button type="text" @click="remFileData(scope.row.fileId)" :disabled="!isAdminOrUser"
                  >{{$t('delete')}}</el-button
                >
              </div>
            </template>
    ...
    <!-- <el-upload action="#" drag accept=".xlsx" :before-upload="beforeupload" :show-file-list="false">
      ...
      <el-upload
        ...
        accept=".docx, .doc, .pdf, .txt, .ofd, .pptx, .ppt, .html"
        @dragenter.native="handleDragEnter"
        @dragleave.native="handleDragLeave"
         @drop.native="handleDrag"
        :class="isDrag?'el-upload-dragger':''"
      >
        <img class="icon-upload" src="@/assets/images/upload-cloud-2-fill.svg" alt="">
        <div class="el-upload__text" v-show="!isDrag">{{$t('dragFileHere')}}{{$t('or')}}{{$t('clickUpload')}}</div>
        <div class="el-upload__condition" v-show="!isDrag">
          <div>{{$t('supportedFormatsOne')}}、html</div>
          <div>{{$t('supportedFormatsTwo')}}</div>
        </div>
          ...
          <template slot-scope="scope">
            <div class="file-wrapper">
              ...
              <div class="file-name">
                ...
                <div class="size">{{formatFileSize(+scope.row.size.replace('kb','')*1024)}}</div>
              </div>
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleFolderChange(data, node, ary) {
       this.folderId = data.foldersId === this.allFolderId ? '' : data.foldersId;
    ...
    async getFoldersListData() {
      ...
      this.allFolderId = data.records.find((item) => {
        return item.name == '全部'})?.foldersId
      this.folderId = this.folderId ? this.folderId : this.allFolderId
      this.$nextTick(() => {
    ...
    beforeupload(file) {
      this.isDrag = false
      const allowedTypes = ['docx', 'doc', 'pdf', 'txt', 'ofd','pptx', 'html','ppt'];
      ...
      if (file.size > maxFileSize) {
        this.$message({
          message: this.$t('fileSizeExceeded'), // 提示：文件大小超过限制
          type: "warning",
        });
        return false;
      }
    ...
    async getFileListData(value) {
      ...
      const { data } = await getFileList({
        ...
        folderId: value === this.allFolderId ? '' : value,
        knowledgeId: this.knowledgeId,
