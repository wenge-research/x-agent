<template>
  <div :class="['kbm-create', isAppManage && widthSide == 240 ? 'kbm-create-fill' : '', isAppManage && widthSide != 240 ? 'kbm-create-min' : '']">
    ...
    <div class="main">
      ...
      <div class="con">
        <QAView :knowledgeId="knowledgeId" v-if="['1'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser"/>
        <DocumentView :knowledgeId="knowledgeId" v-if="['2'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser" />
        <UrlView :knowledgeId="knowledgeId" v-if="['3'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser"/>
        <ApiView :knowledgeId="knowledgeId" v-if="['4'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser"/>
        <StructuringView :knowledgeId="knowledgeId" v-if="['5'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser"/>
        <MultiMedia :knowledgeId="knowledgeId" v-if="['6'].includes(activeConfig)" :isAdminOrUser="isAdminOrUser"/>
...
export default {
  ...
  computed: {
    ...
    isAdminOrUser(){
      ...
      return obj.powerType==0 || obj.accountName==this.rowData.createUser
    }
  },
  watch: {
    // isCollapse(val) {
  ...
  methods: {
    ...
    async getKnowledgeInfoListData() {
      ...
      this.rowData.knowledgeName = rowData.knowledgeName
