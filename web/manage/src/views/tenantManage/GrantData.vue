<template>
  <div class="grant-data">
    <div class="grantData">
      <div class="grantData-item padding16" v-loading="userLoading" style="width: 328px">
        <div>
          <el-input
        ...
        <ul class="padding16">
          ...
          >
            <div class="flex">
              ...
              <span style="cursor: pointer" @click="removeSelectedUser(item)">
...
export default {
  ...
  methods: {
    ...
    getSelectedIdsForCurrentPage() {
      return this.getSelectedUsersForCurrentPage()
    },
    // 获取当前页已选用户的ID
    ...
    getData(condition) {
      ...
      if (!condition) {
        this.getAllUserByTenant(this.tenantId);
      }
    },
    userInput(value) {
      this.pageNo = 1
    ...
    saveGrant() {
      ...
      let param = {
        ...
        userIdList: targetIdList,
      };
      updateTenantUser(param).then((res) => {
        if ("000000" === res.code) {
          this.$message({
   ...
   getAllUserByTenant(tenantId) {
      getAllUserByTenant({ tenantId }).then((res) => {
        this.allSelectedUsers = res.data;
        this.selectedUsers = res.data.map(item => item.id);
      });
    },
    // 删除选中
    toggleSelection() {
