<template>
  <div class="plug-in">
    ...
    <div v-loading="loading">
      <div style="margin-top: 16px">
        ...
        >
          ...
          <el-table-column prop="publishType" :label="$t('classification')" width="280">
            <template slot-scope="scoped">
              <div v-if="scoped.row.publishType">
                <div class="table-classification" v-for="(item, index) in arrayPublishType(scoped.row.publishType)" :key="index">
                  {{ item }}
                </div>
              </div>
            </template>
    ...
    <ListingReviewDetailsPlugin v-model="drawer" :sourceData="reviewData" @closeDrawer="closeDrawer" />
  </div>
</template>

<script>
// components
import ListingReviewDetailsPlugin from './listing-review-details-plugin';
import { apiGetLabelTypes, apiGetListPage } from "@/api/app";
export default {
  components: { ListingReviewDetailsPlugin },
  data() {
    return {
