...
export default class Pop extends Base {
  ...
  #pop;
	...
	constructor() {
		const random = Math.floor(Math.random() * (9999999999 - 100000 + 1)) + 100000; 
		super();
		this.attachShadow({ mode: "open" });
		this.adoptedStyle(style);
    this.shadowRoot.innerHTML = `
    <div id="pop-${random}" class="pop" data-popover="manual" part="pop">
      <slot></slot>
    </div>
    `
    this.#pop = this.shadowRoot.getElementById(`pop-${random}`);
    // this.#pop = this.shadowRoot.getElementBy;
  ...
  get isSuportPopOver() {
    ...
    if (!this.node) return
		...
		this.style.setProperty("--top", parseInt(top + window.pageYOffset + -6));
