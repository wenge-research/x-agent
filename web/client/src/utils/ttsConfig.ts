// ttsConfig.js
import { getTtsConfig } from '/@/api/chat';
...
export async function getCurTtsConfig(applicationId) {
  const responseData = await getTtsConfig({ applicationId: applicationId });
	if (responseData?.code === '000000') {
		localStorage.setItem(`ttsConfig`, JSON.stringify(responseData.data));
	}
}