<template>
	<div ref="messageRef" class="items-start w-full yy-message-mobileUniversalTemplate"
		...
		<div class="items-start max-w-full fontSize16" style="max-width: calc(100vw - 78px);overflow: hidden;">
			...
			<div class="relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
				...
				<div v-show="!inversion &&!loading && !chatStore.isShare" class="btns adaption">
					<!-- 播放 -->
					...
					<div class="zhuanfa" @click="handleShare">
						<iconpark-icon name="share-forward-line" color="#797F8A" size="20"></iconpark-icon>
								...
								<div v-if="appId != '20'" style="position:relative;flex: 1;display: flex;flex-wrap: nowrap;justify-content: flex-end">
									<!-- 踩和赞 暂时隐藏 关芯客服H5 -->
                <el-popover :visible="isupVisible" placement="top" :width="270" >
                  <div class="popcontent">
                    <h2 style="margin-bottom:5px;">{{ chatStore.zanForm.popTitle }}</h2>
                  ...
                  <template #reference>
                    <div style="width: 32px;height: 32px;margin-right:8px;display: flex;justify-content: center;align-items: center;" class="dianzan">
                      ...
                      <!-- <CoolThumbUpLineWe v-if="!isupVisible" size="18" />
                      ...
                      <iconpark-icon name="thumb-up-line" size="18" color="#494C4F"></iconpark-icon>
								...
								<el-popover :visible="isdownVisible" placement="top" :width="270" >
                  <div class="popcontent">
                    <h2 style="margin-bottom:5px;">{{ chatStore.caiForm.popTitle }}</h2>
                    <w-checkbox-group v-model="caiValue" direction="vertical">
                      <w-checkbox v-for="(item, index) in chatStore.caiForm.options" :key="index"
                        :value="item.id">{{ item.content }}</w-checkbox>
                    </w-checkbox-group>
                    <w-textarea v-model="caiText" placeholder="您认为更理想的答案是什么" :max-length="1000"
                      show-word-limit :auto-size="{ minRows: 6, maxRows: 10 }" />
                  </div>
                  <div style="text-align: right; margin-top:15px;">
...
watch(
	...
	(newVal: any) => {
		console.log('==>>>>>>>>>>>watch');
		isupVisible.value = newVal == 1 ? true : false;
...
.yy-message-mobileUniversalTemplate {
	display: flex;
	margin: 16px 0;
	.ml8 {
		margin-left: 8px;
