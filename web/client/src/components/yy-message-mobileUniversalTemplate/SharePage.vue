  ...
  <div class="popup-overlay" @click="close">
    <div class="popup-content">
      ...
      <div class="share-content" ref="shareRef">
        ...
        <div class="main-cont" :class="{isQRcode:isQRcode}">
          <div v-for="(item, index) of isSelected" :key="item.id" class="main" :class="{ hide: !chatStore.hidden[index] }" >
            ...
            <div class="checkBox" @click.stop="handleSelected(item, index)" v-if="checkBoxHidden && !isQRcode">
        ...
        <div v-if="isQRcode" style="position: relative;bottom: 20px;display: flex;  width: 100%;; background-color: #FFFFFF; padding: 16px;">
          <div  style="flex: 1.7;" >
            <img style="width: 160px;position: relative;top: 20px;" src="../../assets/logo-new.png" alt="" />
          </div>
          <div  style="flex: 1;position: relative;"  >
            <qrcode-vue :value="qrcodeValue" :size="100" />
          </div>
        </div>
      </div>
      <div class="share-footer">
        <div v-if="!isQRcode" style=" display: flex; width: calc(100% - 66px); ">
          <el-button @click.stop="handleSaveImg('fuzhi')" style="flex: 1;
...
height: 48px;">
          ...
          <el-button @click.stop="handleSaveImg('baocun')" style="flex: 1; 
...
height: 48px;">
        ...
        <div v-if="isQRcode" style=" display: flex; width: calc(100% - 66px); ">
          <el-button @click.stop="SaveImg('baocun')" :loading="saveImgFlag" style="flex: 1; 
            background: #2065D6!important; 
border-radius: 8px!important;
font-family: MiSans, MiSans!important;
font-weight: 400!important;
...
import { isWechatBrowser } from '/@/utils/tool';
...
const captureImg2 = async (element) => {
  try {
    ...
    // 计算完整高度，添加额外的缓冲值（例如 20px）
    const fullHeight = element.scrollHeight + 50;
    ...
    const domPng = await domtoimage.toPng(element, {
      height: fullHeight,
      width: element.clientWidth,
      quality: 0.95, // 提高质量
      bgcolor: '#ffffff',
...
const SaveImg = async (type) => {
  if (type == 'baocun') {
    ...
    if (result) {
      if(isWechat){
        imagePreview.value?.openPreview(result.base64);
      } else {
