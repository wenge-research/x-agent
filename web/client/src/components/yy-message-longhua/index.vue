<template>
	<div ref="messageRef" class="flex w-full items-start"
		:class="[{ 'flex-row-reverse': inversion, 'mb-30': inversion, 'mt-30': inversion, 'mb-3': !inversion }, className]">
		<div :class="['flex-shrink-0', inversion ? 'ml-18' : 'mr-18']">
		...
		<div class="fontSize16 max-w-full items-start">
			...
			<div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
				...
				<TextComponent v-else :inversion="inversion" :syFlag="syFlag" :loading="loading" :citations="citations"
					:dialogueFileIds="dialogueFileIds" :dialogueFolderIds="dialogueFolderIds" :paragraph="paragraph || ''"
					:skillId="skillId" :text="inversion ? question : answer" :as-raw-text="false" :asRawText="asRawText"
					:params="param" :imgUrl="imgUrl" v-model:isHighlightCode="isHighlightCode">
					<template #footer>
						<!-- <div @click="opendialog" style="cursor:pointer;display:flex;align-items:center;position:absolute;top: 0px;right: 0;font-family: MiSans, MiSans;font-weight: 400;font-size: 14px;color: #646479;line-height: 20px;text-align: left;line-height: 5px;font-style: normal;width: 108px;height: 36px;background: rgba(64, 133, 244, .12);border-radius: 0px 12px 0px 8px;">
							<span style="display:inline-block;width:14px;height:14px;border-radius:50%;border:1px solid rgb(100, 100, 121);margin-right:3px;position: relative;top: 2px;">
              ...
              <div class="relate-source" v-if="syList.length>0">
								<span class="title">相关推荐 {{syList.length}}</span>
								<ul>
									<li  v-for="(item, index) in syList" :key="index">
                    <div v-if="item.fileLink">
                      <w-link :href="item.fileLink" v-if="item.sourceType === 4"  target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                      <w-link v-else-if="previewDocFlag()" @click.stop="openPreview(item)" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                      <w-link v-else :href="item.fileLink" target="_blank" icon  >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
											...
											<w-link v-else-if="item.sourceType!=1" :href="item.fileLink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link> -->
                    </div>
									</li>
								</ul>
							</div>
                  ...
                  <img style="height: 20px" src="/src/assets/zc/send1.png" />
							...
							<div class="btns adaption bottom-btns">
								...
								<div class="rebuildBtn">
									...
									<span style="color:#646479;font-size:16px;">免责声明</span>
									</w-button>
								</div>
                      ...
                      <CoolThumbUpFillWe v-else size="14" />
									...
									<w-tooltip :content="!iscopy ? '复制' : '已复制'" placement="top" mini>
										<w-button type="text" shape="circle" @click="handleSelect('copyText')">
											<CoolFuzhi_9oe2ecmc v-if="!iscopy" size="16" style="margin-right: 6px" />
											<CoolCheckLineWe v-else size="16" style="margin-right: 6px" />
											复制
										</w-button>
									</w-tooltip>
									<!-- 虚拟园区隐藏播放小图标 -->
									<w-tooltip content="播放" placement="top" mini>
										<w-button v-if="!inversion && route.path.includes('/knowledgeDetailLh/')" type="text" shape="circle" @click="handleBofang">
											<CoolVolumeUpLineWe size="16" />
										</w-button>
									</w-tooltip>
								</div>
							</div>
...
import { computed, ref, watch, onBeforeMount, defineAsyncComponent, onMounted, onUnmounted } from 'vue';
...
onMounted(() => {
	...
	// 缓存3条数据
	if (props?.cacheSyList?.length) {
		syList.value = props.cacheSyList;
	}
});
...
const sourceAnswerList = async () => {
	...
	if (res?.code == '000000') {
		...
		if (list.length > 0) {
			...
			chatStore.cacheSyList(syList.value, sessionStorage.getItem('dialogueId'));
			setScrollPosition(isMobile.value ? '.message-list' : '.center-side', 'auto');
		} else {
			syList.value = [];
		}
...
.bottom-btns {
	...
	align-items: center;
	height: 50px;
	border-top: 1px solid rgba(38, 146, 207, 0.35);
	margin-top: 15px;
	padding-top: 15px;
}
