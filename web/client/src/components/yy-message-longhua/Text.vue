...
const wrapClass = computed(() => {
  return [
    ...
    props.inversion ? "text-message-request" : "",
    props.inversion ? "border-right" : "border-left",
    props.inversion ? "longhua-question" : "longhua-answer",
  ];
});
...
const maxHight = computed(() => {
  ...
  let background = import.meta.env.VITE_MSG_BACKGROUND;
  if (!background || "yes" !== background) {
    cssList.push("leading-relaxed");
  }
  return cssList;
});
...
<template>
  <div
    ...
    :class="[...wrapClass]"
  >
    <div ref="textRef" :class="[...maxHight, 'break-words']">
      <!-- 知识库 -->
      <div v-if="!inversion">
        <div
          v-if="!asRawText && text"
        ...
        <div class="markdown-body icon-box" v-if="!asRawText && !text">
          ...
          <i><img src="/src/assets/longhuaNewUI/loading.png" alt="" /></i>
          <span style="margin-left: 12px"> AI飞速思考中，请稍等...</span>
        </div>
        <div v-if="!inversion && !loading && syFlag" v-html="geTailTalk()" />
...
.longhua-question {
  ...
  box-shadow: 0 0 10px rgba(93, 155, 255, 0.35) !important;
  line-height: 1.6 !important;
  padding: 15px 30px !important;
  margin-top: 5px !important;
  background: #4e87ed !important;
...
.longhua-answer {
  ...
  color: #fff !important;
  background: url("/src/assets/longhuaNewUI/QABg.png") no-repeat center !important;
  background-size: cover !important;
  .text-message-reply > div {
