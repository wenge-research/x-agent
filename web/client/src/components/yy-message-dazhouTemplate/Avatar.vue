...
<template>
  ...
  <template v-if="image">
    <w-avatar v-if="getAppInfo()?.userIcon" :size="28"><img :src="getAppInfo()?.userIcon || defaultAvatar" /></w-avatar>
  </template>
  <template v-else>
    <w-avatar v-if="getAppInfo()?.robotIcon" :size="28" style="margin-top: 0"><img :src="getAppInfo()?.robotIcon || BotAvatar" /></w-avatar>
  </template>
</template>
