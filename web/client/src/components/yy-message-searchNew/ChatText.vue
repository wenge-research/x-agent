...
const addEventListeners = () => {
	...
	const handleMouseOver = (event) => {
		...
		if (target?.classList?.contains('ragTag-docx')) {
			...
			// 设置相对于元素的偏移
			tooltipStyle.value = {
				top: `${elementTop + target.offsetHeight - 26}px`,  // 在元素下方5px
    ...
    const handleClick = (event) => {
		...
		if (target?.classList?.contains('ragTag-docx')) {
	        ...
	        mittBus.emit('handleClickIndex', (index - 1));
		}
	};
...
<template>
	<div class="text-black" style="background-color: transparent; position: relative;overflow: scroll;" :class="wrapClass">
		<div ref="textRef" class="leading-relaxed break-words" :class="maxHight">
			...
			<div v-if="!inversion">
				<div
					...
					v-html="highlightText(text, props.searchTitle)"
				/>
		...
		<div v-if="showTooltip && !inversion" ref="refTooltip" class="tooltip" @mouseleave="handleTooltipMouseLeave"
			:style="tooltipStyle">
