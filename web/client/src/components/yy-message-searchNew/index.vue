	...
	<div ref="messageRef" class="flex w-full items-start"
		...
		<div class="fontSize16 max-w-full items-start">
			...
			<div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
				<w-tooltip v-if="inversion && route.path.includes('/knowledgeDetails')" placement="tr" :arrow-style="{ right: 0, left: 'inherit' }" :update-at-scroll="true"
				...
				<TextComponent v-if="!inversion&&route.path.includes('/knowledgeDetails')" :inversion="inversion" :loading="loading" :citations="citations"
					:dialogueFileIds="dialogueFileIds" :dialogueFolderIds="dialogueFolderIds" :paragraph="paragraph || ''"
					:skillId="skillId" :text="inversion ? question : answer" :as-raw-text="false" :asRawText="asRawText"
                    ...
                    <w-link v-if="item.fileLink && !chatStore.isFileLink(item?.fileLink)" :href="item.fileLink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
							...
							<div class="btns adaption">
								...
								<div v-if="appId != '20'" style="position:relative;">
									<!-- 踩和赞 暂时隐藏 -->
									<!-- <w-popconfirm v-if="!isup && !isdown" update-at-scroll :popup-container="textRef"
										content-class="popoverClass" @popup-visible-change="popupchange" ok-text="提交"
										@ok="feedbacksubmit(1)" placement="top" trigger="click" mini>
										<w-button type="text" shape="circle">
											<CoolThumbUpLineWe size="14" />
										...
										<template #content>
											<div class="popcontent">
												...
												<w-textarea v-model="zanText" :placeholder="chatStore.zanForm.placeholder" :max-length="200"
													show-word-limit :auto-size="{ minRows: 3, maxRows: 3 }" />
											</div>
										</template>
									</w-popconfirm>
									...
									<w-popconfirm v-if="!isup && !isdown" update-at-scroll :popup-container="textRef"
										content-class="popoverClass" @popup-visible-change="popupchange" ok-text="提交"
										@ok="feedbacksubmit(0)" :content="!isdown ? '踩' : '已踩'" placement="top" trigger="click" mini>
										<w-button type="text" shape="circle">
										...
										<template #content>
											<div class="popcontent">
												...
												<w-textarea v-model="caiText" :placeholder="chatStore.caiForm.placeholder" :max-length="200"
													show-word-limit :auto-size="{ minRows: 3, maxRows: 3 }" />
											</div>
										</template>
									</w-popconfirm>
				...
				<ChatTextComponent v-if="!inversion&&!route.path.includes('/knowledgeDetails')" :inversion="inversion" :loading="loading" :citations="citations"
					:dialogueFileIds="dialogueFileIds" :dialogueFolderIds="dialogueFolderIds" :paragraph="paragraph || ''"
					:skillId="skillId" :text="inversion ? question : answer" :as-raw-text="false" :asRawText="asRawText"
						...
						<div v-if="!inversion && !loading"
							...
							<div class="btns adaption">
								<div class="rebuildBtn">
									<template v-if="!chatStore.isSensitive">
										<w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
											<img src="/src/assets/chatTheme/refresh.svg" style="width:15px;height:15px;margin-right:3px;">
											<span style="color:#169E9A;">重新回答</span>
										</w-button>
									</template>
								</div>
								<div v-if="appId != '20'" style="position:relative;">
									...
									<w-tooltip background-color="#fff" mini>
										<w-button type="text" style="margin-right: 16px;" class="cxsc" @click="opendialog">
											<span style="color:#A6A5B8;font-size:15px;position:relative;right:40px;">
											<img src="/src/assets/chatTheme/mzsm.svg" style="width:16px;height:16px;position:absolute;top:4px;left:-18px;">
											免责声明</span>
										</w-button>
									</w-tooltip>
									<w-tooltip :content="!iscopy ? '复制' : '已复制'" placement="top" mini>
