...
const handleFillForm = (info) => {
	router.push({
		name: 'information',
		params: { appId: route.params.appId, matterId: info.matterId },
	});
};
const geTailTalk = () => {
...
<template>
	<div
		...
		:class="wrapClass"
		:style="{ padding: !inversion ? '8px 4px!important' : '8px 20px!important' }"
	>
		<div ref="textRef" class="leading-relaxed break-words" :class="maxHight">
			...
			<div v-if="!inversion">
				...
				<div class="markdown-body tailTalk" v-html="geTailTalk()" v-if="finishReason === 'ANSWER_COMPLETE'" />
...
.text-message-requests {
	background-color: #2155c9 !important;
