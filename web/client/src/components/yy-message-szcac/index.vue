		...
		<div class="fontSize16 max-w-full items-start">
			...
			<div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
              ...
              <div class="relate-source" v-if="syList.length>0">
								<span class="title">相关来源 {{syList.length}}<iconpark-icon @click="showRelateSource = !showRelateSource" class="arrow" :class="{'down' : showRelateSource}" size="24" name="arrow-down-s-line"></iconpark-icon></span>
								<ul v-show="showRelateSource">
									<li  v-for="(item, index) in syList" :key="index">
                    <div v-if="!isMobile">
                      <w-link v-if="item.fileLink && item.sourceType === 4" :href="item.fileLpreviewDocFlagink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                    </div>
                    <div v-else>
											<w-link v-if="item.fileLink" :href="item.fileLink" target="_blank" icon  >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                      <!-- <w-link v-if="item.fileLink"  @click.stop="openPopup(item)" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link> -->
                    </div>
									</li>
								</ul>
							</div>
								...
								<div class="rebuildBtn">
									<template v-if="!chatStore.isSensitive">
										<w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
											<img src="/src/assets/chatTheme/refresh1.svg" style="width:15px;height:15px;margin-right:3px;">
											<span style="color:#2155C9;">重新回答</span>
										</w-button>
									</template>
								</div>
								<div class="rebuildBtn">
...
:deep(.mzsmDialog){
	...
	.dialogTitle{
		...
		text-align: center;
		font-style: normal;
		background-image: linear-gradient(180deg, rgba(33, 85, 201, 0.2) 0%, rgba(22,158,154,0.00) 99%);
	}
	.el-button{
		width: calc(100% - 30px);
		padding: 24px 0;
		border:1px solid rgba(33, 85, 201, 1)!important;
		...
		border-radius: 20px;
		>span{
			color: rgba(33, 85, 201, 1)!important;
		}
	}
}
.fontSize16 {
