<template>
  <div ref="messageRef" class="flex w-full items-start" 
    :class="[{ 'mb-6': inversion, 'mt-6': inversion, 'mb-3': !inversion }, className]"  :style="align=='left'?'':'justify-content: flex-end;'">
    <div :class="['flex-shrink-0', inversion ? 'mr-3' : 'mr-3']" v-if="getAppInfo().robotIcon&&align=='left'">
      <AvatarComponent  v-if="!isMobile" :isLoading="loading" :image="inversion" />
    </div>
    <div class="fontSize16 items-start" :class="align=='right'?'text-bg':''" style="max-width: 100% ;">
      ...
      <div class="flex items-end gap-1 relative" 
			 ...
			 <!-- v-if="inversion" -->
        <w-tooltip v-if="inversion" placement="top" :arrow-style="{ right: 0, left: 'inherit' }" :update-at-scroll="true"
          ...
          <div> 
            ...
            :imgUrl="imgUrl" :params="param" :answerFlag="answerFlag" :question="question" v-model:isHighlightCode="isHighlightCode" :businessScenarioLists="businessScenarioLists" :businessScenario="businessScenario" :index="index"></TextComponent>
					</div>
        </w-tooltip>
		
        <TextComponent v-else :inversion="inversion" :answerFlag="answerFlag" :question="question" :syFlag="syFlag" :loading="loading" :citations="citations"
          :dialogueFileIds="dialogueFileIds" :dialogueFolderIds="dialogueFolderIds" :paragraph="paragraph || ''"
          :skillId="skillId" :text="inversion ? question : answer" :as-raw-text="false" :asRawText="asRawText"
          :params="param" :imgUrl="imgUrl" v-model:isHighlightCode="isHighlightCode" :matterGuide="matterGuide"
          :finishReason="finishReason" :businessScenarioLists="businessScenarioLists" :businessScenario="businessScenario" @handleFillForm="handleFillForm"
					:index="index">
          <template #footer>
			...
			<!-- 文件溯源 -->
					...
					<span class="title">相关来源 {{syList.length}}</span>  
						<ul>
							<li v-for="(item, index) in syList" :key="index">
								<w-link v-if="item.sourceType==1 && item?.transPdfUrl" @click.stop="openPreview(item)" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
								<w-link v-else :href="item.fileLink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
							</li>
						</ul>
				</div> -->
              <div class="show-preview" v-if="showPreview">
					<div  v-for="(item, index) in tjList" :key="index">
						...
						<div class="btns adaption" v-if="!inversion&&!loading&&!chatStore.isShare">
                <div class="left">
                  <div class="voice">
										<w-tooltip :content="isPlayer ? '停止' : '播放'" placement="top" mini>
                    <w-button type="text" shape="circle" v-if="!inversion">
											<!-- <div v-if="isPlayer && clientId == clientIdNew" style="width: 14px;height: 100%" @click="szrBofang">
												<div id="preloader">
													<div id="loader">
											...
											<!-- <CoolVolumeUpLineWe size="18"  @click="handleBofang" /> -->
											<iconpark-icon name="volume-up-line" size="18" @click="handleBofang" color="#494C4F"></iconpark-icon>
											<!-- v-else  -->
                    </w-button>
                  </w-tooltip>
									</div>
                  <!-- <div class="rebuildBtn">
                    ...
                    <w-button type="text" shape="circle" @click="handleSelect('copyText')">
                      <!-- <CoolFuzhi_9oe2ecmc v-if="!iscopy" size="18" /> -->
											<!-- <iconpark-icon name="file-copy-line" v-if="!copy" size="18" ></iconpark-icon> -->
                      
											<iconpark-icon name="file-copy-line" size="18" color="#494C4F" v-if="!iscopy"></iconpark-icon>
                      <CoolCheckLineWe v-else size="18" />
                    </w-button>
                  </w-tooltip>
									</div>
									<div class="rebuildBtn">
                    <template v-if="!chatStore.isSensitive">
						<el-tooltip content="重新生成" placement="top" class="custom-tooltip"  :popper-options="{ modifiers: [{ name: 'offset', options: { offset: [0, 5] } }] }">
                      <w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
                        <template #icon>
                          <CoolRefreshLineWe size="18" color="#383D47"/>
                        </template>
                        <!-- <span style="color:#383D47;">重新回答</span> -->
                      </w-button>
					</el-tooltip>
                    </template>
                  </div>
				  <el-tooltip content="转发" placement="top" class="custom-tooltip"  :popper-options="{ modifiers: [{ name: 'offset', options: { offset: [0, 5] } }] }">
									<div class="zhuanfa" @click="handleShare">
										<iconpark-icon name="share-forward-line" color="#494C4F" size="18"></iconpark-icon>
									</div>
								</el-tooltip>
									<!-- 查看 -->
									<el-tooltip content="图表" placement="top" class="custom-tooltip" v-if="answer.indexOf(`!DOCTYPE html`) > -1"  :popper-options="{ modifiers: [{ name: 'offset', options: { offset: [0, 5] } }] }">
									<div class="chakan" @click="handleView">
                ...
                <div v-if="appId != '20'" style="position:relative;">
                  <!-- 踩和赞 标准应用 -->
                  <div class="right">
                      <el-popover :visible="isupVisible" placement="top" :width="300" >
                        <div class="popcontent">
                          ...
                          <w-textarea v-model="zanText" placeholder="您可以在此提交宝贵意见或改进建议，这些都会成为我们不断进步的重要依据" :max-length="1000"
                            show-word-limit :auto-size="{ minRows: 6, maxRows: 10 }" />
                        </div>
                        <div style="text-align: right; margin-top:15px;">
                          <el-button size="small" text @click="popupchange" style="border:0!important;color:black!important" class="cancel">取消</el-button>
                          <el-button size="small" color="red" @click="onSubmit(1)" style="border:0!important ;background:#4888EF!important;color:#ffffff!important;" >
                            确定
                          </el-button>
                        </div>
                        <template #reference>
                          <div style="width: 32px;height: 32px;margin-right:8px;display: flex;justify-content: center;align-items: center;" class="dianzan">
														<w-button type="text" @click="isupVisible = true;isdownVisible=false" shape="circle">
                            <!-- <CoolThumbUpLineWe v-if="!isup" size="18" />
                            <CoolThumbUpFillWe v-else size="18" /> -->
														<iconpark-icon name="thumb-up-line" size="18" color="#494C4F"></iconpark-icon>
                          </w-button>
													</div>
                        </template>
                      </el-popover>
                      <!-- <w-popconfirm 
								v-if="!isdown" update-at-scroll :popup-container="textRef" :append-to-body="true"
									...
									<el-popover :visible="isdownVisible" placement="top" :width="300" >
                        <div class="popcontent">
                          <w-checkbox-group v-model="caiValue" direction="vertical">
                            <w-checkbox v-for="(item, index) in chatStore.caiForm.options" :key="index"
                              :value="item.id">{{ item.content }}</w-checkbox>
                          </w-checkbox-group>
                          <w-textarea v-model="caiText" placeholder="您可以在此提交宝贵意见或改进建议，这些都会成为我们不断进步的重要依据" :max-length="1000"
                            show-word-limit :auto-size="{ minRows: 6, maxRows: 10 }" />
                        </div>
                        <div style="text-align: right; margin-top:15px;">
                          <el-button @click="popupchange" size="small" text  style="border:0!important;color:black!important" class="cancel">取消</el-button>
                          <el-button @click="onSubmit(0)" size="small" color="red"  style="border:0!important ;background:#4888EF!important;color:#ffffff!important;" >
                            确定
                          </el-button>
                        </div>
                        <template #reference>
                          <div style="width: 32px;height: 32px;display: flex;justify-content: center;align-items: center;" class="diancai">
														<w-button type="text" @click="isdownVisible = true;isupVisible=false" shape="circle"  >
                            <!-- <CoolThumbDownLineWe v-if="!isdown" size="18" />
										<CoolThumbDownFillWe v-else size="18" /> -->
										<iconpark-icon name="thumb-down-line" size="18" color="#494C4F"></iconpark-icon>
                          </w-button>
													</div>
                        </template>
                      </el-popover>
			...
			<div v-if="!inversion && !loading && props.isAiQuestion" style="background: #fff;display: flex;"><span style="white-space: nowrap;">试题链接：</span><el-link type="primary" :href="currentPath + '#/testPreview/'+ route.params.appId +'/'+ dialogueId" target="_blank" :underline="false" style="word-break: break-all;">{{ currentPath }}#/testPreview/{{route.params.appId}}/{{ dialogueId }}</el-link></div>
          </template>
        </TextComponent>
	  ...
	  <div class="question-list" v-if="tjList.length > 0">
		<span class="title">您可能还想了解：</span>
		<span class="itemList" v-for="(item, index) in tjList" @click="sendQuestion(item.question)" :key="index">
        	<span class="name">{{ item.question }}</span>
		    <!-- <img style="height: 14px;" src="/src/assets/zc/zgc-fs.svg" /> -->
			<iconpark-icon name="send-plane-fill" size="16" color="#2065D6"></iconpark-icon>
		</span>
	  </div>
...
3.您在使用本智能问答服务中遇到任何问题或建议意见，欢迎您通过答复右下角按钮反馈，我们将不断改进、完善。 -->
	...
	<div :class="['flex-shrink-0', inversion ? 'mr-3' : 'mr-3']" v-if="getAppInfo().robotIcon&&align=='right'"
    ...
    <el-dialog width="435px" class="mzsmDialog" v-model="dialogVisible">
      ...
      <p>2、具体办理信息可进一步向深圳市市场监督管理局咨询。地址：福田区深南大道7010号工商物价大厦，电话：0755-12315。</p> -->
...
import { computed, ref, watch, onBeforeMount, onMounted, onUnmounted, defineAsyncComponent,inject} from 'vue';
...
const answerFlag = ref(props.answerFlag);
const question = ref(props.question);
const showSuggestOrg = ref(false);
const showPreview = ref(false);
const className = computed(() => {
	if (props.inversion) {
		return isMobile.value ? 'isPaddingRightMobile' : '';
	} else {
		return isMobile.value ? 'isPaddingLeftMobile' : '';
		// isPaddingLeft
	}
});
...
const currentPath = ref(window.location.origin + window.location.pathname)
...
//查看html解析弹窗
const handleView =()=>{ 
	mittBus.emit('open-html-modal', true);
}
// 反馈
const feedbackFlag = computed(() => {
	let appInfo = JSON.parse(window.localStorage.getItem(`${route.params.appId}`));
	return appInfo?.feedbackFlag == '是' ? true : false;
});

const clientIdNew = ref('')
...
const handleSelect = (key: 'copyText' | 'delete' | 'toggleRender', source: string = '') => {
	...
	if (key === 'toggleRender') {
		...
		if(chatStore.attachmentList?.length) {
			data.attachmentList = chatStore.attachmentList;
		}
		chatStore.setChatList(data);
		return;
	}
...
onMounted(() => {
	...
	if(virtualHumanType.value == '小冰数字人') {
		mittBus.on('startTalk', (val: string) => {
			isPlayer.value = true;
			clientIdNew.value = val;
		});
		mittBus.on('onTalkEnd', (val: string) => { 
			isPlayer.value = false;
		});
	} else {
		mittBus.on('samrPlaying', (val: string) => { 
			isPlayer.value = true;
			clientIdNew.value = val;
		});
		mittBus.on('samrEnding', (val: string) => { 
			isPlayer.value = false;
		});
	}
});
...
const sourceAnswerList = async () => {
	...
	if (res?.code == '000000') {
		syFlag.value = res?.data?.networkCheckFlag !== 1
		let list = res?.data?.sourceAnswerResultList ? res.data.sourceAnswerResultList : res.data
    if(res?.data?.networkCheckFlag === 1 && list.length > 0){
      list = list.filter((obj:object) => obj?.sourceType === 4)
    }
		if (list.length > 0) {
      //重庆高新区
			if (list[0].sourceType == 4) {
        syFlag.value = true
      }
			showPreview.value = list?.find((obj) => obj?.sourceType == 3);
			if (list[0].sourceType == 1) {
				syList.value = list.filter((obj, index, self) => obj.fileName);
			} else {
...
const szrBofang = () => {
	if(isPlayer.value) {
		if(virtualHumanType.value == '小冰数字人') {
			mittBus.emit('stopTalk');
		} else {
			mittBus.emit('stopTalkSAMR');
		}
		isPlayer.value = false;
	}
}
...
const popupchange = (val) => {
	...
	// if (!val) {
		...
		zanText.value = '';
	// }
	console.log(zanText.value,'赞的信息')
};
...
.diancai{
	&:hover{
		cursor: pointer;
		  background: rgba(105, 105, 106, 0.1)!important;
	}
}
.dianzan{
	&:hover{
...
/* 穿透 scoped 作用域，修改 el-tooltip 的弹出框样式 */
.custom-tooltip :deep(.el-tooltip__trigger) {
  padding: 0 !important; /* 移除弹出框的内边距 */
  margin: 0 !important; /* 移除弹出框的外边距 */
  box-sizing: border-box !important; /* 确保尺寸计算包含内边距和边框 */
}
 
/* 进一步调整文字样式 */
.custom-tooltip :deep(.el-tooltip__popper .el-tooltip__text) {
  margin: 0 !important; /* 移除文字的外边距 */
  padding: 0 !important; /* 移除文字的内边距 */
  line-height: 1px !important; /* 调整行高，避免额外间距 */
  display: inline-block !important; /* 确保文字是行内块元素 */
}
</style>