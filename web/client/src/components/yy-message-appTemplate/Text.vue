...
import { getById,getVagueQuestionList  } from '/@/api/knowledge';
...
const answerFlag = computed(() => chatStore.chatList[curIndex.value].answerFlag);
// 侧拉流程分组
...
const previewPic = (item: Object) => {
  if(previewDocFlag() && chatStore.isFileLink(item?.url)){
    openPreview(item);
  } else {
    if(item.url){
...
let timer = ref();
let curNum = ref(1);
const loading  = computed(() => chatStore.chatList[curIndex.value].loading);
watch(
		() => loading.value,
		(val) => {
			if(!val){
				getVagueQuestionListFun()
			}									
		}
	);
const getVagueQuestionListFun = async () => {
    	if(text.value!=''){
    		let res = await getVagueQuestionList({
    			question: question.value,
				dialogueId:sessionStorage.getItem('dialogueId'),
    			applicationId: localStorage.getItem(`${route.params.appId}appId`),
    		});
    		cjxwList.value = res.data;
    ...
    const handleMouseOver = (event) => {
      ...
      if (target?.classList?.contains('ragTag')) {
        ...
        tooltipTitle.value = data.title;
		 // 对 data.content 进行高亮处理
		if (data.content) {
		data.content = data.content.replace(/(?:\n)?<em>(.*?)<\/em>/g, (_, content) => {
			return `<span style="color: #7E56EB;">${content}</span>`;
		});
		}
        ...
        // 设置相对于元素的偏移
        tooltipStyle.value = {
          top: `${elementTop + target.offsetHeight}px`,  // 在元素下方5px
...
onMounted(() => {
	...
	// })
	if(chatStore.$state.tempFileList && chatStore.$state.tempFileList.length && chatStore.$state.tempFileList[props.index] && chatStore.$state.tempFileList[props.index].length>0){
		...
		<div ref="textRef" class="leading-relaxed break-words" :class="maxHight">
			...
			<div v-if="!inversion">
				...
				<div v-else class="markdown-body icon-box">
						...
						<div v-if="answerFlag=='true'" class="flex icon5" style="align-items: center;"><img style="height: 28px;margin-right: 12px;" src="./yhdwb.svg" alt="" />已回答完毕</div>
					    <iconpark-icon v-if="answerFlag=='true'" @click="showRelateSource5 = !showRelateSource5" class="arrow" size="18" :name="showRelateSource5?'arrow-up-s-line':'arrow-down-s-line'"></iconpark-icon>
					<!-- <span> AI飞速思考中，请稍等...</span> -->
				   <div v-if="answerFlag=='false'">
				   	<div class="cjxw">
				   		<div class="cjxw-title">抱歉，我无法回答该问题，我猜你想问</div>
				   		<div class="cjxw-content">
				   			<div style="margin-top: 20px;cursor: pointer;" v-for="(item, index) in cjxwList" :key="index"
				   				 @click="sendQuestion(item.question)">
				   				<span class="cjxw-content-item">{{ item.question }}<img class="video-file" src="/src/assets/jt.svg"</span>
				   			</div>
				   		</div>
				   	</div>
				   </div>
				...
				<div class="suyuan" v-show="showRelateSource5&&answerFlag=='true'">
					<div v-for="(item,index) in progressList" :key="index">
						...
						<div v-else-if="item.progress=='检索知识库'">
							<div class="flex suyaun-lwjs suyuan-mt10" style="align-items: center;flex-wrap: wrap;"  v-if="item.resultList.filter(el => el.url).length>0">
								...
								<span class="suyuan-c9" @click="showRelateSource4 = !showRelateSource4" style="cursor: pointer;">相关材料</span>
								<iconpark-icon @click="showRelateSource4 = !showRelateSource4" style="cursor: pointer;" class="arrow"  size="18" :name="showRelateSource4?'arrow-up-s-line':'arrow-down-s-line'"></iconpark-icon>
							</div>
						...
						<div v-else-if="item.progress!='风险检测'">
							...
							<span class="suyuan-c6 suyuan-max-w200" :title="itm1" v-for="(itm1,idx) in progressList[0].resultList" :key="idx">{{itm1}}</span>
							<span class="suyuan-c9" @click="showRelateSource2 = !showRelateSource2" style="cursor: pointer;">相关材料</span>
							<iconpark-icon @click="showRelateSource2 = !showRelateSource2" style="cursor: pointer;" class="arrow"  size="18" :name="showRelateSource2?'arrow-up-s-line':'arrow-down-s-line'"></iconpark-icon>
							</div>
					...
					<!-- <div v-show="!loading" class="flex icon5 suyuan-mt10" style="align-items: center; font-size: 14px;"><img style="height: 14px;margin-right: 12px;" src="./yhdwb.svg" alt="" />已回答完毕</div> -->
				</div>
				<div
					v-if="!asRawText && text"
					class="markdown-body"
				...
				<div class="markdown-body markdown-epilogue" style="display: block" v-html="geTailTalk()" v-if="finishReason === 'ANSWER_COMPLETE'" />
				<div v-if="asRawText" style="color: #fff;" class="whitespace-pre-wrap" v-text="text" />
			</div>
			...
			<div style="display:flex;height:58px;align-items:center;margin-bottom:12px;" v-if="inversion&& !isContainFile && fileList.length>0 && isFileList" >
				<div v-for="(item,index) in fileList" :key="index"
				style="display:flex;align-items:center;margin-right:8px;
			 width: 240px;height: 58px;background: #FFFFFF;border-radius: 8px;border: 1px solid #D0DFFD;font-size:0;gap:0">
						<div style="width:56px;height:56px;display:flex;justify-content:center;align-items:center">
							...
							<svg t="1744274231278" class="icon" viewBox="0 0 1024 1024" version="1.1"
								...
								v-if="item.fileType == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || item.fileType == 'application/vnd.ms-excel' || item.fileType == 'xlsx' || item.fileType == 'xls'">
								<path
							...
							<svg t="1744273886599" class="icon" viewBox="0 0 1024 1024" version="1.1"
								...
								v-if="item.fileType == 'application/msword' || item.fileType == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || item.fileType == 'doc' || item.fileType == 'docx'">
								<path
...
align-items:center;">{{changeFileSize(item.fileSize)}}</div>
  ...
  <div v-if="showTooltip && !inversion" ref="refTooltip" class="tooltip" @mouseleave="handleTooltipMouseLeave" :style="tooltipStyle">
