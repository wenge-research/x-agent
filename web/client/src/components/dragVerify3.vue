<template>
  ...
  >
    ...
    >
      ...
      <component :is="getIconName" color="#9A99AA" />
...
export default {
  ...
  mounted: function() {
    this.IShandlerIcon = this.handlerIcon
  ...
  computed: {
    ...
    handlerStyle: function() {
      return {
        ...
        borderRadius: parseFloat(this.radius.replace('px')) - 2 + 'px'
      };
    },
    message: function() {
  ...
  methods: {
    ...
    dragMoving: function(e) {
      if (this.isMoving && !this.isPassing) {
        ...
        if (_x >= 4 && _x <= this.width - this.height - 4) {
          handler.style.left = _x + "px";
          this.$refs.progressBar.style.width = _x + parseFloat(this.height) + 4 + "px";
        } else if (_x > this.width - this.height - 4) {
          handler.style.left = this.width - this.height - 4 + "px";
    ...
    dragFinish: function(e) {
      if (this.isMoving && !this.isPassing) {
        ...
        if (_x < this.width - this.height) {
          ...
          var that = this;
          setTimeout(function() {
            that.$refs.handler.style.left = "4px";
    ...
    passVerify: function() {
      ...
      this.getIconName = this.successIcon
      this.$refs.progressBar.style.background = this.completedBg;
      this.$refs.message.style["-webkit-text-fill-color"] = "unset";
      this.$refs.message.style.animation = "slidetounlock2 3s infinite";
      this.$refs.message.style.color = "#2B6DFF";
      this.$emit("passcallback", this.reset);
    },
    reset: function() {
      ...
      this.getIconName = this.handlerIcon
