<template>
  <div ref="messageRef" class="w-full"
    ...
    <div class="fontSize16 max-w-full items-start" style="width: 100%;" :class="[!inversion ? 'disFlex' : 'disFlexInversion']">
      ...
      <div class="flex flex-wrap items-center justify-start">
        <div class="tagList" v-for="(item, key) in paramData" :key="key">
          ...
          <span class="tip">
			...
			<w-avatar v-if="!inversion" style="margin-right: 8px" :size="32"><img :src="getAppDetail()?.robotIcon || BotAvatar" /></w-avatar>
      <div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef" style="flex: 1;">
        <w-tooltip v-if="inversion" placement="tr" :arrow-style="{ right: 0, left: 'inherit' }" :update-at-scroll="true"
          background-color="#fff" :content-style="{ padding: '8px 16px', borderRadius: '4px', zIndex: '9999' }">
          <template #content v-if="inversion">
            ...
            <CoolFuzhi_9oe2ecmc title="复制" @click="copy(question, imgUrl)" size="14" color="#9A99AA"
              style="cursor:pointer;" />
					...
					<w-avatar style="margin-right: 8px" :size="32"><img :src="getAppDetail()?.userIcon || UserIcon" /></w-avatar>
          <TextComponent :inversion="inversion" :loading="loading" :citations="citations"
            ...
            :imgUrl="imgUrl" :params="param" v-model:isHighlightCode="isHighlightCode" :syList="syList" :syAnswerFlag="sourceAnswerFlag"  :syText="sourceAnswerPrompt"></TextComponent>
        </w-tooltip>
				
        <TextComponent v-else :inversion="inversion" :loading="loading" :citations="citations"
          :dialogueFileIds="dialogueFileIds" :dialogueFolderIds="dialogueFolderIds" :paragraph="paragraph || ''"
          :skillId="skillId" :text="inversion ? question : answer" :as-raw-text="false" :asRawText="asRawText"
          :params="param" :imgUrl="imgUrl" v-model:isHighlightCode="isHighlightCode" :syList="syList" :matterGuide="matterGuide" :syFlag="syFlag" :finishReason="finishReason" :syAnswerFlag="sourceAnswerFlag"  :syText="sourceAnswerPrompt">
          <template #footer>
                    ...
                    <div class="item" v-if="index < 2" :key="index" @click="sendQuestion(item)">
                      <div class="box" :style="{ 'justify-content': isMobile ? 'flex-start' : 'space-between' }">
                        ...
                        <img class="pc-img" v-if="!isMobile" style="height: 20px" src="/src/assets/zc/send1.png" alt="" />
							...
							<div class="voice-btn" style="height: 24px;">
								<iconpark-icon v-if="!isPlayer" name="play-circle-fill" size="21" color="#00A9E4" style="margin-left: 14px" @click="handleBofang"></iconpark-icon>
								<img class="pc-img" v-else style="height: 21px;margin-left: 14px" src="/src/assets/chongqing/voice.svg" alt="" @click="handleBofang" />
							</div>
			  <!-- 回答问题相关 -->
			  <!-- 重庆显示工作数据 --> 
		  <!-- <div class="work-data-list" v-if="matterGuide.length && !benkeGuide"> --> 
		  <div class="work-data-list" v-if="false">
          <div v-for="(item, index) in matterGuide" :key="index" class="list">
            <div class="name">
              {{ item.name }}
            </div>
            <div class="content">
            ...
            <div class="flex-just">
              <div class="job-details" @click="jobClick(item.detailUrl)">岗位详情</div>
              <div>
                <w-button
                  style="
		  ...
		  <div class="personal-data" v-if="false">
		  <!-- <div class="personal-data" v-if="matterGuide?.length && benkeGuide"> -->
          <div v-for="(item, index) in matterGuide" :key="index" class="flex">
            <div class="name">
          ...
          <div class="flex-just" style="margin-top: 10px;">
            ...
            <div>
              ...
              <div class="btns adaption">
				<!-- 先注释 -->
                <!-- <div class="left">
					<w-tooltip content="语音播报" placement="top" mini>
                    <w-button type="text" shape="circle" v-if="!inversion" @click="handleBofang">
						<img src="/src/assets/chongqing/play-circle-fill.svg" style="width:20px;height:20px;">
                    </w-button>
                  </w-tooltip> -->
                  ...
                  <!-- <div class="rebuildBtn">
                    <template v-if="!chatStore.isSensitive">
                      <w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
                        <template #icon>
                          <CoolRefreshLineWe size="14" color="#1A6DD2"/>
                        </template>
                        <span style="color:#1A6DD2;">重新生成</span>
                      </w-button>
                    </template>
                  </div>
                  <div class="rebuildBtn">
...
3.您在使用本智能问答服务中遇到任何问题或建议意见，欢迎您通过答复右下角按钮反馈，我们将不断改进、完善。 -->
    ...
    <div v-if="!inversion && tjList?.length" class="recommend">
			<img src="/src/assets/chongqing/left-top1.svg" alt="" class="recommend-top" />
			<div class="recommend-title">你可能还想了解</div>
			<ul class="recommend-list">
				<li class="recommend-list-item" v-for="(tj, index) in tjList" :key="index" @click="sendQuestion(tj.question)"><span class="drop"></span>{{ tj.question }}</li>
			</ul>
			<img src="/src/assets/chongqing/left-top1.svg" alt="" class="recommend-bottom" />
		</div>
...
import { computed, ref, watch, onBeforeMount, onMounted, onUnmounted } from 'vue';
...
import UserIcon from '/@/assets/zc/jqr.png';
...
const className = computed(() => {
	...
	// }
	if (props.inversion) {
		return isMobile.value ? '' : 'isPaddingRight';
	} else {
		return isMobile.value ? '' : 'isPaddingLeft';
	}
});
...
const syList = ref([]);
const sourceAnswerFlag = ref('');
const sourceAnswerPrompt = ref('');
const sourceAnswerList = async () => {
	...
	if (res?.code == '000000') {
		...
		sourceAnswerFlag.value = res.data?.sourceAnswerFlag;
		if(sourceAnswerFlag.value) {
			sourceAnswerPrompt.value = res.data?.sourceAnswerPrompt;
		}
...
onMounted(() => {
	if (!props.inversion) {
		mittBus.on('dialogueId', (id) => {
			...
			// setTimeout(() => {
			//   let guide;
			//   if (sessionStorage.getItem('matterGuide') == 'undefined') {
			//   } else {
			// 	guide = JSON.parse(sessionStorage.getItem('matterGuide'))
