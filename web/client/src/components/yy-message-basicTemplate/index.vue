<template>
  <div ref="messageRef" class="flex w-full items-start"
    :class="[{'mb-6': inversion, 'mt-6': inversion, 'mb-3': !inversion }, className]">
    <div :class="['flex-shrink-0', inversion ? 'mr-3' : 'mr-3']">
      <AvatarComponent v-if="!isMobile" :isLoading="loading" :image="inversion" />
    </div>
    <div class="fontSize16 items-start" style="max-width: 91%;border-radius: 2px 4px 0 0;" :class="[inversion ? 'question-row' : 'answer-row']">
      ...
      <div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
        <w-tooltip v-if="inversion" placement="tr" :arrow-style="{ right: 0, left: 'inherit' }" :update-at-scroll="true"
          ...
          <TextComponent :inversion="inversion" :loading="loading" :citations="citations"
            ...
            :imgUrl="imgUrl" :params="param" v-model:isHighlightCode="isHighlightCode" :businessScenarioLists="businessScenarioLists" :businessScenario="businessScenario"></TextComponent>
        </w-tooltip>
          ...
          <template #footer>
            ...
            <div style="padding: 0 0 1rem 1rem;margin-top: -20px;display: flex;align-items: center"
              ...
              <w-button @click="sendQuestion(props.suggestOrg)" size="small"
                style="background-color: #409EFF;color: #F7F8FA;border-radius: 5px">
                {{ props.suggestOrg }}
              </w-button>
              <span style="color: #000000;font-size:16px;">查询或咨询</span>
            </div>
            <div v-if="!inversion && !loading" style="background: #fff;padding-top: 0;padding-left: 0;border-radius: 0;">
              ...
              <div class="btns adaption">
                ...
                <div v-if="appId != '20'" style="position:relative;display:flex">
                  <!-- 踩和赞 标准应用 -->
				  <!-- <template v-if="feedbackFlag"> -->
				  <template v-if="false">
					   <w-popconfirm v-if="!isup" update-at-scroll :popup-container="textRef" :append-to-body="true"
                      ...
                      <CoolThumbUpFillWe v-else size="14" />
								...
								<w-popconfirm v-if="!isdown" update-at-scroll :popup-container="textRef" :append-to-body="true"
										content-class="popoverClass" @popup-visible-change="popupchange" ok-text="提交"
                  ...
                  <w-tooltip :content="!iscopy ? '复制' : '已复制'" placement="top" mini>
                    <w-button type="text" shape="circle" @click="handleSelect('copyText')">
                      ...
                      <CoolCheckLineWe v-else size="14" />
									...
									<w-tooltip :content="'重新回答'" placement="top" mini>
                    <div class="rebuildBtn" @mouseenter="isHover = true" @mouseleave="isHover = false">
                      ...
                      <w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
                        <template #icon>
                          <!-- <CoolRefreshLineWe size="14" color="#383D47"/> -->
													<iconpark-icon v-if="!isHover" name="reset-left-line" color="#1d2129"></iconpark-icon>
													<iconpark-icon v-else name="reset-left-line" color="#2065D6"></iconpark-icon>
                        </template>
                  ...
                  <!-- <w-tooltip :content="isPlayer ? '停止' : '播放'" placement="top" mini>
                    <w-button type="text" shape="circle" v-if="!inversion">
											...
											<CoolVolumeUpLineWe v-else size="14"  @click="handleBofang" />
                    </w-button>
                  </w-tooltip> -->
                </div>
              </div>
	  ...
	  <div class="question-list" v-if="tjList.length > 0">
		...
		<div v-for="(item, index) in tjList" @click="sendQuestion(item.question)" :key="index" class="question-list-item">
			<span class="itemList">
...
3.您在使用本智能问答服务中遇到任何问题或建议意见，欢迎您通过答复右下角按钮反馈，我们将不断改进、完善。 -->
    ...
    <comDocPreview ref="previewRef" width="100%" />
  </div>
</template>
