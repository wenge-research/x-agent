      ...
      <div class="flex items-end gap-1 relative" 
        ...
        <w-tooltip v-if="inversion" placement="top" :arrow-style="{ right: 0, left: 'inherit' }" :update-at-scroll="true"
          ...
          <div>
            ...
            :imgUrl="imgUrl" :params="param" v-model:isHighlightCode="isHighlightCode" :businessScenarioLists="businessScenarioLists" :businessScenario="businessScenario" :index="index" :summaryText="dialogueSummary"></TextComponent>
					</div>
        </w-tooltip>
        <TextComponent v-else :inversion="inversion" :syFlag="syFlag" :loading="loading" :citations="citations"
          ...
          :finishReason="finishReason" :businessScenarioLists="businessScenarioLists" :businessScenario="businessScenario" @handleFillForm="handleFillForm"
					:index="index" :summaryText="dialogueSummary">
          <template #footer>
            ...
            <div v-if="!inversion && !loading" style="background: rgba(255,255,255,0.7);padding-top: 0;">
...
import { listDialogueQuestion, tjQuestion, sourceAnswer, apiConversationSummary } from '/@/api/chat';
...
onMounted(() => {
	if (!props.inversion) {
		mittBus.on('ttAnalysisSummarize', handleSummarize);
...
onUnmounted(() => {
	...
	// 移除所有监听器
  if (!props.inversion) {
    mittBus.off('ttAnalysisSummarize', handleSummarize);
  }
});
const tjList = ref([]);
...
const conversationSummary = async () => {
	chatStore.completeSummary = false;
	const parmas = {
		conversation: chatStore.answerParams
	}
	const res = await apiConversationSummary(parmas)
	if(res?.code == '000000') {
		dialogueSummary.value = res.data || ''
		chatStore.completeSummary = true;
	} else {
		chatStore.completeSummary = true;
	}
}
