<template>
  <div ref="messageRef" class="flex w-full items-start"
    :class="[{ 'mb-4': inversion, 'mt-4': inversion, 'mb-3': !inversion }, className]">
                  ...
                  <div class="rebuildBtn">
                    <template v-if="!chatStore.isSensitive">
                      <w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
                        <template #icon>
                          <CoolRefreshLineWe size="14" color="#828894"/>
                        </template>
...
const sourceAnswerList = async () => {
	...
	if (res?.code == '000000') {
		if (res?.data?.sourceAnswerResultList.length > 0) {
			showPreview.value = res.data.sourceAnswerResultList.find((obj) => obj.sourceType == 3);
...
watch(() => props.finishReason, (newVal:any) => {
