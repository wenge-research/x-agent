<template>
  <div ref="messageRef" class="yy-message-longhuaChat flex w-full items-start"
            ...
            <div v-if="!inversion && !loading" style="background: #fff;padding-top: 0;">
              ...
              <!-- <div>
							...
							<div class="relate-source" v-if="syList.length>0">
								<span class="title" @click="showRelateSource = !showRelateSource">相关来源 {{syList.length}}<iconpark-icon class="arrow" :class="{'down' : showRelateSource}" size="24" name="arrow-down-s-line"></iconpark-icon></span>
								<ul v-show="showRelateSource">
									<li  v-for="(item, index) in syList" :key="index">
                    <!-- <div v-if="!isMobile">
                      <w-link v-if="item.fileLink && item?.docLinkType == 2" :href="item.fileLink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                    	<w-link v-else-if="item.fileLink && item?.docLinkType == 1 && !previewDocFlag()" :href="item.fileLink" target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
										</div>
                    <div v-else>
                      <w-link v-if="item.fileLink && item.sourceType === 4"  @click.stop="openPopup(item)" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
										...
										<w-link @click.stop icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
									</li>
								</ul>
							</div>
                ...
                <div class="left">
                  <div class="rebuildBtn">
                    <template v-if="!chatStore.isSensitive">
                      <w-button type="text" class="cxscNo" @click="handleSelect('toggleRender')">
                        <template #icon>
                          <CoolRefreshLineWe size="18" color="#494C4F"/>
                        </template>
                  ...
                  <!-- <div class="rebuildBtn">
                    <w-button type="text" @click="shareHandler">
                      <iconpark-icon name="share-forward-line" size="20" color="#797F8A"></iconpark-icon>
                    </w-button>
                  </div> -->
                  <!-- <div class="rebuildBtn">
...
const sourceAnswerList = async () => { 
	...
	if (res?.code == '000000') {
		const list = res?.data?.sourceAnswerResultList || [];
		if(list.length) {
