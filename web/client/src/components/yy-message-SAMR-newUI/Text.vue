...
const wrapClass = computed(() => {
	return [
		...
		props.inversion ? 'text-message-request-newUi' : 'text-message-reply',
...
const handleImageClick = (event) => {
	if (event.target.tagName === 'IMG') {
		currentImageSrc.value = event.target.src;
...
const resetZoom = () => {
	...
	if (previewImage.value) {
		previewImage.value.style.left = '50%';
		previewImage.value.style.top = '50%';
	}
};
const startDrag = (e) => {
	...
	startX.value = e.pageX - previewImage.value.offsetLeft;
	startY.value = e.pageY - previewImage.value.offsetTop;
	scrollLeft.value = previewImage.value.scrollLeft;
	scrollTop.value = previewImage.value.scrollTop;
...
const onDrag = (e) => {
	...
	const y = e.pageY - startY.value;

	previewImage.value.style.left = `${x}px`;
	previewImage.value.style.top = `${y}px`;
};
...
<template>
	<div class="text-black" style="border: 1px solid #fff; position: relative" :class="wrapClass">
								...
								<div class="outer2">
									...
									<div class="outer2-right">
										<div class="outer2-right-name">
											{{ fileName }} <iconpark-icon name="checkbox-circle-fill" size="12" color="#1DB9A2"></iconpark-icon>
										</div>
		...
		>
			<template #header>
				<div class="my-header" style="text-align: right">
					<div class="my-header-close" @click="handleClose">
						<iconpark-icon name="close-large-line" size="24" color="rgba(255,255,255,0.8)"></iconpark-icon>
					</div>
				</div>
			</template>
        ...
        <el-button @click="resetZoom" icon="el-icon-refresh-left" circle></el-button>
      </div> -->
		</el-dialog>
	</div>
</template>
...
.text-message-request-newUi {
	background-color: #1477e3 !important;
	border: none !important;
