<template>
	<div ref="messageRef" class="w-full items-start"
		:class="[{ 'flex-row-reverse': inversion, 'mb-10': inversion, 'mt-10': inversion, '': !inversion }, className]">
		<div :class="['flex-shrink-0', inversion ? 'ml-2' : 'mr-3']">
			<AvatarComponent v-if="false" :isLoading="loading" :image="inversion" />
		</div>
		<div class="fontSize32 max-w-full items-start">
			...
			<div class="flex items-end gap-1 relative" :class="[inversion ? 'flex-row-reverse' : 'flex-row']" ref="textRef">
				...
				<ChatTextComponent v-else :inversion="inversion" :loading="loading" :citations="citations"
					...
					<template #footer>
						...
						<div v-if="!inversion && !loading"
							style="background: #fff;padding: 0 40px 32px!important;">
              ...
              <div class="relate-source-large" v-if="syList.length>0">
								<span class="title">相关来源 {{syList.length}}<iconpark-icon @click="showRelateSource = !showRelateSource" class="arrow" :class="{'down' : showRelateSource}" size="32" name="arrow-down-s-line"></iconpark-icon></span>
								<ul v-show="showRelateSource">
									<li  v-for="(item, index) in syList" :key="index">
                    <div v-if="isMobile">
                      <w-link :href="item.fileLink" v-if="item.fileLink && item.sourceType === 4"  target="_blank" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
                    </div>
                    <div v-else>
                      <w-link v-if="item.fileLink && item.sourceType === 4"  @click.stop="openPopup(item)" icon >{{item.fileName !=null?item.fileName :item.fileLink }}</w-link>
							...
							<div class="btns adaption">
								<div class="rebuildBtn-large">
									<template v-if="!chatStore.isSensitive">
										<w-button type="text" class="cxsc" @click="handleSelect('toggleRender')">
											<iconpark-icon name="refresh-line" size="32" color="#494C4F"></iconpark-icon>
											<!-- <span style="color:#169E9A;font-size:28px;">重新回答</span> -->
										</w-button>
									</template>
								</div>
								<div class="rebuildBtn-large">
										...
										<w-button type="text" shape="circle" v-if="!inversion" @click="handleBofang">
											<!-- <CoolVolumeUpLineWe size="28" /> -->
											<iconpark-icon name="volume-up-line" size="32" color="#494C4F"></iconpark-icon>
										</w-button>
									</w-tooltip>
									
								</div>
								<div v-if="appId != '20'" style="position:relative;flex: 1;display: flex;flex-wrap: nowrap;justify-content: flex-end;height: 80px;align-items: center">
									<!-- 踩和赞 暂时隐藏 关芯客服H5 -->
                <!-- <w-popconfirm v-if="!isup" update-at-scroll :popup-container="textRef"
										...
										<w-button type="text" shape="circle">
											<CoolThumbUpLineWe v-if="!isup" size="28" />
                      <CoolThumbUpFillWe v-else size="28" />
										</w-button>
										<template #content>
								...
								<!-- <w-popconfirm v-if="!isdown" update-at-scroll :popup-container="textRef"
									...
									<w-button type="text" v-else shape="circle">
										<CoolThumbDownLineWe v-if="!isdown" size="28" />
										<CoolThumbDownFillWe v-else size="28" />
									</w-button> -->
									<!-- <w-tooltip :content="!iscopy ? '复制' : '已复制'" placement="top" mini>
										<w-button type="text" shape="circle" @click="handleSelect('copyText')">
											<CoolFuzhi_9oe2ecmc v-if="!iscopy" size="28" />
											<CoolCheckLineWe v-else size="28" />
										</w-button>
									</w-tooltip> -->
									<w-tooltip content="免责声明" placement="top" mini>
										<w-button type="text" class="cxsc" @click="opendialog">
										<iconpark-icon name="information-line" size="32" color="#797991"></iconpark-icon>
										<span style="color:#797991;font-size:32px;margin-left: 10px">免责声明</span>
									</w-button>
									</w-tooltip>
								</div>
							</div>
			...
			<div class="question-list-large" v-if="!isMobile && tjList.length > 0">
	        	...
	        	<div class="itemList" v-for="(item, index) in tjList" @click="sendQuestion(item.question)" :key="index">
	        	    ...
	        	    <img style="height: 32px" src="/src/assets/zc/zgc-fs.svg" />
	        	</div>
	        </div>
		</div>
		...
		<el-dialog v-model="dialogVisible" width="90%" class="mzsmDialog">
			...
			<div v-html="getmzsm()" style="padding:0 24px;line-height:24px;font-size:32px;line-height: 48px;color:#181b49;white-space:pre-wrap;"></div>
...
const paramData = computed(() => {
	if (props.param) {
		...
		for (const key in params) {
			...
			try {
				let keyObj: any = chatStore.dialogueParamsListOld.filter(item => { return item.key === key })[0]
...
const appear = (content, index, data) => {
	...
	function changeContent() {
		questionList.value[index] =
			content.substring(0, count); //截取字符串
...
:deep(.mzsmDialog){
	...
	.dialogTitle{
		...
		text-align: center;
		font-style: normal;
		background-image: linear-gradient(180deg, rgba(22,158,154,0.20) 0%, rgba(22,158,154,0.00) 99%);
	}
	.el-button{
		width: calc(100% - 30px);
		padding: 42px 0;
...
.fontSize32 {
	@include add-size($font-size-base32, $size);
}
