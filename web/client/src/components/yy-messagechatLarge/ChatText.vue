    ...
    if (tokens[idx].nesting === 1) {
      ...
      if (arr[1]) {
        ...
        recommendKey.forEach((element, index) => {
          ...
          result += `
						<button type="button"  onclick='setsendMessage("${key}",${JSON.stringify(element.param)},${props.params
      ...
      if (!chatMode.value) {
        ...
        const params = {};
        return `
					<div  class="recommend-title target-${idx * random} recommend-title-pointer" onclick='setsendMessage("${key}",${JSON.stringify(params)},${props.params
          })'>
					${arr[0]}
					</div>
				`;
...
const wrapClass = computed(() => {
  return [
    ...
    props.inversion ? 'text-message-requests-large' : 'text-message-reply text-message-reply-large',
    props.inversion ? 'border-right-large' : 'border-left-large',
  ];
});
...
const handleFillForm = (info) => {
  router.push({ name: 'information', params: { appId: route.params.appId, matterId: info.matterId } });
}
const geTailTalk = () => {
...
<template>
	<div class="text-black" style="border: 1px solid #fff; position: relative" :class="wrapClass">
		<div ref="textRef" class="leading-relaxed break-words" :class="maxHight">
			<!-- 知识库 -->
			<div v-if="!inversion">
				<div v-if="!asRawText && text" class="markdown-body-large"
					:class="['markdown-body-as-img-' + isMarkdownImg.length, chatStore.isLightTheme ? 'hljs-atom-one-light' : 'hljs-atom-one-dark']"
					v-html="text" />
				<div class="markdown-body-large icon-box" v-if="!asRawText && !text">
				<!-- <div class="markdown-body-large icon-box"> -->
        ...
        <div class="matter-box" v-if="finishReason === 'ANSWER_COMPLETE' && matterGuide">
          <div class="message">{{ matterGuide?.enterTip }}</div>
          <div class="btn" @click="handleFillForm(matterGuide)">
            <img :src="matterGuide?.iconUrl" alt="">
            {{ matterGuide?.matterName }}
          </div>
        </div>
        <div class="markdown-body-large geTailTalk" v-html="geTailTalk()" v-if="finishReason === 'ANSWER_COMPLETE'" />
      ...
      <div v-if="inversion && !isContainFile" class="whitespace-pre-wrap"
        :class="inversion && (paragraph || imgUrl) ? 'textBorder' : ''" v-text="text" />
      <div v-if="isContainFile" class="whitespace-pre-wrap"
        :class="inversion && (paragraph || imgUrl) ? 'textBorder' : ''" ref="containFile" />
...
.text-message-reply-large {
  padding: 40px 40px 32px!important;
}

.text-message-requests-large {
  background-color: #169E9A !important;
