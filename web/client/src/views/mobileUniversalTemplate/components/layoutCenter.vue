<template>
	<div class="relative flex flex-col w-full h-full" :class="{ centerMobile: isMobile }">
		<div class="mobileUniversalTemplate-layoutCenter center-side-parent" id="bg-main" :style="`height: 100%;`">
					...
					<div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side" v-if="!loading">
						...
						>
							...
							<div v-if="curRouteId == policyUrl && isMobile">
								<div v-if="virtualHumanLogo || getAppDetail()?.identityIcon || getAppDetail()?.greeting" class="mobileUniversalTemplate-layoutCenter-header">
									<div class="top">
										<div v-if="virtualHumanLogo" style="width: 48px;height: 48px;display: flex;justify-content: center;
										align-items: center;overflow: hidden;border-radius: 50%;margin-left:12px;">
										<img :src="virtualHumanLogo" alt="" style="height: 100%;" />
									</div>
									<div class="title">
									...
									<div v-if="getAppDetail()?.identityIcon || getAppDetail()?.greeting" style="flex: 1; overflow: hidden; margin:0 12px">
										<div class="message">{{ getAppDetail()?.greeting }}</div>
									</div>
								</div>
								<div class="mobileUniversalTemplate-layoutCenter-prologue" style="display: flex" :style="{ padding: isMobile ? '' : ' 0 30px' }">
									<w-avatar v-if="getAppDetail()?.robotIcon" class="robotIcon" :size="36"><img :src="getAppDetail()?.robotIcon" /></w-avatar>
									<div class="txt fontSize16 min-w-[20px] text-box-1716763096571080706 dark:bg-[#fff] message-request">
										{{ getAppDetail()?.prologue || '您好，很高兴为您服务！请问有什么可以帮您的？' }}
									</div>
								</div>
								<div v-if="tjQuestList.length" class="mobileUniversalTemplate-layoutCenter-tjQuestList">
									<p class="label">您可以这么问我</p>
									<div v-for="(item, index) in tjQuestList" @click="sendQuestion(item.question)" :key="index" class="question">
							...
							<div style="display: flex" :style="{ padding: isMobile ? ' 0 12px' : ' 0 30px' }" v-if="curRouteId == policyUrl && !isMobile">
								<w-avatar v-if="getAppDetail()?.robotIcon" style="margin-right: 8px" :size="28"><img :src="getAppDetail()?.robotIcon" /></w-avatar>
								<div
...
.mobileUniversalTemplate-layoutCenter {
	flex: 1;
	height: 100%;
...
.mobileUniversalTemplate-layoutCenter-prologue .txt{
	background-color: #fff
}
::v-deep .text-message-reply-left{
	background: #fff;
}
::v-deep .yy-message-mobileUniversalTemplate .btns{
	background-color: #fff;
	margin-top: -4px;
