<template>
	<div :class="isMobile ? 'chatModule-mobile' : 'chatModule'" @click="stop" ref="$el">
		<ChatInput v-if="route.path.includes('/knowledgeDetails') || route.path.includes('/knowledgeDetailLh') || route.path.includes('/virtualTemplate')"></ChatInput>
		<ChatInputTheme v-else></ChatInputTheme>
	</div>
</template>
...
const ChatInputTheme = defineAsyncComponent(() => import('./components/chatInputTheme.vue'));
const stop = (e) => {
	e.stopPropagation();
};
