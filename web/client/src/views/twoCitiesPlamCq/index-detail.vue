				...
				<div class="chatDivFlex" :class="{ chatDivFlex2: !isMobile }" :style="particlesContainerStyle">
					<div class="chat-content">
						...
						<div class="chat-bottom" :class="{ 'chat-bottom-mobile': isMobile }">
							<!-- 嗨实践/嗨活动 -->
							<ChatLayoutCenterHiPractice v-if="['hiPractice', 'hiActivity'].includes(assistantType)" />
							<!-- 政策帮 -->
							<ChatLayoutCenterPolicyHelp v-if="['policyHelp'].includes(assistantType)" />
							<!-- 全模板 -->
							<ChatLayoutCenter v-if="assistantType == 'gaoxinqu'" />
						</div>
					</div>
				</div>
			</w-layout-sider>
...
const ChatLayoutCenter = defineAsyncComponent(() => import('../chat/components/layoutCenterCqH5.vue'));
// 嗨实践/嗨活动
const ChatLayoutCenterHiPractice = defineAsyncComponent(() => import('../chat/components/layoutCenterCqH5HiPractice.vue'));
// 政策帮
const ChatLayoutCenterPolicyHelp = defineAsyncComponent(() => import('../chat/components/layoutCenterCqH5PolicyHelp.vue'));
