<template>
	...
	<div style="position: relative" class="apptemplate-chatInput"
		...
		<div class="flex-box">
			<div v-if="!isMobile && videoResolveFlag" class="content-bottom"
				:class="[showVideoContent ? 'showPosition' : 'hidePosition']">
				<el-upload v-if="!fileData?.fileName" class="upload-demo" ref="upload" action="#"
					:show-file-list="false" :before-upload="videoBeforeUpload" :http-request="videoUploadHandler"
					accept=".mp4">
					<div class="video-btn"><img class="video-file" src="/src/assets/apptemplate/video-file.svg"
							alt="" />视频解析</div>
				</el-upload>
				<div v-else class="video-show" :style="{ height: !showVideoContent ? '56px' : '184px' }">
					<div class="video-show-content">
						<div class="video-show-content-left">
							...
							<iconpark-icon v-if="!isAnalysis" name="play-circle-line" size="16" color="#1D2129"
								style="margin-left: 12px; cursor: pointer" @click="viewVideo"></iconpark-icon>
							<div v-else class="isAnalysis"><img class="ring-resize"
									src="/src/assets/apptemplate/ring-resize.svg" alt="" />解析中</div>
						</div>
						<div class="video-show-content-right">
							<iconpark-icon v-if="!showVideoContent && videoContent" name="movie-ai-line" color="#000000"
								size="16"></iconpark-icon>
							<iconpark-icon v-if="showVideoContent && videoContent" name="arrow-down-double-line"
								color="#000000" size="16"></iconpark-icon>
							<span v-if="videoContent" @click="showVideoContent = !showVideoContent">{{ !showVideoContent
								? '视频总结' : '收起总结' }}</span>
							<iconpark-icon name="delete-bin-4-line" color="#1D2129" size="16" style="cursor: pointer"
								@click="clearFileData"></iconpark-icon>
						</div>
					</div>
			...
			<div class="select-tag-box" v-if="aiQuestionFlag">
				<el-tag :effect="isCheckSetQuestion ? 'dark' : 'plain'" class="select-tag" @click="addQuestion"> AI出题
				</el-tag>
			</div>
		</div>
		...
		<div class="title-flag">
			<img src="../../../../../assets/aiVideoSplitting/vidioe-falg.png" alt="">
			<span>视频生成</span>
		</div>
		<div class="textarea">
			...
			<w-textarea v-show="!isInsertPrompt" :class="{other:fileList.length > 0 ? true :false,}" ref="textareaRef"
				@input="changeTextare" :style="{height:changeTextare()<=2?'110px':'', }"
			...
			<div class="video-tpye">
				<div class="type-item">
					<div class="item-box" @click="isShowProportion = true">
						<iconpark-icon name="table-line" size="15px" color="#3F4247"
						style="margin-right: 6px;"></iconpark-icon>
					<span>比例</span>
					</div>
					<div class="proportion" v-show="isShowProportion" @mouseleave="isShowProportion = false">
						<div class="proportion-item" :class="{ 'active': resolutionValue === item.name }" v-for="(item, index) in proportionData" :key="item.name" @click="chooseProportion(item)">
				...
				<div class="type-item">
					<div class="item-box" @click="isShowResolution = true">
						<iconpark-icon name="shape-2-line" size="15px" color="#3F4247"
						style="margin-right: 6px;"></iconpark-icon>
					<span>分辨率</span>
					</div>
					<div class="proportion" v-show="isShowResolution" @mouseleave="isShowResolution = false">
						<div class="proportion-item" :class="{ 'active': ratio === item.value }" v-for="(item, index) in resolution" :key="item.name" @click="chooseResolution(item)">
							<span>{{ item.name }}</span>
						</div>
					</div>
				</div>
				<!-- timeData -->
				<div class="type-item">
					<div class="item-box" @click="isShowTime = true">
						<iconpark-icon name="time-line" size="15px" color="#3F4247"
						style="margin-right: 6px;"></iconpark-icon>
					<span>时长</span>
					</div>
					<div class="proportion" v-show="isShowTime" @mouseleave="isShowTime = false">
						<div class="proportion-item"  :class="{ 'active': duration === item.value }" v-for="(item, index) in timeData" :key="item.name" @click="chooseTime(item)">
							<span>{{ item.name }}</span>
						</div>
					</div>
				</div>
			...
			<el-tooltip style="width: 100%;height: 100%;" effect="dark" placement="top">
				<template #content>
					<div class="uploadtxt">最多上传{{info?.type != 'workflow' && info?.type != 'dialogue'
						?1:10}}个文档，单个最大{{info?.type != 'workflow' && info?.type != 'dialogue'?'15':'20'}}MB</div>
	 ...
	 ">
		...
		<el-tooltip effect="dark" content="按需搜索网页" placement="right">
			<el-button v-if="showNetworkFlag && isShowNetWork" style="position:absolute;bottom:13px;left:16px;border:0!important;width: 106px;
				...
				" :style="{background:!chatStore.$state.isNewWork?'#f4f6f9!important':'#E7ECFC!important'}"
				@click="chatStore.$state.isNewWork = !chatStore.$state.isNewWork;">
				<iconpark-icon name="global-line" size="17"
...
line-height: 20px;
text-align: left;
font-style: normal;" :style="{color:chatStore.$state.isNewWork?'#4D6BFE':'#3F4247'}">联网搜索</span>
			</el-button>
		</el-tooltip>
		<getAudio v-if="(csStt || rsjTts) && !dialogueInputLoading && isHaveTtsId()" :isMobile="isMobile"
			@sendAudio="sendAudio" :dialogueInputLoading="dialogueInputLoading"></getAudio>
		<span v-if="!csStt && !rsjTts && !dialogueInputLoading && isHaveTtsId()">
...
import {apiGetText,apiGetImg, getVidioeData} from '/@/api/chat'
...
import img1to1 from '../../../../../assets/aiVideoSplitting/1-1.svg'
import img4to3 from '../../../../../assets/aiVideoSplitting/4-3.svg'
import img3to4 from '../../../../../assets/aiVideoSplitting/3-4.svg'
import img16to9 from '../../../../../assets/aiVideoSplitting/16-9.svg'
import img9to16 from '../../../../../assets/aiVideoSplitting/9-16.svg'
import img21to9 from '../../../../../assets/aiVideoSplitting/21-9.svg'
import img9to21 from '../../../../../assets/aiVideoSplitting/9-21.svg'
//视频比例数据
const proportionData = ref ([
	{
		name: '1:1', img: img1to1,
	},
	{
		name: '4:3', img: img4to3,
	},
	{
		name: '3:4', img: img3to4,
	},
	{
		name: '16:9', img: img16to9,
	},
	{
		name: '9:16', img: img9to16,
	},
	{
		name: '21:9', img: img21to9,
	},
	{
		name: '9:21', img: img9to21,
	}
])
//视频分辨率数据
...
const isShowProportion = ref(false)  //比例
const isShowResolution = ref(false)   //分辨率
const isShowTime = ref(false)  //市场
const resolutionValue = ref('')
const ratio = ref('')
const duration = ref(5)
//选择比例
const chooseProportion = (item) => {
	resolutionValue.value = item.name
	isShowProportion.value = false
}
//选择分辨率
const chooseResolution = (item) => {
...
//获取视频接口
const getVidioe = async () => {
	const res = await getVidioeData({content: 'maomi'})
}
...
const setInput = async () => {
	mittBus.emit('text-event', text.value);
	textarea.classList.remove('padding1')
	if(speechAlBox.value)
	speechAlBox.value.stop()
