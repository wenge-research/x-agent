	...
	<el-dialog v-model="outLoginDialogVisible" width="300" center top="35vh">
		...
		<template #footer>
			<div class="dialog-footer">
				...
				<el-button type="primary" @click="outLoginDialogVisible = false"> 取消 </el-button>
			</div>
		</template>
	</el-dialog>
	<div class="relative container_chat" :class="wrapClass" @click="openAllDialog">
		<div class="user-menu" v-if="isToken && isHovered" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
		...
		<w-layout :class="{ MobileH: !isMobile }">
			...
			<w-layout-sider
				...
				class="flex w-layout-sider-right zcflex"
				:resize-directions="previewData.active && route.params.appId != 0 ? ['left'] : []"
			>
				<div class="chatDivFlex" :class="{ chatDivFlex2: !isMobile, bgorImg: !backgroundImageUrl() }" :style="particlesContainerStyle">
						...
						<div class="top">
							<div class="smallImg" style="margin-bottom: 16px" @click="historyDialogClick">
								...
								<div class="chat-box" title="历史对话">
									...
									<iconpark-icon name="chat-history-line" color="#1D2129" size="20"></iconpark-icon>
								</div>
						...
						<div class="footer">
							<div class="smallImg" v-if="getmzsm()">
								<el-popover placement="right-end" :width="400" :show-arrow="false">
									...
									<template #reference>
										<iconpark-icon name="information-line" size="20" color="#3F4247 "></iconpark-icon>
									</template>
								</el-popover>
							</div>
							<div class="smallImg" style="display: flex; justify-content: center; align-items: center">
								<el-popover class="box-item" :width="216" trigger="click" placement="right-start">
									...
									<div @mouseleave="mouseleave" class="font-box">
										<div style="font-family: MiSans, MiSans; font-weight: 500; font-size: 14px; color: #383d47; line-height: 20px">字号大小</div>
									...
									<template #reference>
										<div>
											<iconpark-icon
												...
												@click="font = true"
											></iconpark-icon>
											<iconpark-icon class="nothover" name="font-size-2" color="#3F4247" size="20" v-show="!showliderValue"></iconpark-icon>
										</div>
									</template>
								</el-popover>
							</div>
					...
					<div class="history" v-if="isHistory && !chatStore.isShare">
						<div class="top">
							...
							<div class="right">
								<iconpark-icon name="close-line" color="#3F4247" size="20" @click="isHistory = false"></iconpark-icon>
							</div>
						</div>
						<div class="content">
							<div class="nodata" v-if="!isToken">
								...
								<div class="txt" style="cursor: pointer" @click="toLogin">登录后可查看</div>
							</div>
					...
					<div
						class="chat-content"
						:class="{ historyBorder: isHistory, number: isNumber, chatEmbed: implantSwitch, openReportBorder: showReport }"
					...
					>
						<div class="top-logo" v-if="!isMobile">
							...
							<div class="newTalk" @click="refreshPage" @mouseenter="isTouch = true" @mouseleave="isTouch = false">
								<div class="img">
									<iconpark-icon name="chat-new-line" color="#1747E5" class="add-icon" size="18"></iconpark-icon>
								</div>
								<div class="txt">新对话</div>
							</div>
						</div>
					...
					<div v-if="showReport" class="open-report">
						<AnalysisReport :sourceData="drawerParams" @closeReport="closeReportHandler" :key="timer" />
					</div>
					<div class="numberPeople" v-if="isNumber && !chatStore.isShare">
						<div class="content">
							<div class="txt">数字人<br />展示区域</div>
						</div>
					</div>
					<div v-if="chatStore.isShare" class="share">
						<div class="share-top">
							<div style="height: 32px; width: 100%; padding: 0 368px 0 296px; display: flex; justify-content: space-between; align-items: center">
								<div class="title">分享对话</div>
						...
						<div class="share-content" ref="shareRef">
							<div v-if="isQRcode" class="share-content-title">
								<img v-if="logoUrl()" class="lh-logo" :src="logoUrl()" style="height: 100%; margin: 0 auto" />
							</div>
							<div v-for="(item, index) of isSelected" :key="item.id" class="main" :class="{ hide: !hidden[index] }">
								<YYMessageTtAnalysis
									:answer="item.answer"
									...
									:id="item.id + ''"
									:isAiQuestion="item.isAiQuestion"
								...
								<YYMessageTtAnalysis
									...
									:clientId="item.clientId"
									@handleFillForm="handleFillForm"
									@playVoice="playVoice(item.answer)"
									align="left"
									:index="index"
								>
								</YYMessageTtAnalysis>
							...
							<div v-if="isQRcode" style="position: relative; height: 148px">
								...
								<qrcode-vue style="position: absolute; right: 76px; bottom: 30px" :value="qrcodeValue" :size="88" />
							</div>
						</div>
						<div class="share-footer">
							<div style="padding: 0 368px 0 296px; width: 100%; height: 100%; display: flex; justify-content: space-between; align-items: center">
								<div class="left"></div>
								<div class="right">
									<el-button
										@click="handleSaveImg('fuzhi')"
										style="
									...
									<el-button
										...
										"
									>
										<iconpark-icon name="download-line" color="#FFFFFF" style="margin-right: 4px" size="16"></iconpark-icon>
										保存为图片
									</el-button>
									<input ref="fallbackInput" type="text" :value="textToCopy" style="position: fixed; top: -100px" readonly />
								</div>
							</div>
						</div>
					</div>
			...
			<w-layout-sider
				...
				class="flex w-layout-sider-right"
	...
	nextTick(async () => {
		...
		if (type == 'baocun') {
			...
			try {
				...
				if (res.code == '000000') {
					const shareLink = `【智川】这是我和${
						JSON.parse(window.localStorage.getItem(`${route.params.appId}`)).applicationName
					}的聊天对话，你也来试试吧~ 【点击链接直接打开】${window.location.href}?key=${res.data}`;
					// 更新二维码的值
