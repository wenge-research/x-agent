					...
					<div class="chat-content">
						<div class="top-logo" v-if="!isMobile">
							...
							<div class="right">
								...
								<!-- 市监局数字人 -->
								<div v-if="isSZR" class="right-box" @click="showSZRHandler">
                    ...
                    <span @click="backOld">返回旧版</span>
						...
						<div class="chat-bottom" :class="{ 'chat-bottom-mobile': isMobile }">
							...
							>
								...
								<div v-show="isSZR && showSZR" class="szr" style="margin-left: 30px; width: 100%; height: 100%"></div>
			...
			<div v-if="aboutWebsite" class="footer-copyrightInfo" :class="{ 'footer-copyrightInfo-mobile': isMobile }">
				...
				<div class="copyrightInfo-inner">
					<div class="copyright-left" style="width: 495px">
						...
						<p>办公室主办<span style="margin-left: 30px; color: #666">网站技术维护电话：0755-23332038</span></p>
...
// 流式播放代码
import { textToSpeechAndPlay } from '/@/utils/newVoiceFun';
...
const start = (e, value: string) => {
	...
	Client = new window.RTCInteraction({
		...
		onError(errorMes) {
			...
			if (errorMes?.message == '权益不足') {
				szrVisible.value = false;
			}
		},
		onInited(res) {
...
const splitTextIntoChunks = (answer, previousAnswer, previousChunks) => {
...
const ttsSync = (text, index) => {
	let cleanedText = text.replace(/(\\n)/g, '');
	cleanedText = cleanedText.replace(/<a[^>]*>.*?<\/a>/g, '');
	Client.talk(cleanedText, true);
	playArr[index] = cleanedText;
	// 确保 startPlay 只被调用一次
...
const startPlay = () => {
	...
	playJson[playIndex].addEventListener('ended', () => {
		playIndex++;
		setTimeout(startPlay, 500); // 延迟 500ms 后播放下一个音频片段
	});
};

// 主播放逻辑
const textToSpeechAndPlay = async (newChunks, clientId) => {
	...
	if (newChunks.length > oldChunks.length) {
		oldChunks = newChunks;
		ttsSync(oldChunks[currentIndex], currentIndex);
		currentIndex++;
	}
};
...
onMounted(async () => {
	...
	mittBus.on('szrPlayStart', ({ chunksList, clientId }) => {
		...
		// 	mittBus.emit('startTalk',clientId);
		// 	newChunks.value = chunksList
		// 	sendStreamTextTest(newChunks.value)
		// }
		// 流式播放代码
		mittBus.emit('startTalk',clientId);
		textToSpeechAndPlay(chunksList, clientId);
	});
	// mittBus.on('stopTalk', () => {
	// 	stopPlay();
	// });
