<template>
  <div class="container_chat relative" :class="wrapClass" @click="openAllDialog">
    <w-layout :class="{ MobileH: !isMobile }">
      <w-layout-content
        v-if="curRouteId == policyUrl && previewData.active && route.params.appId != 0"
      >
        <layoutCenterPdf v-if="previewData.active && route.params.appId != 0" />
      </w-layout-content>
      <w-layout-content
        v-if="curRouteId == judicialUrl && previewData.active && route.params.appId != 0"
        class="sfLayout"
      >
        <layoutCenterPdf v-if="previewData.active && route.params.appId != 0" />
      </w-layout-content>
          ...
          <div class="chat-content">
            <div class="top-logo" v-if="!isMobile">
              <div>
                ...
                <img
                  class="lh-logo"
                  src="/src/assets/chatImages/pageTitle.svg"
                />
              </div>
              <div class="right">
...
const ChatLayoutCenter = defineAsyncComponent(
  () => import("../chat/components/chatLayoutCenterSzcac.vue")
);
const layoutCenterPdf = defineAsyncComponent(
  () => import("../knowledge/components/layoutCenterPdf.vue")
);
// 移动端自适应相关
