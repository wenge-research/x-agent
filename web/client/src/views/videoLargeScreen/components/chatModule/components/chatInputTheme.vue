<template>
	<div class="chatInputTheme-videoLargeScreen">
		<img
			v-if="dialogueInputLoading && !uploadImgStatusLoading"
			@click="stopChatAndVideo"
			src="/src/assets/videoLargeScreen/stop.png"
			alt=""
		/>
		<div v-else>
			<img v-if="btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING'" @click="openYY('ly')" src="/src/assets/videoLargeScreen/start.png" alt="" />
			<img v-else @click="openYY('ly')" src="/src/assets/videoLargeScreen/speak.png" alt="" />
		</div>
	</div>
	<div v-if="false" class="tj" :style="{ 'justify-content': 'flex-end' }">
	...
	<div v-if="false" class="chatInput" :class="{ 'chatInput-mobiles': isMobile, 'chatInput-pc': !isMobile }">
		...
		/>
		<div v-else>
			<img v-if="btnStatus == 'OPEN'" @click="openYY('ly')" src="/src/assets/videoLargeScreen/speak.png" alt="" />
			<img v-else @click="openYY('ly')" src="/src/assets/videoLargeScreen/start.png" alt="" />
		</div>
...
const { startPlaying } = storeToRefs(chatStore);
...
const stopChatAndVideo = async () => {
	handleclickStopChat();
	stopPlay(() => {
		chatStore.startPlaying = false;
	});
};
...
.chatInput {
	...
	:deep(.w-textarea) {
		...
		padding: 16px 70px 4px 16px;
	}
	.active {
		:deep(.w-textarea) {
			padding: 16px 86px 4px 16px;
		}
	}
...
.chatInputTheme-videoLargeScreen {
	display: flex;
	align-items: center;
	justify-content: center;
	img {
