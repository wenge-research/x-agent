<template>
	<div class="datapanel" v-show="datapanelParasList.length">
		<w-scrollbar ref="panelScrollbarRef" class="datapanel-center-side" style="max-height: 176px; overflow: auto">
			<div class="panelList" ref="panel">
				<div
					class="panelItem"
					:active="active == index"
					v-for="(item, index) in datapanelParasList"
					:key="index"
					@click="penelItemConfirm(index, item)"
				>
					<i v-if="config.icon">
						<CoolWenjianjia class="fileIcon" v-if="item.type === 1" size="20" />
						<CoolDocx class="fileIcon" v-else-if="item[config.name].indexOf('.doc') != -1 || item[config.name].indexOf('.docx') != -1" size="20" />
						<CoolPdf class="fileIcon" v-else-if="item[config.name].indexOf('.pdf') != -1" size="20" />
						<CoolTxt class="fileIcon" v-else-if="item[config.name].indexOf('.txt') != -1" size="20" />
					</i>
					<span class="category" v-if="config.category" v-html="highlightText(item[config.category])"></span>
...
const active = ref(0);
const panel = ref();
const penelItemConfirm = (index: number, item: any) => {
	if (!datapanelParas.value.show) {
		return;
	}
	if (!index) {
		index = active.value;
		item = datapanelParasList.value[index];
	}
	let value = item[config.value.name];
	if (config.value?.multiple && value) {
		value += config.value.desc ? item[config.value.desc] : '';
	}
	if (datapanelParas.value.show === 2) {
		value = '@' + value;
	}
	// datapanelParas.value.show = 0;
	// datapanelParas.value.value = value || '';

	// datapanelParas.value.value = ''
	// datapanelParas.value.show = 0;
	// console.log(datapanelParas.value.show)

	// if(route.path.indexOf('chat') == -1 && datapanelParas.value.show != 1){
	if (route.path.indexOf('chat') == -1) {
		// if(datapanelParas.value.show === 1){
		// 	item = {}
		// }
		let params = {
			value,
		...
		mittBus.emit('InsertHtml', params);
	} else {
