						...
						<div class="top-logo" v-if="!isMobile">
							...
							<div class="right">
				                ...
				                <div class="font-setting">
									<div>
										<span style="color:rgba(255, 255, 255, .3)">字号设置：</span>
										<span @click="changeFontSize('2')" :style="{ color: sizeType === '2' ? '#fff' : 'rgba(255, 255, 255, .3)' }">大</span>
										<span @click="changeFontSize('1')" :style="{ color: sizeType === '1' ? '#fff' : 'rgba(255, 255, 255, .3)' }">中</span>
										<span @click="changeFontSize('0')" :style="{ color: sizeType === '0' ? '#fff' : 'rgba(255, 255, 255, .3)' }">小</span>
									</div>
								</div>
...
import chatLine from '/@/assets/ai/chat-history-line1.svg';
const knowledgeState = useKnowledgeState();
const previewData: any = computed(() => knowledgeState.previewData);
const chatStore = useChatStore();
import { formatDate } from '/@/utils/formatTime';
const ChatLayoutCenter = defineAsyncComponent(() => import('../chat/components/layoutCenterAssPc.vue'));
...
.container_chat :deep(.w-layout-sider-right) {
	...
	.chatDivFlex2 {
		...
		position: relative;
		background: url(../../assets/zhushou/pcbg.png) no-repeat;
		background-size: 100% 100%;
		.chat-content {
			position: absolute;
			width: 50%;
