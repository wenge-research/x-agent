<template>
	<div class="chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
		<i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="isMobile ? '' : 'top:20px;right:96color: rgb(65, 134, 244);px;'"
		>
        <iconpark-icon v-if="(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag" id="btn_control" name="mic-fill" size="16" color="#B4BCCC" style="margin-right: 18px;cursor: pointer"></iconpark-icon>
			<!-- <img
				src="/src/assets/zhushou/mic.svg"
				class="sendVoiceBtn"
			...
			<div v-else class="vedioLoaingBtn" :style="isMobile ? '' : 'top:-2px;'">
				...
				<CoolStopCircleLineWe size="16" color="var(--w-color-primary)" />
			</div>
		</i>
...
const APPID_TTS = "xxxxxxx";
const API_SECRET_TTS = "xxxxxxx";
const API_KEY_TTS = "ddc4e7ce8e45298cd90d5a6a956f2dc8";
...
const getWebSocketUrl_tts = () => {
  var apiKey = API_KEY_TTS;
  var apiSecret = API_SECRET_TTS;
...
.chatInput {
	...
	.sendBtn {
		width: 64px;
		height: 40px;
		background: linear-gradient(270deg, #218ff5 0%, #1563d6 100%);
