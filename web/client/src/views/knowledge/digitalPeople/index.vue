<template>
  <div class="container_chat relative" :class="wrapClass" @click="openAllDialog">
    <w-layout :class="{ MobileH: !isMobile }">
      ...
      >
        <div class="chatDivFlex" :class="{ chatDivFlex2: !isMobile }">
          <div class="chatDivFlex-left">
            <img src="/src/assets/szr/left-icon.svg" alt="" />
            <iconpark-icon
          ...
          <div class="chatDivFlex-content">
            <LayoutCenterSZR @handleFillForm="handleFillForm" />
          </div>
        </div>
      </w-layout-sider>
      <w-layout-sider
...
const Information = defineAsyncComponent(
  () => import("/@/views/information/index-pc.vue")
);
const LayoutCenterSZR = defineAsyncComponent(
  () => import("./components/layoutCenter-szr.vue")
);
...
const wrapClass = computed(() => {
  return [
    isMobile.value ? "container_chat_mobile" : "flex",
    curRouteId.value == import.meta.env.VITE_JUDICIAL_QA ? "chat-bg2" : "",
  ];
});
...
.container_chat :deep(.w-layout-sider-right) {
  ...
  .chatDivFlex {
	...
	.chatDivFlex2 {
		...
		width: 100vw;
		position: relative;
		background: url(../../../assets/szr/bg.png) no-repeat;
		background-size: 100% 100%;
		padding: 12px;
