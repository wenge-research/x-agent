			...
			<w-scrollbar ref="layoutScrollbarRef" class="center-side1">
				<div class="container-center">
					...
					<div class="container-center-head">
						<img src="/src/assets/szr/add-icon.png" class="add-icon" alt="" />
						<img v-if="logoUrl()" class="logo" :src="logoUrl()" />
						<img v-else class="logo" src="/src/assets/szr/default-img.png" />
						<div class="right" @click="showSZRHandler">
							<img v-if="!showSZR" src="/src/assets/szr/left-icon.png" class="left-icon" alt="" />
							<iconpark-icon name="account-circle-fill" color="#383D47" size="16"></iconpark-icon>
							<img v-if="showSZR" src="/src/assets/szr/right-icon.png" class="right-icon" alt="" />
						</div>
					</div>

					<!-- 问答内容 -->
					<div class="container-center-content" :class="[showSZR ? 'ml48' : 'm322']">
						<div :class="[showSZR && 'flex-1']">
							<!-- 问答部分 -->
							<div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side" v-if="!loading">
								...
								<div class="message-list">
									...
									<div :style="{ padding: isMobile ? ' 0 10px' : ' 0 30px' }" v-for="(item, index) of chatList" :key="index">
										<!-- <div @click="playSZR(item.answer)">{{ item.answer }}</div> -->
										<!-- {{ item }} -->
										<yyMessageSZR
											:answer="item.answer"
								...
								<div class="text-center" :style="{ height: isMobile ? '56px' : '2rem' }" style="position: relative; z-index: 1000; bottom: 8px">
									...
									>
										<template #icon>
											<iconpark-icon name="stop-fill" color="#1477E3" size="16" style="margin-top: 6px"></iconpark-icon>
										</template>
										停止生成
									</w-button>
								</div>
							...
							<!-- <div @click="closeSZR">关闭数字人</div> -->
							<!-- <div @click="initSZR">初始数字人</div>
							<div @click="playSZR">播报</div> -->

							<!-- 输入框 -->
							<ChatModuleSZR ref="chatModuleRef"></ChatModuleSZR>
						</div>

						<!-- 数字人部分 -->
						<div v-show="showSZR" class="szr"></div>
					</div>
				</div>
...
import { defineAsyncComponent, ref, computed, onMounted, watch, onUnmounted, nextTick } from 'vue';
...
const ChatModuleSZR = defineAsyncComponent(() => import('./index-new.vue'));
...
// 新代码逻辑
const showSZR = ref(true);

const showSZRHandler = () => {
	showSZR.value = !showSZR.value;
	start();
	// start().talk(
	// 	'时代浪潮奔腾向前，中华民族伟大复兴的征程已经开启。在这个伟大的时代，我们青年一代肩负着历史赋予的重任，应以青春之笔，绘时代华章。青春是用来奋斗的。“自信人生二百年，会当水击三千里。',
	// 	true
	// );
...
.center-side-parent {
	...
	.container-center {
		...
		&-content {
			display: flex;
			height: calc(100vh - 40px - 32px - 24px - 36px);
...
.question-box {
	...
	.title {
		...
		.tips {
			...
			line-height: 20px;
			text-align: center;
			background: rgba(20, 119, 227, 0.1);
			border-radius: 12px;
			font-family: MiSans, MiSans;
			font-weight: 400;
			font-size: 12px;
