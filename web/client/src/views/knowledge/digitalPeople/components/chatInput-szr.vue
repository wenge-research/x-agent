<template>
	...
	<div class="chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
		...
		<w-button
			...
			:style="isMobile ? '' : 'position:absolute;top:calc(50% - 23px);right:8px;'"
        ...
        <img :src="sendIcon" alt="" />
			...
			<iconpark-icon name="space-ship-fill" size="24" color="#fff"></iconpark-icon>
		</w-button>
		<getAudio
		...
		<i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="isMobile ? '' : 'top:calc(50% - 13px);right:68px;color: #1477E3;'"
		>
    <iconpark-icon v-if="(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag" class="sendVoiceBtn" name="mic-fill" size="20" color="#1477E3"></iconpark-icon>
