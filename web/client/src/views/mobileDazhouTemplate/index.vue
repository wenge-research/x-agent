<template>
	<div class="mobileUniversalTemplate container_chat relative" :style="{ backgroundImage: 'url(' + (backgroundImageUrl() ? backgroundImageUrl() : '/src/assets/chatImages/pageTitle.svg') + ')' }" :class="wrapClass" @click="openAllDialog">
    ...
    <startPage v-if="!showLayoutCenter" @sendStartParams="sendStartParams"></startPage>
		<w-layout :class="{ MobileH: true }" v-else>
			<ChatHeader @backHome="showLayoutCenter = false" />
			<w-layout-sider
