						...
						>
							...
							<!-- <div v-if="curRouteId == policyUrl && isMobile">
								<div v-if="virtualHumanLogo || getAppDetail()?.identityIcon || getAppDetail()?.greeting" class="mobileUniversalTemplate-layoutCenter-header">
									<div v-if="virtualHumanLogo" style="height: 64px; align-self: flex-start">
										<img :src="virtualHumanLogo" alt="" class="facadeImageUrl" />
									</div>

									<div v-if="getAppDetail()?.identityIcon || getAppDetail()?.greeting" style="flex: 1; overflow: hidden; margin-left: 12px">
										<div class="title">
...
.mobileUniversalTemplate-layoutCenter {
	...
	&-header {
		...
		.title {
			@include add-size($font-size-base18, $size);
			font-family: MiSans, MiSans;
			font-weight: 600;
			color: #000000;
