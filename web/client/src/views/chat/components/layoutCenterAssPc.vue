          ...
          <div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side"
            ...
            <div class="message-list">
              ...
              <div class="question-box" v-if="curRouteId == policyUrl && !isMobile">
                <img class="jqr-icon" style="height: 100%" :src="virtualHumanLogoUrl() ? virtualHumanLogoUrl() : '/src/assets/zhushou/guanxin.png'"  alt="" />
                <div>
                  <div style="margin-bottom:3px;">
                    <!-- <img style="height: 21px" src="/src/assets/zc/wdzs.png" alt="" />
										<div class="tip">试运行版本</div> -->
                    <!-- <img style="width:300px;"
                      :src="identityIconUrl() ? identityIconUrl() : '/src/assets/chatImages/pageTitle.svg'" /> -->
                    <div class="title">
                      <img   v-if="getAppDetail()?.identityIcon" :src="getAppDetail()?.identityIcon" style="width:300px;">
                      <span>试运行版本</span>
                    </div>
                    <!-- <div class="message">我可以为您解答政务相关问题，提供政策查询，政策解读和办事指南，为您提供贴心的服务</div> -->
                  </div>
                  <div class="box">
              ...
              <div class="quest-list">
                <p style="
                  ...
                  margin-bottom:12px;
                  text-align: justify;
                  font-style: normal;">您可以这么问我</p>
                <div class="item" v-for="(item, index) in tjQuestList" :key="index">
                  <!-- <div class="box" :style="{ width: tjQuestList.length <= 2 ? '43%' : (tjQuestList.length <= 3 ? '33%' : '') }"> -->
                  <div class="box" @click="sendQuestion(item.question)"
                    :class="{ active: curQuestion == item.question }">
                    <span style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">{{ item.question
                    }}</span>
                    <img style="height: 20px" src="/src/assets/zhushou/send.svg" />
                  </div>
                </div>
              </div>
              <div style="
							background: linear-gradient( 180deg, #154DF5 0%, #0076FF 22%, rgba(0,160,255,0) 100%), radial-gradient( 0% 100% at 78% -2%, #07FBFF 0%, rgba(57,118,246,0) 100%);
                ...
                <div
                    ...
                    <div style="width:calc(100% - 30px)">
                      <div
                        ...
                        <p style="font-family: MiSans, MiSans;
													...
													line-height: 24px;
													text-align: justify;
													font-style: normal;">您好！我是{{ getAppDetail()?.applicationName }}</p>
                      </div>
                      <div style="color: #fff;font-size:12px;line-height: 19px;margin-bottom:36px;">{{
                        getAppDetail()?.greeting }}</div>
                    </div>
                    <div style="width:90px;position:relative;">
                      <img src="/src/assets/chatImages/woman.png"
                        style="width:110px;max-width: 110px;position: absolute;top: -26px;right: -22px;">
                      <img src="/src/assets/chatImages/man.png"
                        style="width:110px;max-width: 110px;position: absolute;top: -26px;right: -22px;" v-if="false">
                      <!-- <span
                        style="cursor:pointer;position: absolute;top: 94px;right: -24px;z-index:100;display:inline-block;width:78px;height:24px;background: #3976F6;border-radius: 12px 0px 0px 12px;">
                        <img src="/src/assets/chatImages/shuaxin.svg"
                          style="width:14px;position: absolute;left: 6px;top: 4px;">
                        <span style="color:#fff;font-size:12px;position: absolute;left: 26px;top: 3px;"
                          @click="changeModel">更换助手</span>
                      </span> -->
                    </div>
                  </div>
                    ...
                    <div style="margin-top:70px;text-align:center;">
                      <el-radio-group v-model="headchoose">
                        <el-radio value="1" size="large">
                          <img src="/src/assets/chatImages/manchat.png"
                            style="width:80px;height:80px;border-radius:50%;position:absolute;top:9px;" />
                          <img src="/src/assets/chatImages/men-line.svg"
                            style="width:16px;position:absolute;top:115px;left:40px;" />
                        </el-radio>
                        <el-radio value="2" size="large">
                          <img src="/src/assets/chatImages/womanchat.png"
                            style="width:80px;height:80px;border-radius:50%;position:absolute;top:9px;" />
                          <img src="/src/assets/chatImages/women-line.svg"
                            style="width:16px;position:absolute;top:115px;left:40px;" />
                  ...
                  <div class="quest-list-mobile">
                    ...
                    <div class="item" v-for="(item, index) in tjQuestList" @click="sendQuestion(item.question)"
                      :key="index">
              ...
              <div :style="{ padding: isMobile ? ' 0 10px' : ' 0 30px' }" v-for="(item, index) of chatList" :key="index">
                <YYMessageAssPc :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                  ...
                  :imgUrl="item.imgUrl" :sensitive="item.sensitive" :stopChatLoading="stopChatLoading"></YYMessageAssPc>
                <YYMessageAssPc :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                  :id="item.id + ''" :dialogueFileIds="item.dialogueFileIds || []"
                  :dialogueFolderIds="item.dialogueFolderIds || []" :paragraph="item.paragraph || ''"
                  :skillId="item.skillId || ''" :param="item.param" :question="item.question" :loading="item.loading"
                  ...
                  :plainText="item.plainText"></YYMessageAssPc>
              </div>
            </div>
...
const ChatModule = defineAsyncComponent(() => import('./chatModule/index-assPc.vue'));
...
.question-box {
  ...
  background: linear-gradient(0deg , rgba(17,144,255,0.2) 0%, rgba(17,144,255,0.03) 99%);  background-size: cover;
  border-radius: 12px;
  padding: 30px 18px 30px 130px;
  display: flex;
  position: relative;
  align-items: center;
