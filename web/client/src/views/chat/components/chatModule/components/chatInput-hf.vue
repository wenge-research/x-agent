<template>
	...
	<div class="tj" v-if="!isMobile && tjQuestList.length > 0" :style="{ 'justify-content': curRouteId == judicialUrl ? 'flex-end' : 'flex-start' }">
		<div class="left-title" v-if="curRouteId == policyUrl">
			<img :src="event" alt="" />
		</div>
		<div class="center-box" v-if="curRouteId == policyUrl">
			<div class="center-item" v-for="(item, index) in tjQuestList.slice(0, 1)" :key="index">
...
import { tjQuestion } from '/@/api/chat';
...
//
const getPresetQuestionListFun = async (val) => {
	let res = await tjQuestion({
		question: val ? val : text.value,
		applicationId: localStorage.getItem(`${route.params.appId}appId`),
	});
	let arr = new Map();
	tjQuestList.value = res.data.filter((a) => a.category && !arr.has(a.category) && arr.set(a.category, 1));
};
const clearQuery = () => {
...
const getTtsConfigListFun = async () => {
	...
	if (appInfo?.virtualHumanFlag == '是') {
		let res = await getTtsConfigList(appInfo?.ttsId);
...
onMounted(() => {
	...
	mittBus.on('getTjList', (val: string) => {
		curAsktext.value = val;
		tjQuestList.value = [];
	});
	mittBus.on('getPresetQuestionList', (tjArr) => {
		let arr = new Map();
		tjQuestList.value = tjArr.filter((a) => a.category && !arr.has(a.category) && arr.set(a.category, 1));
	});
...
const refreshPage = () => {
	...
	mittBus.emit('getTjList', '');
};
// 页面销毁时，关闭监听
onUnmounted(() => {
	mittBus.all.delete('getTjList', () => {});
...
watch(
	() => knowledgeState.param1,
	(val) => {
		if (val.trim()) {
		console.log(222222222);
			text.value = val;
			setInput();
			knowledgeState.setparam1('')
		}
	}
);
watch(
