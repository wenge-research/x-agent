<template>
	...
	<div class="chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
		...
		<w-button
			...
			:style="isMobile ? '' : 'position:absolute;top:calc(50% - 29px);right:10px;'"
        ...
        <img :src="sendIcon" alt="" />
			...
			<!-- 发送 -->
      		<iconpark-icon name="space-ship-fill" color="#fff" size="18"></iconpark-icon>
		</w-button>
		<getAudio
		...
		<i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="isMobile ? '' : 'top:calc(50% - 10px);right:96color: rgb(65, 134, 244);px;'"
		>
			<!-- <img
				src="/src/assets/chatImages/yuyin-new.png"
				class="sendVoiceBtn"
			/> -->
			<iconpark-icon v-if="(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag" name="mic-fill" size="16" color="#A40002" style="margin-right: 24px;cursor: pointer"></iconpark-icon>
			<div v-else class="vedioLoaingBtn" :style="isMobile ? '' : 'top:-9px;'">
				<div class="time-box">
				...
				<CoolStopCircleLineWe size="24" color="#A40002" />
			</div>
		</i>
...
import backIcon from '/@/assets/chat/tz-arrow.png';
...
.tj {
	...
	.right-back {
		...
		.back {
			...
			height: 32px;
			background: #ffffff;
			box-shadow: 0px 4px 8px 0px rgba(43, 84, 61, 0.1);
			border-radius: 8px;
...
.chatInput {
	...
	.sendBtn {
		width: 80px;
		height: 52px;
		background: linear-gradient(90deg, #e54b4d 0%, #a90507 100%);
		border-radius: 6px;
		display: flex;
		align-items: center;
		justify-content: center;
