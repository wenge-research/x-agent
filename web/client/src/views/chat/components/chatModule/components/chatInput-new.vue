<template>
  ...
  <div class="tj" v-if="!isMobile" :style="{ 'justify-content': curRouteId == judicialUrl || tjQuestListRecommend.length == 0 ? 'flex-end' : 'flex-start' }">
  ...
  <div class="chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
    ...
    <w-textarea v-show="!isInsertPrompt" ref="textareaRef" @keydown="setInputEnter($event)" v-model="text"
      :placeholder="placeholder" :auto-size="{ minRows: 1, maxRows: 6 }" :max-length="500" />

    <w-button type="primary" v-if="!dialogueInputLoading"
      :style="isMobile ? '' : 'position:absolute;top:calc(50% - 23px);right:20px;'"
      :disabled="imgUrl == '' && insertpromptText == '' && text == ''" class="sendBtn"
      :class="{ sendBtnMobile: isMobile }" @click="setInput">
      <!-- <template #icon>
        <img :src="sendIcon" alt="" />
      </template> -->
      发送
    </w-button>
    <getAudio v-if="rsjTts && !dialogueInputLoading && isHaveTtsId()" :isMobile="isMobile" @sendAudio="sendAudio" :dialogueInputLoading="dialogueInputLoading"></getAudio>
    <span v-if="!rsjTts && !dialogueInputLoading && isHaveTtsId()">
			<speech-ali v-if="ttsConfig && ttsConfig.strategy === 'sttAliyunStrategy'" :appId="ttsConfig.appId" @changeResultText="changeResultText"></speech-ali>
			<i v-else class="vedioBtn" id="btn_control" @click="openYY('ly')"
        :style="isMobile ? '' : 'bottom:24px;right:96color: rgb(65, 134, 244);px;'">
        <img src="/src/assets/chatImages/yuyin-new.png" class="sendVoiceBtn"
    ...
    <Vue3Lottie v-if="dialogueInputLoading && !uploadImgStatusLoading" class="loadingAnimate" :animationData="toJSON"
      :height="55" :width="55" :style="isMobile ? '' : 'top:5px;right:10px;'" />
  </div>
  ...
  <el-dialog title="" v-model="chatFlag" width="80%" :modal="false" :append-to-body="true" custom-class="chat-dialog"
    ...
    <div class="box">
      ...
      <div
        style="width:364px;height:364px;border-radius:50%;background: rgba(255,255,255,.1);margin:20px auto 0;position:relative;">
        <div
          style="width:264px;height:264px;border-radius:50%;background: rgba(255,255,255,.1);margin:auto;  position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);">
          <div
            style="width:192px;height:192px;border-radius:50%;background: rgba(255,255,255,.1);margin:auto;position:absolute;top:50%;left: 50%;transform:translate(-50%, -50%);">
...
const speechAli = defineAsyncComponent(() => import('/@/views/appTemplate/components/chatModule/components/speechAli.vue'));
