<template>
  <citationContent
    v-if="!isChat && citationText"
    @instructSend="instructSend"
    @instructClear="instructSet"
  >
  </citationContent>
  ...
  <div
    class="chatInput"
    :class="{
      'chatInput-mobiles': isMobile,
      'chatInput-pc': !isMobile,
      chatInputYy:
        !((btnStatus === 'UNDEFINED' ||
          btnStatus === 'CLOSED' ||
          btnStatus === 'CLOSING') &&
  ...
  >
    <div v-show="isInsertPrompt" class="yayi-editor-wrapper">
      <div
        ref="insertpromptbox"
        class="yayi-editor scroll-display-none"
        @input="setDivInput()"
        contenteditable="true"
      ></div>
      <span
        v-if="insertpromptText.length > 2000"
        class="w-textarea-word-limit yayi-editor-limit"
    ...
    <w-textarea
      ...
      :placeholder="isContentLengthLimit() ? placeholder : '在此输入您的问题'"
			...
			:max-length="isContentLengthLimit() ? 80 : null"
    />
    <div v-if="inputType == 'text'" class="voice-btn">
      按住说话
    </div>
    ...
    >
      <iconpark-icon v-if="inputType == 'text'" name="keyboard-fill" size="20" color="#818999" style="margin-left: 12px;position: relative;top: -2px;left: 0;" @click="checkInputType('voice')"></iconpark-icon>
      <img v-if="
          inputType == 'voice'" style="width: 42px; height: 42px" src="/src/assets/chongqing/input-left.png" />
    </span>
    <!-- <i
    ...
    >
      <!-- <img
        src="/src/assets/chongqing/mic-fill.svg"
        class="sendVoiceBtn"
        v-if="
          (btnStatus === 'UNDEFINED' ||
            btnStatus === 'CLOSED' ||
      ...
      /> -->
        ...
        @click="openYY('ly')"
      />
      <img v-else-if="text" style="width: 42px; height: 42px;margin-right: 6px" src="/src/assets/chongqing/send.png"  @click="setInput" />

      <div v-else class="vedioLoaingBtn" :style="isMobile ? '' : 'top:-2px;'" @click="openYY('ly')"> 
        <div class="time-box">
    ...
    <Vue3Lottie
      ...
      :height="55"
      :width="55"
      :style="isMobile ? '' : 'top:11px;right:10px;'"
    />
  </div>
  ...
  >
    <div class="box">
      <div style="display: flex; justify-content: flex-start">
        <img
          src="/src/assets/chatImages/left.svg"
          style="width: 36px; height: 51px; margin: 0 0 0 16px"
          @click="quitYY"
        />
      ...
      <div class="chatType" v-if="btnStatus == 'OPEN' && btnStatus_tts != 'PLAY'">
        <span style="font-size: 22px; color: #fff; font-weight: 500; margin: 12px 0 8px"
          >正在聆听…</span
        >
        ...
        <div style="margin-top: 60px">
          <div style="display: flex; justify-content: center; align-items: center">
            ...
            <img
              src="/src/assets/chatImages/guaduan.svg"
              style="width: 64px; height: 64px"
              @click="quitYY"
            />
          </div>
          <p style="font-size: 15px; color: #fff; text-align: center; margin-top: 24px">
            结束语音对话
          </p>
        </div>
      </div>
      <div
        class="chatType"
        v-if="btnStatus == 'CLOSED' && btnStatus_tts != 'PLAY' && chatSBFlag"
      >
        <span style="font-size: 22px; color: #fff; margin-top: 24px">正在思考...</span>
        <img
          src="/src/assets/chatImages/guaduan.svg"
          style="width: 64px; height: 64px; margin: 90px auto 0"
          @click="quitYY"
        />
        <p style="font-size: 15px; color: #fff; text-align: center; margin-top: 12px">
          结束语音对话
        </p>
      </div>
      <div
        ...
        v-if="btnStatus == 'CLOSED' && btnStatus_tts != 'PLAY' && !chatSBFlag"
      >
        <span style="font-size: 15px; color: #fff; text-align: center; margin-top: 24px"
          >未识别到声音</span
        >
      ...
      <div class="chatType" v-if="btnStatus_tts == 'PLAY'">
        ...
        <img
          src="/src/assets/chatImages/guaduan.svg"
          style="width: 64px; height: 64px; margin: 60px auto 0"
          @click="quitYY"
        />
        <p style="font-size: 15px; color: #fff; text-align: center; margin-top: 12px">
          结束语音对话
...
const btnControlFun = (type: string = "") => {
  console.log("btnStatus.value", btnStatus.value)
...
const limitWords = () => {
  if (insertpromptbox.value.innerText.length > maxLength.value) {
    insertpromptbox.value.innerText = insertpromptbox.value.innerText.substring(
      0,
      maxLength.value
    );
    setTimeout(() => {
...
const promptInsertHtml = (data) => {
  ...
  nextTick(() => {
    ...
    if (highlightInput) {
      ...
      highlightInput.style.setProperty(
        "border",
        "1px solid rgb(36, 84, 255)",
        "important"
      );
...
const InsertHtml = (data: any) => {
  ...
  nextTick(async () => {
    ...
    let spanHtml = `<span class="span-highlight-input ${
      data?.id ? "dataId" : ""
    }" style="margin: 0 0 0 4px; padding: 3px 3px; border-radius: 4px;" data-id="${
      data?.id ? data.id : ""
    }" data-type="${data?.type ? data.type : ""}" data-prompt="${
      data?.prompt ? data.prompt : ""
    }">${val}</span><span>${data?.prompt ? " @" : " "}</span>`;
    // if(!data?.value){
...
const getTtsConfigListFun = async () => {
  let appInfo = localStorage.getItem(`${route.params.appId}`)
    ? JSON.parse(localStorage.getItem(`${route.params.appId}`))
    : "";
...
const connectWebSocket_tts = (tt) => {
  ...
  ttsWS.onopen = (e) => {
    ...
    if (
      ttext.lastIndexOf("我还在成长中") > -1 &&
      ttext.lastIndexOf("抱歉，我还在成长中") == -1
    ) {
...
onUnmounted(() => {
  ...
  insertpromptbox.value &&
    insertpromptbox.value.removeEventListener("click", handleClick);
  document.removeEventListener("click", handleClick);
  insertpromptbox.value &&
    insertpromptbox.value.removeEventListener("keydown", handleKeydown);
  insertpromptbox.value &&
    insertpromptbox.value.removeEventListener("paste", handlePaste);
});
let status = 0;
watch(
  () => text.value,
...
.chat-dialog {
  ...
  .el-dialog__header {
    background: radial-gradient(
        70% 50% at 91% 76%,
        rgba(103, 209, 255, 0.2) 0%,
        rgba(103, 209, 255, 0) 100%
      ),
      radial-gradient(
        69% 49% at 97% 4%,
        rgba(162, 119, 255, 0.1) 0%,
        rgba(162, 119, 255, 0) 100%
      ),
  ...
  .el-dialog__body {
    ...
    .box {
      ...
      &::before {
        ...
        right: 0;
        bottom: 0;
        background: url("../../../../../assets/zhushou/yuyinbg.png") no-repeat center
          center;
        background-size: 100% 100%;
        // filter: blur(5px);
        z-index: -1;
