<template>
	...
	<div class="chatInput"  :class="{ 'chatInput-mobiles': isMobile, 'chatInput-pc': !isMobile }">
		<div v-show="isInsertPrompt" class="yayi-editor-wrapper">
			<div ref="insertpromptbox" class="yayi-editor scroll-display-none" @input="setDivInput()" contenteditable="true">
			</div>
		...
		<w-textarea v-show="!isInsertPrompt" ref="textareaRef" @keydown="setInputEnter($event)" v-model="text"
    :placeholder="isContentLengthLimit() ? placeholder : '在此输入您的问题'"
    :auto-size="{ minRows: 1, maxRows: 6 }"
    :max-length="isContentLengthLimit() ? 80 : null" />

		<span 
		:disabled="imgUrl == '' && insertpromptText == '' && text == ''" class="sendBtn" :class="{ sendBtnMobile: isMobile }" @click="setInput" v-if="!dialogueInputLoading"
		style="
			padding: 10px 20px;
			...
			line-height: 20px;
			text-align: justify;
			font-style: normal;height: 40px;background: linear-gradient( 270deg, #31CDCA 0%, #169E9A 100%);border-radius: 20px;">发 送</span>
		<i v-if="!dialogueInputLoading && isHaveTtsId()" class="vedioBtn" id="btn_control" @click="openYY('ly')" :style="isMobile?'':'top:28px;right:100px;'">
			<img src="/src/assets/chatTheme/mic.svg"
			class="sendVoiceBtn"
			...
			<div v-else class="vedioLoaingBtn" :style="isMobile?'':'top:-2px;'">
				...
				<CoolStopCircleLineWe size="28" color="#169E9A" />
			</div>
		</i>
		
		<CvUpload v-if="route.name == 'chat'"></CvUpload>
		<div v-if="dialogueInputLoading && uploadImgStatusLoading" class="sendBtn"><w-spin /></div>
		<Vue3Lottie v-if="dialogueInputLoading && !uploadImgStatusLoading" class="loadingAnimate" :animationData="toJSON" :height="55" :width="55"  :style="isMobile?'':'top:11px;right:10px;'"/>
	</div>
	...
	<el-dialog
		...
		<div class="box">
			...
			<div style="width:364px;height:364px;border-radius:50%;background: rgba(255,255,255,.1);margin:20px auto 0;position:relative;">
				<div style="width:264px;height:264px;border-radius:50%;background: rgba(255,255,255,.1);margin:auto;  position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);">
					<div style="width:192px;height:192px;border-radius:50%;background: rgba(255,255,255,.1);margin:auto;position:absolute;top:50%;left: 50%;transform:translate(-50%, -50%);">
						<img src="/src/assets/chatTheme/chatzhushou.png" class="boxbg">
					</div>
				</div>
			</div>
      ...
      <van-popup v-model:show="warningShow" round position="bottom" :close-on-click-overlay="false">
        <div class="warning-cont">
          <img src="/src/assets/chatTheme/error-warning-line.png">
          <p class="tip1">非常抱歉，这个问题我无法回答</p>
          <p class="tip2">您可以尝试联系人工客服</p>
          <p class="tip3">客服在线时间：09:00-22:00</p>
          ...
          <p class="tip5" @click="warningShow = false">算了</p>
        </div>
      </van-popup>
		</div>
...
import toJSON from '/@/assets/json/chatcircle.json';
...
.chat-dialog{
	...
	.chatbg,.el-dialog__body,.el-dialog__header{
		background: radial-gradient( 70% 50% at 91% 76%, rgba(103,209,255,0.2) 0%, rgba(103,209,255,0) 100%), radial-gradient( 69% 49% at 97% 4%, rgba(162,119,255,0.1) 0%, rgba(162,119,255,0) 100%), rgba(255,255,255,0.8);
...
.warning-cont{
  background: url("/@/assets/chatTheme/warning-bg.png") no-repeat;
