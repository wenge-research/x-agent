<template>
	...
	<div v-if="false" class="chatInput" :class="{ 'chatInput-mobile': !isMobile, 'chatInput-pc': isMobile }">
		...
		<w-button
			...
			:style="!isMobile ? '' : 'position:absolute;top:18px;right:20px;'"
		...
		<i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="!isMobile ? '' : 'top:28px;right:80px;'"
		>
			<img
		...
		<div class="refresh" @click="refreshPage" v-if="isMobile">
	...
	<div v-if="!isMobile" class="chatInput-large">
		<!-- 输入 -->
		<div v-if="inputType == 'input'" class="chatInput-large-input">
			<div class="left" @click="checkInputTypeHandler('voice')">关闭</div>
			<div class="right">
				<w-textarea
					v-show="!isInsertPrompt"
					ref="textareaRef"
					...
					:max-length="200"
				/>
				<div v-if="!dialogueInputLoading" class="right-sendBtn" @click="setInput">
					<img :src="sendIconLarge" alt="" />
				</div>
				<Vue3Lottie
					v-if="dialogueInputLoading && !uploadImgStatusLoading"
					class="right-loadingAnimate"
					:animationData="toJSON"
					:height="80"
					:width="80"
					:style="!isMobile ? '' : 'top:11px;right:10px;'"
		...
		<!-- 语音 -->
		<div v-else class="chatInput-large-voice" :class="[(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag ? '' : 'justify-between']">
			<div class="keyboard" @click="checkInputTypeHandler('input')">
				<img src="/@/assets/chat-large/keyboard.png" alt="" />
			</div>
			<div class="center" :class="[(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag ? 'ml156' : '']">
				<!-- 未说话 -->
				<div v-if="(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING') && !chatFlag" class="speak" @click="openYY('ly')">
					<iconpark-icon name="mic-fill" color="#fff" size="48" style="margin-right: 16px"></iconpark-icon>
					点击开始说话
				</div>
				<!-- 说话中 -->
				<div v-else class="speaking">
					<div class="vedioLoaingBtn" :style="!isMobile ? '' : 'top:-2px;'">
						<div class="time-box">
							<span class="start-taste-line">
								<hr class="hr1" />
								<hr class="hr2" />
					...
					<div class="speaking-text">您请说</div>
				</div>
			</div>
			<div v-if="!(btnStatus === 'UNDEFINED' || btnStatus === 'CLOSED' || btnStatus === 'CLOSING')" class="stop" @click="stopYY">停止</div>
		</div>
	</div>
	<el-dialog
		title=""
	...
	>
		<div class="box">
			...
			<div style="width: 364px; height: 364px; border-radius: 50%; background: rgba(255, 255, 255, 0.1); margin: 20px auto 0; position: relative">
				<div
					style="
						width: 264px;
						height: 264px;
...
import sendIconLarge from '/@/assets/chat-large/send.svg';
...
const checkInputTypeHandler = (type: string) => {
...
.chatInput-large {
	...
	&-voice {
		...
		.center {
			...
			height: 120px;
			background: #169e9a;
			box-shadow: 0px 12 32px 0px rgba(19, 67, 66, 0.1);
			border-radius: 60px;
			color: #fff;
			font-weight: 600;
			font-size: 32px;
