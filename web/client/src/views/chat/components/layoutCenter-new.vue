          ...
          <div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side"
            ...
            <div class="message-list">
              ...
              <div style="display: flex" :style="{ padding: isMobile ? ' 0 10px' : ' 0 30px' }"
                ...
                <w-avatar v-if="!isMobile" style="margin-right: 8px" :size="28"><img
                    :src="getAppDetail()?.robotIcon || BotAvatar" /></w-avatar>
              ...
              <div :style="{ padding: isMobile ? ' 0 10px' : ' 0 30px' }" v-for="(item, index) of chatList" :key="index">
                <!-- {{ item }} -->
                <YYMessageNew :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                  ...
                  :matterGuide="item.matterGuide" :finishReason="item.finishReason" @handleFillForm="handleFillForm(item.matterGuide)"></YYMessageNew>
                <YYMessageNew :answer="item.answer"  :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                  :id="item.id + ''" :dialogueFileIds="item.dialogueFileIds || []"
                  :dialogueFolderIds="item.dialogueFolderIds || []" :paragraph="item.paragraph || ''"
                  :skillId="item.skillId || ''" :param="item.param" :question="item.question" :loading="item.loading"
                  ...
                  :plainText="item.plainText" :matterGuide="item.matterGuide" :finishReason="item.finishReason" @handleFillForm="handleFillForm(item.matterGuide)">
                </YYMessageNew>
              </div>
            </div>
...
const ChatModule = defineAsyncComponent(() => import('./chatModule/index-new.vue'));
