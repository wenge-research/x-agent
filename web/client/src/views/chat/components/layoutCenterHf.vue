<template>
            ...
            <div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side"
              ...
              <div class="message-list">
                <div class="question-box" v-if="curRouteId == policyUrl && !isMobile">
                  ...
                  <div>
                    ...
                    <div class="quest-list">
                      <div class="item" :class="{ active: curQuestion == item.question }"
                        v-for="(item, index) in tjQuestList" @click="sendQuestion(item.question)" :key="index">
                ...
                <div :style="{ padding: isMobile ? ' 0 10px' : ' 0 30px' }" v-for="(item, index) of chatList" :key="index">
                  <yyMessageHf :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                    ...
                    :imgUrl="item.imgUrl" :sensitive="item.sensitive" :stopChatLoading="stopChatLoading"></yyMessageHf>
                  <yyMessageHf :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
                    :id="item.id + ''" :dialogueFileIds="item.dialogueFileIds || []"
                    :dialogueFolderIds="item.dialogueFolderIds || []" :paragraph="item.paragraph || ''"
                    :skillId="item.skillId || ''" :param="item.param" :question="item.question" :loading="item.loading"
                    ...
                    :plainText="item.plainText"></yyMessageHf>
                </div>
              </div>
  ...
  const ChatModule = defineAsyncComponent(() => import('./chatModule/index-hf.vue'));
