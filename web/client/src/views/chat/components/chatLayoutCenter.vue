			...
			<w-scrollbar ref="layoutScrollbarRef" class="center-side1">
				<div class="container-center">
					<div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side"
						...
						<div class="message-list" :style="!isSensitive && dialogueLoading && !chatStore.uploadImgStatus?'height:calc(100% - 60px);':'height:calc(100% - 16px);'">
							...
							display: flex; margin-bottom: 24px; padding: 12px" v-if="curRouteId == policyUrl && isMobile">
								<div class="fontSize16 text-black min-w-[20px] text-box-1716763096571080706 message-request text-black border-right" style="padding: 4px 0 0!important">
									<div style="display:flex;justify-content:space-between;background: linear-gradient( 180deg, rgba(22,158,154,0) 0%, rgba(22,158,154,0.2) 100%);border-radius: 8px;backdrop-filter: blur(1px);">
										<div style="width:90px;position:relative;">
											<img src="/src/assets/chatTheme/kefu.png" style="width:112px;max-width: 110px;position: absolute;bottom: 0;left: 0;">
											<img src="/src/assets/chatImages/man.png" style="width:112px;max-width: 110px;;position: absolute;bottom: 0;left: 0;" v-if="false">
										...
										<div style="width:calc(100% - 92px);padding:16px;z-index:1;">
									...
									<div>
										<p style="margin:16px 0;font-family: MiSans, MiSans;font-weight: 500;font-size: 16px;color: #434649;line-height: 18px;text-align: justify;font-style: normal;">您可以试着问我：</p>
										<div v-for="(item, index) in tjQuestList" @click="sendQuestion(item.question)" :key="index">
											<div style="display:inline-block;color:#646479;padding:8px 16px;margin-bottom:9px;background: rgba(255,255,255,0.9);border-radius: 2px 12px 12px 12px;border: 1px solid rgba(0,0,0,0.12)">{{ item.question }}</div>
										</div>
									</div>
								</div>
							</div>
							<div style="display: flex" :style="{ padding: isMobile ? ' 0 12px' : ' 0 30px' }"
							...
							<div :style="{ padding: isMobile ? ' 0 12px' : ' 0 30px' }" v-for="(item, index) of chatList"
								:key="index">
								<YYMessageChat :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
									...
									:imgUrl="item.imgUrl" :sensitive="item.sensitive" :stopChatLoading="stopChatLoading" :matterGuide="item.matterGuide" :finishReason="item.finishReason"></YYMessageChat>
								<YYMessageChat :answer="item.answer" :dialogueId="item.dialogueId" :citations="item.citations || []" :createTime="item.createTime"
									:id="item.id + ''" :dialogueFileIds="item.dialogueFileIds || []"
									:dialogueFolderIds="item.dialogueFolderIds || []" :paragraph="item.paragraph || ''"
									:skillId="item.skillId || ''" :param="item.param" :question="item.question" :loading="item.loading"
									...
									:plainText="item.plainText" :matterGuide="item.matterGuide" :finishReason="item.finishReason"></YYMessageChat>
							</div>
						</div>
						<div class="text-center" :style="{ height: isMobile ? '56px' : '2rem' }" style="position:relative;z-index:1000;bottom:8px;">
							<w-button type="outline" :loading="stopChatLoading" shape="round" class="stopBtn"
								...
								<span style="margin-right:5px;display:inline-block;width: 11px;height: 11px;background: #169E9A;"></span>
								停止生成
							</w-button>
						</div>
