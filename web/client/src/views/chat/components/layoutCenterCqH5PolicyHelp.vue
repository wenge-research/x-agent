				...
				<div class="container-center" :class="[showSZR ? 'pt208' : '']">
					...
					<div v-if="isMobile && !showSZR && !chatList.length" class="cq-head">
						<div class="cq-head-box">
							...
							<!-- <img v-else src="/src/assets/chongqing/policy-img.png" class="cq-head-box-left" />  -->
							<div class="cq-head-box-right">
								<!-- <img src="/src/assets/chongqing/policy-right.png" alt="" class="img"> -->
								<ul class="ul">
									<li v-for="(item, index) in tjQuestList" :key="index" @click="sendQuestion(item.question)">
					...
					<div
						:style="{ overflow: isMobile ? 'hidden' : 'scroll' }"
						class="center-side"
						:class="[isMobile && !showSZR && !chatList.length ? 'height1' : 'height2']"
						v-if="!loading"
					>
						...
						<div :class="['message-list', { 'message-list-one': chatList.length }]">
							...
							<div
								...
								:class="[showSZR ? '' : 'pt42']"
								v-if="curRouteId == policyUrl && firstTjList.length"
							>
								...
								>
									...
									<div class="inner">
										<div class="inner-title">根据您的个人信息，为您推荐一下可能适合您的政策</div>
										<div v-for="(item, index) in firstTjList" :key="index" class="inner-list" @click="sendQuestion(item)">
											<div class="dian"></div>
											{{ item }}
										</div>
									</div>
...
import { getPresetQuestionList, apiGetRecommendByUserInfo } from '/@/api/knowledge';
...
const firstTjList = ref([]);
// 推荐
const getRecommendByUserInfo = async () => {
	let res = await apiGetRecommendByUserInfo({
		applicationId: localStorage.getItem(`${route.params.appId}appId`),
		userId: sessionStorage.getItem('userId'),
	});
	firstTjList.value = res.data.filedList || [];
};
...
.cq-head {
	...
	&-box {
		...
		width: 100%;
		height: 100%;
		background: url('/@/assets/chongqing/policy-top.png') no-repeat;
		...
		&-right {
			...
			left: 46%;
			top: 26%;
			// background: url('/@/assets/chongqing/policy-right.png') right center no-repeat;
