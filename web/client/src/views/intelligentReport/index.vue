<template>
	<div class="container_chat relative" :class="wrapClass" @click="openAllDialog">
		<w-layout :class="{ MobileH: !isMobile }">
			...
			<w-layout-sider v-if="curRouteId == policyUrl" class="w-layout-sider-right flex zcflex"
				...
				<div class="chatDivFlex" :class="{ chatDivFlex2: !isMobile }" :style="particlesContainerStyle">
					...
					<div class="chatDivFlex-left">
						...
						<iconpark-icon name="writer-gaixie" color="#fff" size="20"
							class="writer-gaixie"></iconpark-icon>
						<iconpark-icon name="account-circle-fill" color="#fff" size="20"></iconpark-icon>
					</div>
					<div class="chat-content" v-show="isShowAnswer">
							...
							<div class="right">
								...
								<el-tooltip :content="showSZR ? '关闭数字人' : '连接数字人'">
									<div v-if="isSZR" @click="showSZRHandler" class="right-box">
										<img v-if="!showSZR" src="/src/assets/municipalSupervisionBureau/left-icon.png"
											class="left-icon" alt="" />
										<iconpark-icon name="account-circle-fill" color="#383D47"
											size="16"></iconpark-icon>
										<img v-if="showSZR" src="/src/assets/municipalSupervisionBureau/right-icon.png"
											class="right-icon" alt="" />
									</div>
								</el-tooltip>
							</div>
						</div>
						<div class="chat-bottom" :class="{ 'chat-bottom-mobile': isMobile }">
							...
							<div v-if="menuServiceFlag || policyListFlag || (isSZR && showSZR)"
								...
								<div v-show="isSZR && showSZR" class="szr" id="ttsa"
									style="margin-left: 30px; width: 100%; height: 100%"></div>
								<img v-if="!szrVisible" style="margin-left: 30px"
									src="/src/assets/municipalSupervisionBureau/virtual-human.png" />
							</div>
						</div>
					</div>
					<div class="chat-content" v-if="!isShowAnswer">
						<answerPage />
						<!-- <answerPageReport /> -->
					</div>
				</div>
			</w-layout-sider>
			<w-layout-sider v-if="curRouteId == judicialUrl" class="w-layout-sider-right flex"
...
const answerPage = defineAsyncComponent(() => import('./components/answerPage.vue'));
const answerPageReport = defineAsyncComponent(() => import('./components/answerPageReport.vue'));
...
const newChunks = ref('');
onMounted(async () => {
	mittBus.on('instructSet', (value) => {
		isShowAnswer.value = value;
	});
	mittBus.on('showInput', (value) => {
		isShowAnswer.value = value;
	});
