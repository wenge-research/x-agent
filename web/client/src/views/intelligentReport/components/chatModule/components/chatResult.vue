      ...
      <div class="mian-content">
        ...
        <!-- <div class="progress-box">
          <div class="flex-s file-info">
            <div class="flex-r-c">
              <img :src="ks" />
              <span style="color: #434649; margin-left: 10px" v-show="progress < 100">正在搜索全库</span>
              <span style="color: #434649; margin-left: 10px" v-show="progress == 100 && selectedFileCount">搜索完成</span>
            </div>
           
            <span style="color: #646479" v-show="progress > 30 && progress < 50">正在提炼关键字</span>
            <span style="color: #646479" v-show="progress > 70 && progress < 100">正在总结并生成回答</span>
            <span style="color: #646479" v-if="progress == 100 && selectedFileCount">查找了 <span>{{ fileTotalCount
                }}</span> 篇相关资料，精选出其中
              <span>{{ selectedFileCount }}</span> 篇</span>
          </div>
          <w-progress class="progress" size="small" :percent="progress / 100" :style="{ width: progressBarWidth }"
            :show-text="false" status="normal">
          </w-progress>
        </div> -->
        ...
        <div v-show="showRresultType == 1">
          ...
          <div id="answer-content" class="answer-content" v-for="(item, index) of chatList" :key="index" ref="textRef">
            <ChatTextComponent v-if="!item.inversion && index == 0" :inversion="item.inversion" :loading="item.loading"
              :citations="item.citations" :dialogueFileIds="item.dialogueFileIds"
              :dialogueFolderIds="item.dialogueFolderIds" :paragraph="item.paragraph || ''" :skillId="item.skillId"
              :text="item.inversion ? item.question : item.answer" :as-raw-text="false" :asRawText="item.asRawText"
              :params="item.param" :imgUrl="item.imgUrl" v-model:item.isHighlightCode="item.isHighlightCode"
              :matterGuide="item.matterGuide" :finishReason="item.finishReason" @handleFillForm="item.handleFillForm">
            </ChatTextComponent>
          </div>
          <div v-if="isFollwUp">
            <div id="answer-content" class="answer-content" v-for="(item, index) of chatList" :key="index"
              ref="textRef">
            ...
            <div class="input-box">
              <w-textarea ref="textareaRef" v-model="continueSearchText" placeholder="继续提问..."
              ...
              <div class="btn-box">
                <w-space>
                  <i class="vedioBtn" id="btn_control">
                    <img src="/src/assets/chatImages/yuyin.svg" class="sendVoiceBtn" @click="openYY('ly')" v-if="
                      btnStatus === 'UNDEFINED' ||
                      btnStatus === 'CLOSED' ||
                      btnStatus === 'CLOSING'
                    " />
...
//截图
const capture = async () => {
  const captureElement = document.getElementById('answer-content');
  console.log('domtoimage', domtoimage)
  console.log('截图ss')
  console.log('captureElement',captureElement)
  try {
    const domImg = await domtoimage.toBlob(captureElement, {
      quality: 0.5,
      bgcolor: '#ffffff',
    })
...
.brainMap {
  ...
  border: 1px solid;
  margin-top: 12px;
  border-image: linear-gradient(163deg, rgba(255, 255, 255, 1), rgba(255, 255, 255, 0.08)) 1 1;
}
