<template>
	...
	<div class="apptemplate-chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
		...
		<div class="new-chat" @click="refreshPage">
			...
			<div class="icon-new" style="display: flex;width: 40px;height: 40px;background: rgba(255,255,255,0.5);
border-radius: 8px;justify-content: center; align-items: center;">
				<iconpark-icon name="chat-new-line" size="20" color="#3F4247"></iconpark-icon>
			</div>
		</el-tooltip>
		</div>
			<div class="textarea">
     ...
     <w-select v-model="model"  placeholder="请选择模型" style="width: auto;min-width: 246px;position: absolute;bottom: 10px;left: 16px;">
      <w-option v-for="(item, index) in llmList" style="color: #828894" :label="item.modelName" :value="item.modelId"></w-option>
		...
		<w-button
			...
			:style="isMobile ? '' : 'position:absolute;bottom:15px;right:17px;'"
			:disabled="imgUrl == '' && insertpromptText == '' && text == ''"
			class="sendBtn"
			:class="{ sendBtnMobile: isMobile }"
			@click="setInput"
		>
		<iconpark-icon name="send-plane-fill" size="23.77" color="#296BD7"></iconpark-icon>
		</w-button>
	  <!-- <el-upload
    style="position: absolute;right:56px;bottom:14px;"
  ...
  >
	...
	justify-content: center;align-items: center;border: 2px solid #3F4247;
	
	 ">
		<iconpark-icon name="add-line" color="#3F4247"></iconpark-icon>
	</div>
	</div>
  </el-upload> -->
		<getAudio
		...
		<i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="isMobile ? '' : 'bottom:14px;right:104px;color: rgb(65, 134, 244);'"
		>
			<iconpark-icon 
		...
		<Vue3Lottie
			...
			style="position:absolute;bottom:15px;right:17px;"
			class="loadingAnimate"
			:animationData="toJSON"
			:height="55"
			:width="55"
			:style="isMobile ? '' : 'top:5px;right:10px;'"
		/>
	</div>
