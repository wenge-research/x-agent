					...
					<div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side" v-if="!loading">
						...
						<div class="message-list">
							<!-- <div class="appTemplate-head" v-if="curRouteId == policyUrl && !isMobile && (virtualHumanLogo || getAppDetail()?.identityIcon || getAppDetail()?.greeting)">
								<div v-if="virtualHumanLogo" style="height: 100px;align-self: flex-start;">
									<img :src="virtualHumanLogo" alt="" class="appTemplate-head-facadeImageUrl" />
								</div>
								
								<div v-if="getAppDetail()?.identityIcon || getAppDetail()?.greeting" style="flex: 1;overflow: hidden;margin-left: 24px;padding: 18px 0">
									<div class="title">
