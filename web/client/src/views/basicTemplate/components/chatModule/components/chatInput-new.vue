<template>
	...
	<div class="basicTemplate-chatInput" :class="{ 'chatInput-mobile': isMobile, 'chatInput-pc': !isMobile }">
		...
		<!-- 关芯助理 -->
		<div v-if="false" class="select-template">
		...
		<w-button
			...
			:style="isMobile ? '' : 'position:absolute;bottom:12px;right:10px;'"
			:disabled="imgUrl == '' && insertpromptText == '' && text == ''"
			class="sendBtn"
			:class="{ sendBtnMobile: isMobile }"
			@click="setInput"
		>
			<img src="/src/assets/send.svg" alt="" class="img">
		</w-button>
		<!-- <el-upload
    style="position: absolute;right:56px;bottom:10px;"
	...
	justify-content: center;align-items: center;border: 2px solid #3F4247;
	 ...
	 ">
		...
		<!-- <i
			...
			id="btn_control"
			@click="openYY('ly')"
			:style="isMobile ? '' : 'top:calc(50% - 13px);right:56PX;color: rgb(65, 134, 244);px;'"
		>
			<iconpark-icon
		...
		<Vue3Lottie
			...
			:height="48"
			:width="55"
			:style="isMobile ? '' : 'top:0px;right:10px;'"
		/>
	</div>
