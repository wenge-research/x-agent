<template>
	<div class="basicTemplate relative flex flex-col h-full w-full" :class="{ centerMobile: isMobile }">
				...
				<div class="container-center">
					<div style="" :style="{ overflow: isMobile ? 'hidden' : 'scroll', height: heightSide }" class="center-side" v-if="!loading">
						...
						<div class="message-list">
							<div
								class="basicTemplate-head"
								v-if="
									curRouteId == policyUrl && !isMobile && (!isDefaultImg(facadeImageUrl) || getAppDetail()?.identityIcon || getAppDetail()?.greeting)
								"
							>
								<div v-if="!isDefaultImg(facadeImageUrl)" style="height: 100%; margin-right: 12px">
									<img :src="facadeImageUrl" alt="" class="basicTemplate-head-facadeImageUrl" />
								</div>
								<div>
									<div class="title">
							...
							<div v-if="tjQuestList?.length" class="quest-list">
								<div class="quest-list-title">你可以试着问我</div>
							...
							<div
								...
								:style="{ padding: isMobile ? ' 0 10px' : '0 36px 0 0' }"
							...
							>
								...
								<div
									style="border: 1px solid #fff; max-width: 91%"
									class="fontSize16 text-black min-w-[20px] text-box-1716763096571080706 dark:bg-[#fff] message-request text-black border-left prologue"
								>
									{{ getAppDetail()?.prologue || '您好，很高兴为您服务！请问有什么可以帮您的？' }}
								</div>
							</div>
							<div :style="{ padding: isMobile ? ' 0 10px' : ' 0' }" v-for="(item, index) of chatList" :key="index">
								<YYMessageBasicTemplate
									:answer="item.answer"
					...
					<!-- <w-popover placement="top" width="460" trigger="hover" content-class="disclaimer-popover">
						<div v-if="getmzsm()" class="disclaimer">
...
const isDefaultImg = (url: any) => {
	...
	let result = false;
	if (
		url.includes('default-logo-1') ||
		url.includes('default-logo-2') ||
		url.includes('default-logo-3') ||
		url.includes('default-logo-4') ||
		url.includes('default-logo-5') ||
		url.includes('default-logo-6') ||
		url.includes('default-logo-7')
	) {
		result = true;
	}
	return result;
...
.basicTemplate {
	&-head {
		...
		.message {
			...
			-webkit-line-clamp: 2; /* 控制显示的行数，这里设为2行 */
			overflow: hidden;
		}
	}
}
