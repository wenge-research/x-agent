<template>
	<div class="container_chat relative" :class="wrapClass" @click="openAllDialog">
		<w-layout :class="{ MobileH: !isMobile }">
			...
			<w-layout-sider class="w-layout-sider-right flex zcflex" :resize-directions="previewData.active && route.params.appId != 0 ? ['left'] : []">
				...
				<div class="footer-copyrightInfo" :class="{ 'footer-copyrightInfo-mobile': isMobile }" v-if="!isMobile">
...
const layoutCenterLeft = defineAsyncComponent(() => import('./components/layoutCenterLeft.vue'));
...
const wrapClass = computed(() => {
	return [isMobile.value ? 'container_chat_mobile' : 'flex'];
});
