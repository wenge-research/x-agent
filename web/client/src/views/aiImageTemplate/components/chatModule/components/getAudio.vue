<template>
    ...
    <p v-if="recordingStatus">{{ recordingStatus }}</p> -->
		<i
			...
			:style="props.isMobile ? '' : 'bottom:13px;;right:100px;color: rgb(65, 134, 244);'"
		>
			<img v-if="!isRecording" src="/src/assets/chatImages/yuyin-new.png" class="sendVoiceBtn" />
			<div v-else class="vedioLoaingBtn" :style="props.isMobile ? '' : 'bottom:-2px;'">
				<div class="time-box">
...
import { csVoiceToText } from '/@/api/chat'; 
...
const startRecording = async () => {
	try {
		...
		if(appInfo.sttId==69){
			mediaRecorder.value = new MediaRecorder(stream.value, { mimeType: 'audio/ogg' });
		}else{
...
const sendAudio = async () => {
	...
	let appInfo = JSON.parse(window.localStorage.getItem(`${route.params.appId}`));
	if(appInfo?.ttsId==69){
		const blob = new Blob(chunks.value, { type: 'audio/ogg' });
	...
	try {
		...
		if(appInfo.sttId==69){
			const obj={
				...
				sttId:69,
				fileUrl:data.data[0].urlPath ,
				merge:1
			}
			const response =await csVoiceToText({...obj})
			if (response.status === 200) {
