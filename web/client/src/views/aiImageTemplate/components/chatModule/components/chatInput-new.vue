<template>
	...
	<div style="position: relative" class="apptemplate-chatInput"
		...
		<div class="title-flag">
			<img src="../../../../../assets/aiVideoSplitting/img_pic.png" alt="">
			<span>图片生成</span>
		</div>
		<div class="textarea">
			...
			<w-textarea v-show="!isInsertPrompt" :class="{other:fileList.length > 0 ? true :false,}" ref="textareaRef"
				...
				<div class="type-item">
					...
					<div class="proportion" v-show="isShowProportion" @mouseleave="isShowProportion = false">
						<div class="proportion-item" :class="{ 'active': resolutionValue.name === item.name }" v-for="(item, index) in proportionData" :key="item.name" @click="chooseProportion(item)">
...
<!-- 
		<el-upload style="position: absolute;right:60px;bottom:13px;" ref="upload" action="#" :multiple="info?.type != 'workflow' && info?.type != 'dialogue'?false:true"
			:limit="info?.type != 'workflow' && info?.type != 'dialogue'?1:10" :on-exceed="handleExceed" v-model:file-list="fileListOrgin" :show-file-list='false'
			:http-request="uploadHandler" :before-upload="beforeUpload" :accept="info?.type != 'workflow' && info?.type != 'dialogue'?'pdf,word,txt,ofd,jpg,jpeg,png':'pdf,word,txt,xlsx,xls'"
			:disabled="isJieXi"
			class="uploadFile">
			<el-tooltip style="width: 100%;height: 100%;" effect="dark" placement="top">
				<template #content>
					<div class="uploadtxt">最多上传{{info?.type != 'workflow' && info?.type != 'dialogue' ?1:10}}个文档，单个最大{{info?.type != 'workflow' && info?.type != 'dialogue'?'15':'20'}}MB</div>
					<div class="uploadtxt">支持{{info?.type != 'workflow' && info?.type != 'dialogue'?'.doc、.docx、.pdf、.txt、.ofd、.png、.jpg':'.doc、.docx、.xls、.xlsx、.pdf、.txt'}}格</div>
    ...
    <el-tooltip effect="dark" content="按需搜索网页" placement="right" >
				<el-button v-if="showNetworkFlag && isShowNetWork"
				style="position:absolute;bottom:13px;left:16px;border:0!important;width: 106px;
				...
				<iconpark-icon name="global-line" size="17" :color="!chatStore.$state.isNewWork?'#3F4247':'rgb(77, 107, 254)'" style="margin-right:7px;"></iconpark-icon>
...
text-align: left;
font-style: normal;"
:style="{color:chatStore.$state.isNewWork?'#4D6BFE':'#3F4247'}">联网搜索</span>
				</el-button>
		</el-tooltip>
		<getAudio v-if="(csStt || rsjTts) && !dialogueInputLoading && isHaveTtsId()" :isMobile="isMobile" @sendAudio="sendAudio"
			:dialogueInputLoading="dialogueInputLoading"></getAudio>
		<span v-if="!csStt && !rsjTts && !dialogueInputLoading && isHaveTtsId()">
			<speech-ali ref="speechAlBox" v-if="ttsConfig && ttsConfig.strategy === 'sttAliyunStrategy'" :appId="ttsConfig.appId"
				@changeResultText="changeResultText"></speech-ali>
		...
		:height="55" :width="55" :style="isMobile ? '' : 'bottom:2px;right:5px;'" />
