			...
			<w-layout-sider v-if="curRouteId == policyUrl" class="flex w-layout-sider-right zcflex"
				...
				<div class="chatDivFlex" :class="{ chatDivFlex2: !isMobile, bgorImg: !backgroundImageUrl() }"
					...
					<div class="chatDivFlex-left" v-if="!implantSwitch">
						...
						<div class="top">
							<div class="smallImg" style="margin-bottom:16px;" @click="historyDialogClick">
								<!-- 历史对话 -->

								<div class="chat-box" title="历史对话">
									<!-- <img :src="chatLine" alt="历史对话" /> -->
									<iconpark-icon name="chat-history-line" color="#86909C" size="20"></iconpark-icon>
								</div>
								<div class="title">
									历史
								</div>
							</div>
							<div class="smallImg" @click="handleNumber" v-if="isSZR">
								<div class="img">
									<iconpark-icon name="robot-3-line" size="20" color="#86909C "></iconpark-icon>
								</div>
								<div class="title">
									数字人
								</div>
						...
						<div class="footer">
							<div class="smallImg" v-if="getmzsm()">
							...
							<div class="smallImg" style="display: flex;justify-content: center;align-items: center;">
						...
						line-height: 20px;">字号大小</div>
										<div class="font-box-slider">
											<el-slider v-model="sliderValue" :step="1" show-stops :marks="sliderMarks"
												:min="0" :max="2" :show-tooltip="false" @change="sliderChange">
											</el-slider>
										</div>
									...
									<template #reference>
										<div>
											<iconpark-icon class="ishover" name="font-size-2" color="#2065D6" size="20"
												v-show="showliderValue" @click="font = true"></iconpark-icon>
											<iconpark-icon class="nothover" name="font-size-2" color="#383D47" size="20"
												v-show="!showliderValue"></iconpark-icon>
										</div>
									</template>
					...
					<div class="chat-content"
						...
						<div class="top-logo" v-if="!isMobile">
							...
							<div class="newTalk" @click="refreshPage" @mouseenter="isTouch = true"
								@mouseleave="isTouch = false">
								<div class="img">
									<iconpark-icon name="chat-new-line" color="#1747E5" class="add-icon"
										size="18"></iconpark-icon>
									<!-- <iconpark-icon name="chat-new-line" color="#3F4247" class="add-icon"
										size="18" v-show="!isTouch"></iconpark-icon>
										<iconpark-icon name="chat-new-line" color="#1747E5" class="add-icon"
										size="18" v-show="isTouch"></iconpark-icon> -->
								</div>
								<div class="txt">
							...
							<!-- <el-tooltip :content="showSZR ? '关闭数字人' : '连接数字人'">
									<div v-if="isSZR" @click="showSZRHandler" class="right-box">
										<img v-if="!showSZR" src="/src/assets/municipalSupervisionBureau/left-icon.png" class="left-icon"
											alt="" />
										<iconpark-icon name="account-circle-fill" color="#383D47" size="16"></iconpark-icon>
										<img v-if="showSZR" src="/src/assets/municipalSupervisionBureau/right-icon.png" class="right-icon"
											alt="" />
									</div>
								</el-tooltip> -->
							<!-- </div> -->
                  ...
                  <div class="change-box">
					...
					<div v-if="chatStore.isShare" class="share">
						...
						<div class="share-content" ref='shareRef'>
							...
							<div v-for="(item, index) of isSelected" :key="item.id" class="main"
								:class="{ hide: !hidden[index] }">
								<YYMessageAppTemplate :answer="item.answer" :dialogueId="item.dialogueId"
									...
									:dialogueFileIds="item.dialogueFileIds || []" :answerFlag="item.answerFlag"
								...
								<div class="checkBox" @click="handleSelected(item, index)" v-if="checkBoxHidden">
									<img src="../../assets/selected.png" alt="" v-if="!isSelected[index].checked">
									<img src="../../assets//notselected.png" alt="" v-else>
								</div>
							</div>
							<div v-if="isQRcode" style="position: relative;height: 148px;">
								<img style="height: 100%; width: 100%;" src="../../assets/img/QRcode.png" alt="">
								<qrcode-vue style="position: absolute;right: 76px; bottom: 30px;" :value="qrcodeValue"
									:size="88" />
							</div>
						</div>
						<div class="share-footer">
							<div
								style="padding:0 368px 0 296px;width: 100%;height: 100%;display: flex;justify-content: space-between;align-items: center;">
								<div class="left"></div>
								<div class="right">

									<el-button @click="handleSaveImg('fuzhi')" style="width: 80px!important;
...
border-radius: 4px!important;">
									...
									<el-button @click="handleSaveImg('baocun')" :loading="saveImgFlag" style="width: 120px; 
...
border:0!important;">
										<iconpark-icon name="download-line" color="#FFFFFF" style="margin-right:4px;"
											size="16"></iconpark-icon>
										保存为图片
									</el-button>
									<!-- <el-button @click="generatePDF" style="width: 120px!important;
									background: #2065D6!important;
									border-radius: 4px!important;
									font-family: MiSans, MiSans!important;
									font-weight: 500!important;
									...
									<input ref="fallbackInput" type="text" :value="textToCopy"
										style="position:fixed;top:-100px" readonly>
								</div>
							</div>
						</div>
					</div>
	...
	<!-- <el-dialog v-model="dialogVisibleHtml" modal-class="html-dialog"
	 title="" close-on-click-modal width="1000" top="5vh" :before-close="handleCloseHtml" destroy-on-close>
	...
	<el-dialog v-model="dialogVisibleHtml" modal-class="html-dialog" title="" close-on-click-modal width="1000"
		top="5vh" :before-close="handleCloseHtml" destroy-on-close>
		<div style="background-color: #fff; margin-top: 20px; position: relative">
			<!-- 加载状态 -->
			<div v-if="iframeLoading" style="
          position: absolute;
          top: 0;
          left: 0;
          right: 0;
        ...
        ">
			...
			<!-- iframe -->
			<iframe ref="iframe" :srcdoc="htmlJs" style="width: 100%; height: 700px; border: none"
...
// import { htnmJs } from './components/defaultHtml';
...
import { apiUploadShare, getEchartsData } from '/@/api/chat/index'
...
const generatePDF = (async () => {
	const element = shareRef.value; // 获取要转换为PDF的DOM元素
  const result = await captureImg2(shareRef.value)
	//const canvas = await html2canvas(element); // 将DOM元素渲染到Canvas上
	const pdf = new jsPDF({
  ...
  console.log(result.dataUrl)
	pdf.addImage(result.dataUrl, 'PNG', 0, 0); // 将图像添加到PDF中
...
onMounted(async () => {
	mittBus.on('open-html-modal', openHtmlDoalog)
	mittBus.on('text-event', (text) => {
...
const dialogVisibleHtml = ref(false);
const handleCloseHtml = () => {
	dialogVisibleHtml.value = false;
};
...
const openHtmlDoalog = (value) => {
	getEchartsHtml();
	dialogVisibleHtml.value = value;
};

const htmlJs = ref('')

//获取echarts图表html
const getEchartsHtml = async () => { 
	iframeLoading.value = true
	try {
		const res = await getEchartsData({
		applicationId: localStorage.getItem(`${route.params.appId}appId`),
		content: receivedText.value,
	});
	if(res.code === '000000') {
		htmlJs.value = res.data
		iframeLoading.value = false
	}
	} catch (error) { 
		console.log(error)
