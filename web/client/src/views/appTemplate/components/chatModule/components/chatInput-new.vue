<template>
	...
	<div style="position: relative" class="apptemplate-chatInput"
		...
		<div class="flex-box" v-if="applicationQuickCommandList.length > 0" style="display: flex;">
			<div v-for="(item, index) in applicationQuickCommandList" :key="item.id" style="margin-right: 20px;cursor: pointer;">
				<span class="commandContent" @click="addQuestionZl(item.commandContent)">
					<img class="video-file" src="/src/assets/zl.png"
						alt="" />{{item.commandName}}
				</span>
			</div>
		</div>
		<div class="textarea">
			<div>
				<div class="questionList">
					<ul v-if="lxQuestionList.length > 0">
						<li v-for="(item,index) in lxQuestionList" :key="item.question" @click="addQuestionvalue(item.question)">{{item.question}}</li>
			...
			<w-textarea v-show="!isInsertPrompt" :class="{other:fileList.length > 0 ? true :false,}" ref="textareaRef"
				...
				:max-length="isContentLengthLimit() ? 3000 : null" 
				/>
		</div>
		...
		<el-upload style="position: absolute;right:60px;bottom:13px;" ref="upload" action="#"
			...
			<el-tooltip style="width: 100%;height: 100%;" effect="dark" placement="top">
				<template #content>
					...
					<div class="uploadtxt">
						支持{{info?.type != 'workflow' && info?.type != 'dialogue'?'.doc、.docx、.pdf、.txt、.ofd、.png、.jpg':'.doc、.docx、.xls、.xlsx、.pdf、.txt'}}格
					</div>
...
font-style: normal;" :style="{color:chatStore.$state.isNewWork?'#4D6BFE':'#3F4247'}">联网搜索</span>
		...
		<Vue3Lottie v-if="dialogueInputLoading && !uploadImgStatusLoading" class="loadingAnimate" :animationData="toJSON"
...
<script lang="ts" setup>
	...
	// 写作
	import { getPresetQuestionList, getTtsConfigList, getCategoryTemplates,getAssociationQuestionList } from '/@/api/knowledge';
    ...
    const applicationQuickCommandList = ref([])
  ...
  const handleUploadSuccess = (params) =>{
    successCount.value++;
    // 检查是否所有文件都上传完成
  ...
  const getText = async() =>{
    ...
    if (info?.type != 'workflow' && info?.type != 'dialogue') {
      ...
      ElMessage({
        ...
        result = await apiGetText({ fileUrlList:fileUrlList.value })
        //result = await apiGetImg({ fileUrlList })
	...
	const uploadHandler = async (params) => {
		...
		try {
			...
			if (res.status === 200) {
				...
				fileUrlList.value.push(res.data.data[0].url)
				chatStore.setFileUploadList(tempFileList.value);
			}
	...
	const changeTextare = () => {
		...
		if (textarea != null) {
			...
			console.log('1111',originalValue)
			// getAssociationQuestionListFun()
			// 临时设置
	...
	const getAssociationQuestionListFun = async () => {
		
			let res = await getAssociationQuestionList({
				question: text.value,
				applicationId: localStorage.getItem(`${route.params.appId}appId`),
			});
			lxQuestionList.value = res.data;
	...
	onMounted(() => {
		...
		applicationQuickCommandList.value = JSON.parse(sessionStorage.getItem("applicationQuickCommandList"));
		getTtsConfigListFun();
