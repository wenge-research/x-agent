<template>
	<div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
		<w-scrollbar class="scrollbarWap" outer-class="scrollbarOut" ref="scrollbarRef" @wheel.prevent="onHandleScroll">
			<ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
				<li
					...
					"
				>
					<SvgIcon v-if="isActive(v)" class="layout-navbars-tagsview-ul-li-iconfont" name="cool-radio-button-line-we" :size="12" />
					<SvgIcon :name="v.meta.icon" v-if="!isActive(v) && getThemeConfig.isTagsviewIcon" class="pr5" />
					<span>{{ setTagsViewNameI18n(v) }}</span>
					<template v-if="isActive(v)">
						<SvgIcon
							name="cool-refresh-line-we"
							class="ml5 layout-navbars-tagsview-ul-li-refresh"
							@click.stop="refreshCurrentTagsView($route.fullPath)"
						/>
						<SvgIcon
							name="cool-close-line-we"
...
const onCurrentContextmenuClick = async (item: RouteItem) => {
	...
	if (!getCurrentRouteItem(item)) return Message({ type: 'warning', message: '请正确输入路径及完整参数（query、params）' });
...
const transUrlParams = (v: RouteItem) => {
	...
	if (v.meta?.isDynamic) {
		 ...
		 * 2、通过事件调用时，路由的 path 不是原始定义的路由格式，如：/params/dynamic/details/we-design-pro/111/我是动态路由测试tagsViewName(非国际化)
		 *
...
const onHandleScroll = (e: WheelEventType) => {
	scrollbarRef.value.$refs.containerRef.scrollLeft += e.wheelDelta / 4;
};
// tagsView 横向滚动
const tagsViewmoveToCurrentTag = () => {
	nextTick(() => {
		...
		let scrollRefs = scrollbarRef.value.$refs.containerRef;
		// 当前滚动条滚动宽度
