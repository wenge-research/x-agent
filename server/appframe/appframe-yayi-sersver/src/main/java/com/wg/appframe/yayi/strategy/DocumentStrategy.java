...
@Slf4j
public class DocumentStrategy implements YayiApiStrategy{
    ...
    public YayiResult run(String fileUrl, YayiParam yayiParam) {
        ...
        if (yayiParam instanceof DocumentParam) {
            DocumentParam param = (DocumentParam) yayiParam;
            result = runDocument(fileUrl, param);

            // 判断是否需要重试
            if (null != result && null != result.getRetryFlag() && result.getRetryFlag()) {
                // 重试次数
                int retryTimes = null != yayiParam.getRetryTimes() ? yayiParam.getRetryTimes() : null != documentConfig.getRetryTimes() ? documentConfig.getRetryTimes() : 1;
                int retryInterval = null != yayiParam.getRetryInterval() ? yayiParam.getRetryInterval() : null != documentConfig.getRetryInterval() ? documentConfig.getRetryInterval() : 300;
                for (int i = 0; i < retryTimes; i++) {

                    // 重试等待时间
                    ThreadUtil.safeSleep(retryInterval);
                    log.info("===>runDocument 第{}次重试 ,总重试次数:{} ,重试间隔:{}毫秒, error:{}", i + 1, retryTimes, retryInterval, result.getMsg());
                    result = runDocument(fileUrl, param);
     ...
     * @return
     */
    public DocumentResult runDocument(String fileUrl, DocumentParam yayiParam) {
        DocumentResult result = new DocumentResult();
        ...
        try {
            ...
            // 设置雅意接口请求的头部参数
            YayiApiUtils.setHeader(builder, documentConfig.getUri(), yayiConfig, yayiParam);

            // 封装文档解析接口入参
            DocumentParam documentParam = documentParam(fileUrl, yayiParam);
     ...
     * @return
     */
    private DocumentParam documentParam(String fileUrl, DocumentParam param) {
        DocumentParam documentParam = new DocumentParam();
        documentParam.setId(IdUtil.simpleUUID());
        documentParam.setUrl(fileUrl);
        DocumentParam.Content content = new DocumentParam.Content();
        documentParam.setContent(content);
        if (StringUtils.isNotBlank(documentConfig.getUserId())) {
            content.setUser_id(documentConfig.getUserId());
        }
        if (null != documentConfig.getDoLayout()) {
            content.setDo_layout(documentConfig.getDoLayout());
        }
        if (null != documentConfig.getGetBbox()) {
            content.setGet_bbox(documentConfig.getGetBbox());
        }
        if (null != documentConfig.getTableHtml()) {
            content.setTable_html(documentConfig.getTableHtml());
        }
        if (null != documentConfig.getWatermark()) {
            content.setTable_html(documentConfig.getWatermark());
        }
        if (null != documentConfig.getFaq()) {
            content.setFaq(documentConfig.getFaq());
        }
        if (null != documentConfig.getMode()) {
            content.setMode(documentConfig.getMode());
        }

        documentParam.setRetryTimes(1);
        if (null != documentConfig.getRetryTimes()) {
            documentParam.setRetryTimes(documentConfig.getRetryTimes());
        }
        documentParam.setRetryInterval(3000);
        if (null != documentConfig.getRetryInterval()) {
            documentParam.setRetryInterval(documentConfig.getRetryInterval());
        }

        if (StringUtils.isBlank(documentParam.getUri())) {
            documentParam.setUri(documentConfig.getUri());
        }
        ...
        DocumentParam.Content content1 = param.getContent();
        if (null != content1) {
     ...
     * @return
     */
    private DocumentResult getResult(Request.Builder builder, YayiParam yayiParam) {
        DocumentResult result = new DocumentResult();
        ...
        if (StringUtils.isNotBlank(body) && body.contains("QPS访问超出限制")) {
            ...
            result = entries.toBean(DocumentResult.class);
        }
        return result;
    }
