...
@Service(LlmConstant.COMMON)
@Slf4j
public class CommoneStrategy implements LlmStrategy {
    ...
    public String generate(String question, List<YayiMessage> messagesList, StepEntity step, boolean longFlag, Consumer<AnswerStepData> answerConsumer) {
        ...
        if (answerConsumer != null) {
            ...
            modelParam.set("stream", true);
            modelParam.set("enable_thinking", true);
            result = runStream(llmParam, answerConsumer, step);
        } else {
            llmParam.setStream(false);
            modelParam.set("stream", false);
            modelParam.set("enable_thinking", false);
            CommoneCompletionResult generate = commoneLLServer.generate(llmParam, results -> {});
            JSONObject llmResult = generate.getLlmResult();
            DoubaoCompletionResult bean = llmResult.toBean(DoubaoCompletionResult.class);
            result = bean.getChoices().get(0).getMessage().getContent();
            // 处理 think 标签
            result = DeepseekStrategy.dealThink(result);
            step.setResult(result);
            //使用的token
            try {
                Integer totalTokens = bean.getUsage().getTotal_tokens();
                step.setTokenTotal(totalTokens);
            } catch (Exception e) {
    ...
    private String runStream(CommoneCompletionParam commoneCompletionParam, Consumer<AnswerStepData> answerConsumer, StepEntity step) {
        ...
        commoneCompletionParam.setStrategy(LlmConstant.COMMON);
        // 大模型推流
    ...
    /**
     * 通义千问，如果格式一样，那么这里可以复用其他大模型
     * @param llmResult
     * @param answerConsumer
     * @param stepData
     * @param stream
     * @param contentStream
     * @param reasoningContentStream
     * @param allStream
     * @param step
     */
