...
@Service(LlmConstant.MINMAX)
@Slf4j
public class MinMaxStrategy implements LlmStrategy{
    ...
    public String generate(String question, List<YayiMessage> messagesList, StepEntity step, boolean longFlag, Consumer<AnswerStepData> answerConsumer) {
        ...
        MinmaxCompletionParam doubaoCompletionParam = modelParam.toBean(MinmaxCompletionParam.class);
        ...
        if (answerConsumer != null) {
            ...
            result = minimaxiServer.generateString(question, doubaoCompletionParam);
     ...
     */
    private String runStream(String question, MinmaxCompletionParam doubaoCompletionParam, Consumer<AnswerStepData> answerConsumer, StepEntity step) {
        ...
        // 大模型推流
        minimaxiServer.generateStream(question, doubaoCompletionParam, resultData -> {
            ...
            List<MinmaxCompletionResult.Choice> choices = resultData.getChoices();
            if (CollectionUtils.isEmpty(choices)) {
                AnswerUtils.notify(stepData, answerConsumer);
                return;
            }
            MinmaxCompletionResult.Choice generateChoice = resultData.getChoices().get(0);
            String finishReason1 = generateChoice.getFinish_reason();
            MinmaxCompletionResult.ChoiceDelta message = generateChoice.getDelta();
            if (null == message) {
                AnswerUtils.notify(stepData, answerConsumer);
                return;
            }
    ...
    public List<YayiMessage.ToolCall> generateTools(String question, List<YayiMessage> messagesList, StepEntity step) {
        ...
        if (null != param && param.containsKey(AnswerStrategyContant.LLM_PARAM_TOOL)) {
            ...
            if (CollectionUtils.isNotEmpty(toolList)) {
                List<MinmaxCompletionParam.Tool> tools = toolList.stream().map(p -> BeanUtil.toBean(p, MinmaxCompletionParam.Tool.class)).collect(Collectors.toList());
        ...
        // 是否有消费者，消费大模型推流消息
        doubaoCompletionParam.setStream(false);
        MinmaxCompletionResult generate = minimaxiServer.generate(question, doubaoCompletionParam);
        List<MinmaxCompletionResult.Choice> choices = generate.getChoices();
        MinmaxCompletionResult.Choice generateChoice = choices.get(0);
        String finishReason = generateChoice.getFinish_reason();
        if ("tool_calls".equals(finishReason)) {
            MinmaxCompletionResult.Message message = generateChoice.getMessage();
            // 转换对象适应策略
            List<MinmaxCompletionResult.MessageToolCall> toolCalls = message.getTool_calls();
