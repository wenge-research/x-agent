...
echo 执行deepseek_r1_8b
echo 检查必要环境...


set docker_version=ccr.ccs.tencentyun.com/wenge/agent-llm:deepseek_r1_8b
set container_name=agent_x_deepseek_r1_8b
...
:: 启动容器，这里使用-d参数以守护进程模式运行容器，你可以根据需要修改容器名和其他参数  --gpus=all
docker run --restart=always -d  --gpus=all  -p 11436:11434 --network agent-x-network --name %container_name%  -e OLLAMA_HOST=0.0.0.0:11434   %docker_version%
