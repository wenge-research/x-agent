...
import static com.wenge.model.constants.AnswerStrategyContant.QA_CONTENT_STRATEGY;

@Service(QA_CONTENT_STRATEGY)
public class ContentStrategy implements RecommendStrategy {
    ...
    public List<RecommendQuestionResult> getRecommendQuestion(RecommendQuestionParam param) {
        ...
        List<KnowledgeData> knowledgeData = answerUtils.byDense(dto, AnswerStrategyContant.CONTENT_DENSE_FILED, applicationInfo.getQaContentScore(), step, step, applicationInfo.getQaRangeContentScore(), new Vector<>());
        if (CollectionUtil.isNotEmpty(knowledgeData)) {
            ...
            List<RecommendQuestionResult> recommendQuestionResults = knowledgeData.stream().map(p -> {
                ...
                recommend.setCategory("QA对回答的推荐");
            ...
            // 按照问题去重
            Map<String, RecommendQuestionResult> collect = recommendQuestionResults.stream().collect(Collectors.toMap(
    ...
    public List<AssociateQuestionResult> getAssociationQuestion(AssociateQuestionParam param) {
        ...
        if (CollectionUtil.isNotEmpty(knowledgeData)) {
            ...
            List<AssociateQuestionResult> associateQuestionResults = knowledgeData.stream().map(p -> {
                AssociateQuestionResult associate = new AssociateQuestionResult();
                associate.setQuestion(p.getTitle());
                associate.setAnswer(p.obtainAnswer());

                associate.setLink(p.getLink());
                ...
                if (StringUtils.isNotBlank(category)) {
                    ...
                    String typeName = collect.get(Long.parseLong(split.get(0)));
                    if (StringUtils.isNotBlank(typeName) && StringUtils.contains(typeName, "事项")) {
                        associate.setCategory(collect.get(Long.parseLong(split.get(split.size() - 1))));
                    }
                }
                return associate;
            }).collect(Collectors.toList());
            ...
            // 按照问题去重
            Map<String, AssociateQuestionResult> collect = associateQuestionResults.stream().collect(Collectors.toMap(
