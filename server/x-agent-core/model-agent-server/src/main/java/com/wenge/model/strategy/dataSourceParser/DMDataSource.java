...
@Component
@Slf4j
public class DMDataSource extends AbstractTemplate implements DataSource{
    @Autowired
    ...
    public List<String> getAllTables(String sourceJdbcUrl, String sourceJdbcName, String sourceJdbcPassword) {
        ...
        try{
            ...
            ResultSet tbrs = stmt.executeQuery("SELECT TABLE_NAME FROM ALL_TABLES;");
            while (tbrs.next()) {
    ...
    @Override
    public Map<String, Object> getAllDataFromTable(String sourceJdbcUrl, String sourceJdbcName, String sourceJdbcPassword,
    ...
    public Map<String, Object> getAllDataFromTable(String sourceJdbcUrl, String sourceJdbcName, String sourceJdbcPassword, String tableName, Integer pageNum, Integer pageSize, String bussiness,MyDatabaseOperations myDatabaseOperations) throws SQLException {
        ...
        try{
            ...
            ResultSet countRs = conn.createStatement().executeQuery("SELECT COUNT(*) AS total FROM "+tableName);
            // 处理分页数据
            ...
            // 分页查询数据
            int offset = (pageNum - 1) * pageSize;
            try (ResultSet dataRs = conn.createStatement().executeQuery("SELECT * FROM "+tableName+" LIMIT "+pageSize+" OFFSET "+offset)) {
                ResultSetMetaData metaData = dataRs.getMetaData();
                int columnCount = metaData.getColumnCount();
